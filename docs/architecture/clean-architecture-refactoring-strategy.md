...
- 기기에서만 가능한 금융 알림 파싱은 Android가 담당하되, 결과는 버전이 있는 `CaptureEnvelope.v1` 계약으로 전달한다. Shortcut도 같은 계약을 사용하고 Android만 선택적 balance branch를 만들 수 있다.
...
Android의 [CardNotificationListenerService.kt](../../android/app/src/main/java/com/household/account/service/CardNotificationListenerService.kt)는 알림 추출, 출처 판별, 원문 기록, Parser 선택, 카드·가맹점·카테고리 검증, 중복·취소, Firestore 저장, Quick Edit 실행을 모두 담당한다. [QuickEditActivity.kt](../../android/app/src/main/java/com/household/account/QuickEditActivity.kt)와 [MainActivity.kt](../../android/app/src/main/java/com/household/account/MainActivity.kt)도 각각 UI와 Command, WebView와 세션·권한·FCM을 함께 담당한다. 기존 Parser 단위 테스트는 목표 계약 테스트 체계로 전환하면서 삭제했다. 이동 전에는 [Android 공급자 Parser Golden 계약](../../functions/test/contexts/payment-capture/android-payment-ingestion/android-provider-parser-golden.contract.test.ts)으로 시간·연도 경계와 승인·취소 동작을 고정한다.
...
| Payment Capture | 결제 설정, 채널 중립 Intake, Android·Shortcut parser | registered cards, merchant rules, parser 계약 |
...
각 feature 내부:
    ...
    feature/payment-parser/
      domain/
        PaymentNotification.kt
        ParsedPaymentCandidate.kt
        parsers/
      application/
        contracts/CaptureEnvelopeV1.kt
        ParseNotificationUseCase.kt
        QueueCaptureEnvelopeUseCase.kt
        SubmitCaptureEnvelopeUseCase.kt
      ports/
...
- Parser는 Android SDK와 Firebase를 모르는 순수 코드다.
...
- SourceDetector, ParserRegistry, `CaptureEnvelope.v1` 변환을 순수 영역으로 이동
- 익명 알림 fixture 기반 전체 Parser 회귀 테스트
- Android SessionStore Port와 기존 SharedPreferences 호환 Adapter 도입
- Android `SubmitCaptureEnvelopeV1` Gateway와 로컬 재시도 Queue
