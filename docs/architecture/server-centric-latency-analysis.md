...
2. bundle을 줄이려고 모달을 동적 import로 옮겼지만, 실제 클릭 전에 해당 chunk를 준비하지 않아 비용이 첫 클릭으로 이동했습니다.
...
> 첫 화면에는 첫 화면 코드만 싣고, 사용자가 누를 가능성이 높은 상호작용 코드는 첫 paint 직후 미리 준비하며, 클릭 뒤에는 고정 timer·불필요한 effect·원격 초기화를 기다리지 않는다. 서버에서는 동일한 안전성을 두 계층이 중복 구현하지 않는다.
...
| 최신 작업 트리 | 약 117kB | 약 300kB | `/`는 약 60%, 자산 route는 직전보다 약 19% 축소 |
...
`/assets`는 요약 도넛 하나 때문에 초기 경로에 포함되던 Chart.js를 브라우저 기본 SVG로 교체해 약 368kB에서 300kB로 줄였습니다. 전용 과거 차트는 실제로 열 때만 내려받습니다. 이와 함께 다음 방식으로 계좌 클릭 비용도 줄였습니다.

- 홈 첫 paint 직후 `/assets` route를 prefetch
- 자산 페이지가 그려진 직후 계좌·자산 모달 chunk를 preload
...
| 일반 모달 첫 클릭 | 컴포넌트가 이미 bundle에 있어 즉시 표시 | 클릭 후 동적 chunk 다운로드·평가, `Portal` effect 뒤 표시 | 첫 paint 후 공통 loader로 preload, `Portal`은 같은 React commit에서 표시 |
...
5. 첫 원장 paint가 완료된 뒤에만 다음 route와 상호작용 모듈을 준비합니다.
   - `/income`
   - `/assets`
   - `/settings`
   - `/stats`
   - 지출 상세·추가·검색·카테고리·지역화폐 UI
   - 자산 추가·수정·상세·차트·명의자 UI
6. Pretendard는 외부 CDN을 기다리지 않는 자체 호스팅 dynamic subset으로 제공합니다. 시스템 글꼴로 바꿔 시각을 달라지게 하지는 않습니다.
7. 지역화폐는 별도 잔액 캐시나 background projection 없이 가구 하위 Canonical balance를 직접 구독합니다. 구형 root `balances` 조회 때문에 발생한 권한 오류만 제거했으며, Firestore 모듈은 지역화폐 카드가 실제로 렌더될 때 동적으로 불러와 작은 root bundle을 유지합니다.
...
- 화면마다 제각각 정의하던 dynamic import를 공통 loader로 통합해 preload와 실제 렌더가 같은 Promise/cache를 사용하게 했습니다.
...
자산 route의 First Load JS는 약 300kB로 리팩토링 전 기준의 약 305kB보다도 작아졌습니다. 홈을 먼저 거치면 route prefetch와 모달 preload의 효과를 추가로 받지만, 새 브라우저에서 `/assets`로 직접 진입하면 이 다운로드·parse 비용 자체는 남습니다. 전용 차트는 초기 경로에서 분리했으며, 배포 후에도 직접 진입이 목표를 넘을 때만 저빈도 자산 유형을 더 분리합니다.
...
첫 원장 paint 직후 자주 쓰는 모달과 주요 route를 미리 받으므로 post-paint 네트워크 사용량과 메모리는 증가합니다. 현재 가구 수와 “첫 클릭 속도 최우선” 요구를 반영한 의도적 선택입니다. 저사양 단말에서 첫 화면과 경쟁하는지 계측해야 합니다.
...
- 첫 화면과 route·상호작용 preload: [`AppProviders.tsx`](../../web/src/components/AppProviders.tsx)
- 공통 동적 loader: [`interactiveUiPreload.ts`](../../web/src/composition/interactiveUiPreload.ts)
