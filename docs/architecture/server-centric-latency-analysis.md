...
| 최신 작업 트리 | 약 118kB | 약 300kB | `/`는 약 60%, 자산 route는 직전보다 약 19% 축소 |
...
7. 지역화폐는 가구 하위 Canonical balance를 구독합니다. 마지막 성공값은 가구·사용자 범위의 가벼운 캐시에서 즉시 표시하고, 첫 paint 뒤 미리 받은 구독 모듈이 권위값으로 갱신합니다. 구독 모듈을 root에 정적 결합하면 `/`가 약 289kB까지 증가하므로 캐시와 원격 구독을 분리했습니다.
