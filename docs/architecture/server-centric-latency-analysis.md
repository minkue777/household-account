...
- 같은 UID의 Firebase Auth가 복원되면 마지막 검증 SessionScope로 Rules 보호 읽기를 시작합니다. 이후 백그라운드 Membership 재검증의 일시 실패가 보유내역 listener·전일 변동·통계 이력 조회를 영구 중단시키지 않으며, 인증 복원 전에 시작하지 못한 조회는 복원 직후 다시 실행합니다.
