...
- 자산 화면은 검증된 SessionScope가 준비되면 고정 대기 없이 전체 시장 시세 갱신을 시작하고, 화면이 foreground에서 보이는 동안 30초마다 반복합니다. 화면 이탈·hidden 상태에서는 중단하고 복귀 시 30초가 지났으면 즉시 재개합니다. 한 번의 전체 갱신은 모든 계좌를 하나의 원자적 반영 단위로 유지하며, 기존 자산 snapshot을 먼저 표시한 뒤 완료된 새 snapshot이 도착하면 총자산과 일간 변동을 즉시 다시 계산합니다.
