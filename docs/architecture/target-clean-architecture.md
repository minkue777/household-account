...
- `submitAndroidRawNotification`, `executeHouseholdCommand`, `executeHouseholdQuery`는 현재 소규모 운영에서 `minInstances=1`로 cold start를 줄인다. capture warm instance는 성공한 활성 Membership을 최대 5분·64개, 성공한 결제 설정 snapshot을 최대 1분·32개만 bounded process-local cache로 재사용하고 거부·실패는 cache하지 않는다. Auth·App Check와 tenant 도출은 계속 서버 경계에서 검증하며, 멤버 제거·연결 변경의 warm instance 반영이 최대 5분 늦을 수 있는 절충을 DEC-068로 수용한다.
...
- PWA·CDN runtime cache는 인증 응답, 가구·금융 API와 session 데이터를 저장하지 않는다. DEC-051에 따라 현재 build의 versioned 정적 shell과 공개 비민감 아이콘·폰트·이미지만 최대 7일 허용하고 임의 cross-origin 응답은 cache하지 않는다. 별도로 DEC-068의 first-party localStorage에는 마지막 서버 검증 UID·가구 화면과 현재 월 원장·가구별 카테고리 표시 snapshot을 둘 수 있으나 서버 권한 근거가 아니며 로그아웃·SessionScope 불일치·권위 거부 뒤 화면에 재사용하지 않는다.
...
| [DEC-068](../requirements/governance/decisions.md#dec-068) 소규모 운영 저지연 경로 | Web last-verified render hint, Android capture journal·QuickEdit snapshot, warm Functions bounded cache | 첫 paint 선표시·배경 Auth 수렴, capture Worker 미경유 direct submit, follow-up 선내구화, callable minInstances=1, Auth·App Check·가구 격리·멱등성 유지 |
