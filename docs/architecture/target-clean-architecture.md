...
- 첫 가계부 route는 가구 이름·헤더만 따로 선표시하지 않고 헤더와 본문 골격을 같은 첫 렌더에서 만든다. 월 원장·카테고리·지역화폐의 준비 상태를 하나의 전역 barrier로 묶지 않으며, 각 첫 서버 snapshot이 도착하는 즉시 그 데이터에 의존하는 영역만 채운다. 월 지출·수입은 원장만, 잔여 예산은 원장과 카테고리만, 지역화폐 카드는 지역화폐 잔액만 기다린다. 아직 준비되지 않은 값은 과거값이나 0으로 확정하지 않고 중립 placeholder 또는 skeleton으로 표시한다.
...
| Delivery Assurance | `verifyCandidate`, `qualitySummary`, `ResolveDeploymentTarget`, `VerifyCompatibilityWindow`, `RecordDeploymentResult` | 독립 CI, 승인된 배포 작업 |
...
- `submitAndroidRawNotification`, `executeHouseholdCommand`, `executeHouseholdQuery`는 현재 소규모·비용 우선 운영에서 `minInstances=0`을 사용한다. Android 세션 교환이 발급한 Native token의 householdId·memberId는 신원 힌트이며 현재 권한을 대신하지 않는다. 모든 capture는 UID 전역 `PrincipalMembershipClaim` 한 문서를 새로 읽어 ActorContext를 만들고 Native 힌트가 있으면 현재 가구·멤버와 일치하는지 확인한다. 전역 claim이 없는 전환 데이터만 기존 Membership view·canonical 검증으로 fallback한다. 완료된 Membership·결제 설정 결과의 process-local TTL cache는 두지 않는다. Auth·Membership 검증은 모든 요청에서 유지하고 App Check는 Native capture·세션 경계에서 유지하며, 단계별 latency 계측 뒤 cold start가 목표 지연을 지배할 때에만 특정 Function의 warm instance를 재검토한다.
...
- 2026-09-11 변경: 결제 설정의 1분·32개 process-local cache를 제거했다. 카드 끝번호 수정 직후 다음 수집이 이전 설정으로 거절되는 E2E 재현을 해결하기 위해 모든 새 수집이 projection을 읽는다. 완료 전 동일 scope 조회의 Promise 병합은 유지하며, 설정 원본 전체 조회는 projection이 무효화됐을 때만 수행한다. 이어 관리자 제거 뒤 같은 JWT의 신규 수집이 저장되는 문제가 확인되어 Membership 5분 cache와 Native JWT 직접 승인도 제거했다. 제거 transaction 완료 후 다음 요청은 권위 claim 부재를 확인해 거절하고 같은 ID 복구 뒤 다시 허용한다.
...
- 대화형 경계는 Web Performance mark/measure, Functions 구조화 `interactive-latency`, Android 단조 시계와 `HHCaptureLatency`를 사용한다. 앱 시작 체감 시간은 Android Activity 생성 또는 iPhone 홈 화면 PWA Navigation 시작부터 월 원장·카테고리·지역화폐와 현재 설정이 노출하는 연간 합계가 최신 서버 결과로 성공한 뒤 실제 paint된 시점까지를 플랫폼별 독립 operation으로 측정한다. 일반 Web과 Android custom-token 세션 생성 시간은 앱 시작 표본에 포함하지 않는다. 푸시 알림은 요청 Command의 Outbox 저장 시간이 아니라 Outbox 생성부터 trigger 대기·FCM provider 응답·terminal 저장까지를 측정한다. FCM 성공은 provider 접수이며 OS 표시 완료를 뜻하지 않는다. `NoTarget`은 알림 미전송 terminal 결과로 끝내고 provider 호출 지표에서 제외하며, Outbox 재전달 로그는 `(endpoint, operation, correlationId)`별 최신 결과 하나로 축약한다. FCM 평균·P95·최대 지연은 실제 provider 성공 표본만 사용한다. 사용자·가구·거래·금액·원문은 기록하지 않으며 Android와 Functions는 결제 수집에서만 `SHA-256(observationId).slice(0, 16)` 상관키를 공유한다.
...
| [DEC-068](../requirements/governance/decisions.md#dec-068) 소규모 운영 저지연 경로 | Web server-first home read·route 내 완성 모달·mutation runtime preload·가구 단위 Position snapshot, Domain-owned atomic receipt, Native capture 권위 Membership 조회, Android capture journal·QuickEdit snapshot | 첫 가계부 화면의 stale 선표시 제거, 자주 쓰는 모달의 chunk/fallback 없는 첫 클릭 표시, 단순 Command 중복 receipt 제거, capture 권위 Membership 한 문서·설정 projection 한 문서·단일 승인 outer receipt 제거, callable minInstances=0, 공용 Auth·Membership 및 Native capture App Check·가구 격리·멱등성 유지 |
...
| [DEC-074](../requirements/governance/decisions.md#dec-074) 배포·전체 CI 분리 | Delivery Assurance wrapper의 immutable 후보 검증과 독립 CI summary | 배포는 build·대상·hash·actor·호환·smoke 검증을 유지하고 CI를 기다리지 않음. CI 실패는 별도 check로 기록하며 pass로 위장하지 않음 |
