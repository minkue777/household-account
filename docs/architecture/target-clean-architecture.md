...
- Web·Android의 보호된 요청·cache·구독은 인증 완료 뒤 만든 `SessionScope(sessionGeneration, principalUid, householdId, memberId)`를 명시적으로 받고, localStorage나 `guest` fallback을 tenant 권위로 사용하지 않는다.
...
7. client Query·listener는 `SessionScope`와 명시적 householdId를 받고 `sessionGeneration`이 바뀌면 취소·폐기한다. localStorage 조회는 composition 경계의 legacy claim 외에는 허용하지 않는다.
...
- Payment Capture는 `observationId + idempotencyKey`의 payload hash와 branch별 stage·typed 결과를 `CaptureSubmissionReceipt`로 보존한다. `Ignored`, `Rejected`, `NeedsConfirmation`, balance-only, 한 branch만 terminal인 부분 진행도 같은 key 재시도에서 이미 끝난 branch를 반복하지 않고 재생한다.
- Ledger commit 뒤 `CaptureSubmissionReceipt` 저장 전에 중단되어도 재시도는 같은 fingerprint·transport key로 Ledger 결과를 복구한 뒤 receipt를 완성한다. receipt가 없다는 이유로 두 번째 거래를 만들지 않는다.
...
서버 권위 Command로 전환해도 알림 수집이 일시적 네트워크 장애 때문에 사라지면 안 된다.
...
[DEC-035](../requirements/governance/decisions.md#dec-035)에 따라 종목 검색 함수는 이 snapshot을 단일 원본으로 읽고 인스턴스 메모리에 5분간 캐시한다. cache는 목표 데이터 모델이 아니며 cold start마다 다시 구성할 수 있어야 한다. 저장소 번들 `stocks.json`, Firestore 복제 catalog, Redis는 목표 구조에 두지 않는다.
...
- 공개 Read Contract만 같은 가구 Membership에 읽기를 허용한다.
...
- PWA·CDN runtime cache는 인증 응답, 가구·금융 API, navigation HTML과 session 데이터를 저장하지 않으며 로그아웃·SessionScope 전환·worker upgrade에서 관련 cache를 폐기한다. DEC-051에 따라 현재 build의 versioned precache와 공개 비민감 아이콘·폰트·이미지만 최대 7일 허용하고 임의 cross-origin 응답은 cache하지 않는다.
...
- Web feature/service에서 localStorage·`getStoredHouseholdKey`로 tenant를 직접 결정하거나 `guest` fallback 사용
