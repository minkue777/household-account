...
7. Web 기기는 snapshot을 IndexedDB와 메모리에 cache하고 최대 5분 간격으로 manifest generation을 확인합니다. 이는 Scheduler job이 아니라 조회 최적화입니다.
8. 기기 cache가 있으면 원격 확인을 기다리지 않고 검색하며 Storage 갱신 실패에도 마지막 성공본을 사용합니다. 최초 기기 cold cache에서 Storage를 읽지 못한 경우만 명시적 실패입니다.
...
| 종목 검색의 5분 manifest 확인 | Web 기기 IndexedDB·메모리 read model |
...
`instrumentCatalogDaily`를 Cloud Functions 2세대에 배포하고 `Asia/Seoul` 기준 매일 06:00 실행하도록 연결했습니다. 배포 직후 수동 실행으로 Cloud Storage의 `market-catalog/v1/latest.json`과 일일 압축 snapshot 생성을 확인했으며, 최초 snapshot은 국내 4,408개와 미국 13,021개, 총 17,429개 종목을 포함합니다. 2026-07-23부터 Web은 이 snapshot을 IndexedDB와 메모리에 동기화해 로컬 검색하며 `stocks.json` fallback은 사용하지 않습니다. Storage CORS는 공개 읽기 전용 catalog에 대해 `GET`·`HEAD`만 허용합니다.
