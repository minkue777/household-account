...
- 명시적으로 결정한 정기 거래 creator → 감사용
  `recurringCreatorMigrationReceipts`; 기존 마지막 처리 월 → `recurringPlans`
...
read-only reconciliation은 원문과 canonical 양쪽에서 보존돼야 하는 업무 사실만
비교합니다. 빈 creator의 확인된 Member 보정, 표시 이름 owner의 profile 참조 변환,
자산 subtype의 canonical code 정규화, 기본 카테고리의 별도 설정 문서 이동, 정기 거래
최초 적용 월 보완처럼 의도적으로 형태나 값이 바뀌는 필드는 원문 digest에서 제외하고
migration plan의 expected target와 actual target 결정 hash `MATCH`로 검증합니다. 기존
정기 거래의 `lastRegisteredMonth`는 변환 제외 항목이 아니며 canonical
`lastProcessedMonth`로 보존되어야 합니다.
