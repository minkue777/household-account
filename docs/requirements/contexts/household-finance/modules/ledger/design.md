...
폼은 메모 아래 `태그` 입력을 두고 입력 안내는 메모와 같은 형식의 `태그를 입력하세요`로 표시합니다. 선택값을 배경·테두리·안쪽 여백 없는 파란색 `#태그` 텍스트와 개별 제거 버튼으로 표시합니다. 목록·검색 결과는 메모와 태그를 같은 flex 행에 두고 태그 사이에는 텍스트 간격만 둡니다. 태그 wrapper는 `display: contents`로 별도 줄을 만들지 않으며, 공간이 부족한 태그만 자연스럽게 다음 줄로 넘깁니다. 메모는 기존 한 줄 말줄임을 유지하고 메모가 없으면 태그만 표시합니다. 검색 결과의 날짜·메모 뒤에도 같은 방식으로 태그를 이어 표시합니다. 기존 태그 추천도 같은 텍스트 형태를 사용합니다. 최대 개수에 도달했을 때만 입력 제한 안내를 표시합니다. Enter·추가 버튼·기존 태그 추천 클릭으로 확정하며 IME 조합 중 Enter는 소비하지 않습니다. 저장 시 미확정 입력도 정규화한 배열에 합칩니다. 추천은 [요구사항의 읽기 범위](requirements.md#3-소유-데이터) 안에서 이미 받은 태그 중 선택하지 않은 값과 입력 부분 일치로 최대 5개를 제시하며 추천만을 위한 서버 조회는 하지 않습니다. 목록·검색 결과 태그 텍스트는 같은 이름을 표시합니다.
...
| `LedgerSearchPolicy` | 일반 검색은 가맹점·메모·태그 부분 일치와 기존 카드 별칭/마스킹 일치의 OR; 선두 # 검색은 정규화된 태그 이름의 부분 일치 |
...
2. 대상이 active가 아니면 일반 사용자에게 `NotFound`를 반환하며 deleted·superseded 상태를 수정하거나 재삭제하지 않습니다.
3. Update patch의 허용 필드만 정규화하고 category 변경 시 Category Reference를 다시 확인합니다.
4. capture claim, source, creator, receipt metadata는 일반 Update로 바꾸지 않습니다.
5. Delete는 Transaction 본문과 provenance를 물리 제거하지 않고 lifecycleState=`deleted`, deletedAt, aggregateVersion을 기록합니다.
6. Canonical 변경 또는 deleted 전이와 receipt, Changed/Deleted Event를 한 UoW로 commit합니다.
...
| Update/Delete | Transaction version 변경 또는 deleted 전이 + receipt + Changed/Deleted Event |
...
- 앱 첫 가계부 route는 헤더와 본문 골격을 같은 첫 렌더에서 만들고, 월 원장·카테고리·지역화폐를 하나의 준비 조건으로 묶지 않습니다. 월 원장 서버 snapshot은 도착 즉시 달력·거래 목록·월 합계에 반영합니다. 잔여 예산은 원장과 카테고리가 모두 준비된 뒤 계산하고, 지역화폐 카드는 지역화폐 잔액만 독립적으로 기다립니다. 아직 준비되지 않은 값은 0이나 과거값으로 확정하지 않습니다.
- 위 세 데이터가 모두 준비된 뒤 idle 시점에는 선택 월의 이전·다음 월 원장을 `getDocsFromServer` 일회성 조회로 받아 `{SessionScope, year, month}` 메모리에만 보관합니다. 인접 월에는 listener를 열지 않습니다. 월 이동 시 보관한 서버 결과를 즉시 base로 표시하고 기존 월 listener를 종료한 뒤 새 선택 월 listener 하나를 열어 최신 snapshot으로 교체합니다. 예를 들어 7월에서 8월로 이동하면 이미 보관한 7월은 다시 사전 조회하지 않고 9월만 보충합니다. 사전 조회 실패는 선택 월 조회를 실패시키지 않으며 가구·SessionScope 변경과 로그아웃에서 모든 인접 월 힌트를 폐기합니다.
...
| SPL-004 | Domain·Emulator | reconfigure monthly | 1개월, 새 개월 수, 원본·기존 파생의 전체 필드·상태, 기존 group 경합, 무관 거래 260건 | 기존 항목과 원본 superseded, 새 active group의 증거 보존, 변경 문서만 한 트랜잭션으로 원자 재구성 | T-SPL-005 |
...
| MRG-001·002 | Client·UI | Web optimistic projection·drag/touch coordinator | pending merge, snapshot 전 연속 merge, 성공·Conflict·RetryableFailure, 500ms 경계, 활성화 전 12px/17px 이동, native dragstart, touchcancel, 합성 click | 새 ID 한 건으로 즉시 치환, 중복 drop 차단, 실패 overlay 전체 rollback·AppDialog, 16px 이내 미세 이동 허용·초과 취소, 터치 중 native drag 차단, 활성화 즉시 scroll lock, touchcancel 무명령 정리, 합성 click 한 번만 억제 | T-MRG-003 |
