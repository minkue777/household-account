...
| SPL-003 | 현재 명세 | 월 분할 항목은 그룹 ID, 순번, 전체 개월 수로 연결하고 원본 거래는 `superseded`로 보존해 그룹 전체를 같은 원본으로 원복할 수 있다. | immutable capture lineage와 source·origin·creator·카드 증거는 `LED-009`에 따라 보존한다. 취소·재구성 시 클라이언트는 화면에 보이는 파생 항목 전체의 version만 보내며, 숨겨진 원본 version은 서버 트랜잭션이 검증한다. 그룹 항목이 누락되거나 중복되면 전체를 거부한다. 저장소는 지정된 그룹과 그 원본만 선택 조회하며 가구 전체 원장을 읽지 않는다. | [월 분할 Service](../../../../../../functions/src/contexts/household-finance/ledger/application/commands/monthlySplitLifecycleService.ts), [Firebase 월 분할 Store](../../../../../../functions/src/adapters/firebase/ledger/firebaseMonthlySplitLifecycleStore.ts), [DEC-041](../../../../governance/decisions.md#dec-041) | U, I, E2E |
