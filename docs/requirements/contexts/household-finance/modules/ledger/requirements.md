...
| SEA-006 | 현재 명세 | 기존 지출 검색의 일반 검색어는 태그 이름의 부분 일치도 포함한다. `#태그이름`은 앞뒤 공백·선행 `#`를 정리하고 대소문자를 구분하지 않는 태그 이름 부분 일치로 검색한다. 지출 목록의 태그 chip을 누르면 해당 `#태그이름`으로 기존 검색을 열고, 같은 가구의 전체 기간에서 일치한 지출의 건수·금액과 월별 합계를 표시한다. | `#`만 입력하면 빈 결과이며 `#2026`·`#부산`·`#여행`은 `2026부산여행` 태그와 일치한다. 같은 문구가 메모·가맹점·카드에만 있는 거래는 `#` 검색에 포함하지 않는다. chip 선택은 거래 편집을 함께 열지 않는다. 태그 추가·삭제는 기존 검색 Projection과 성공 후 재조회에 반영하고 실패하면 원래 태그와 결과로 복구한다. 태그 하나가 여러 번 입력됐더라도 거래 금액은 한 번만 합산한다. 기존 카드 검색·거래 유형·상태·가구 격리와 SEA-003·SEA-004의 최신 응답·전체 결과 계약을 유지한다. | [검색 matcher](../../../../../../web/src/lib/expenseService.ts), [태그 chip](../../../../../../web/src/components/expense/ExpenseTags.tsx), [검색 연결](../../../../../../web/src/components/home/LedgerPage.tsx), [검색 합계](../../../../../../web/src/components/search/SearchResultList.tsx) | U, C, UI, E2E; T-SEA-004 |
...
| T-SEA-004 | 현재 명세 | 월을 가로지르는 같은 여행 태그·비슷한 태그·태그와 같은 메모·중복 태그, 빈 `#`, 대소문자·앞뒤 공백, 태그 수정 성공·실패 / 일반 부분 검색·`#` 태그 부분 일치·목록 chip 선택·태그 제거 뒤 새로고침 / 일반 검색은 태그를 포함하고 `#` 검색은 이름에 검색어를 포함하는 태그의 지출만 한 번씩 합산, chip은 편집 없이 검색을 열며 전체·월별 건수와 금액이 일치, 저장 성공은 결과 갱신·실패는 복구 | SEA-001, SEA-004, SEA-006 |
