...
|---|---|---|---|---|---|
| LED-001 | 현재 명세·목표 보완 | 선택 월과 거래 유형별로 일반 조회 가능한 거래를 날짜·시각·ID 결정 순서로 제공한다. 브라우저 Web·PWA·Android WebView는 모두 같은 공개 Firestore Read Contract를 실시간 구독하고 생성·수정·삭제 Command는 낙관적 Projection으로 즉시 화면에 반영한 뒤 Canonical 결과와 snapshot으로 수렴한다. 첫 화면용 월 원장 cache도 같은 정렬 정책을 적용하여 실시간 snapshot 도착 전후에 항목 순서가 바뀌지 않는다. | 조회는 검증된 SessionScope의 householdId와 날짜 범위를 사용해 `householdId + date` 복합 index로 제한한다. 거래 유형과 lifecycle 가시성은 모든 runtime에 동일하게 적용한다. 기존 기기에 정렬되지 않은 배열이 저장돼 있어도 읽는 시점에 정규화한다. typed rejection·version conflict이면 해당 낙관적 변경만 rollback하며 별도 기간 Query·focus 재조회·30초 polling을 사용하지 않는다. transactionType 누락 문서는 지출, lifecycleState 누락 문서는 active로 호환하고 `superseded`, `deleted` 또는 legacy `deletedAt`이 있는 문서는 목록·검색·합계에서 제외한다. | [expenseService](../../../../../../web/src/lib/expenseService.ts), [Ledger Optimistic Projection](../../../../../../web/src/features/ledger/application/ledgerOptimisticProjection.ts), [DEC-065](../../../../governance/decisions.md#dec-065) | U, Contract, I, E2E |
...
|---|---|---|---|
| T-LED-001 | 호환·현재 명세 | transactionType 없는 기존 문서, 월·양끝 포함 기간, 같은 날짜의 정렬되지 않은 기기 cache, 같은 시각 여러 ID, active 외 상태, NoData·원천 실패 / 지출 조회 / legacy 지출 포함, cache 첫 화면과 실시간 결과가 모두 날짜·시각·ID 결정적 내림차순을 지키며 실패를 빈 결과로 축약하지 않음 | LED-001, SYS-002 |
