...
| MRG-001 | 현재 명세 | 같은 목록의 서로 다른 지출을 대상 거래로 합치고 대상의 표시 정보를 유지하며 금액을 합산하되 모든 원본은 `superseded`로 보존한다. | 서버는 `commandId`로 새 merged transaction ID를 결정해 성공 응답으로 반환한다. Web은 이전 target을 수정하지 않고 target/source 제거 overlay와 반환 ID의 생성 overlay를 한 작업으로 투영하며, 이 ID와 version으로 `A+B=M` 직후 `M+C=N`을 수행할 수 있어야 한다. 진행 중 중복 합치기는 차단하고 typed 실패는 해당 overlay 전체를 rollback한 뒤 앱 대화상자로 알린다. 서버는 원본별 표시 정보와 immutable capture lineage를 보관하고 version map을 검증하며, DEC-056에 따라 leaf 원본까지 평탄화한다. 겹치는 leaf·순환·불완전 snapshot은 write 0건으로 거부한다. 한 lineage가 취소되면 현재 active인 공유 merge 결과만 복원 근거로 사용해 파생 거래를 제거하고 취소되지 않은 leaf를 복원하며, deleted·superseded 과거 merge를 근거로 일반 삭제 거래를 되살리지 않는다. | [useDragAndDrop](../../../../../../web/src/components/expense/hooks/useDragAndDrop.ts), [expenseService](../../../../../../web/src/lib/expenseService.ts), [DEC-041](../../../../governance/decisions.md#dec-041), [DEC-056](../../../../governance/decisions.md#dec-056) | U, I, UI, E2E |
...
| T-MRG-003 | 현재 명세 | Web pending merge, 연속 drop, 서버 성공·Conflict·RetryableFailure, touchcancel / optimistic projection·상호작용 / old target/source 대신 새 merged ID 한 건만 표시, pending 중 중복 호출 차단, 실패 시 세 overlay 전체 rollback과 AppDialog 안내, drag·scroll 상태 정리 | MRG-001, MRG-002 |
