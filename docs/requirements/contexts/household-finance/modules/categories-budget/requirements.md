...
| 데이터 | 소유 범위 | 비고 |
|---|---|---|
| `households/{householdId}/categoryCatalog/current` | categories 배열의 안정 ID, 이름, 색상, 월 예산, 순서, 상태, 개별 version과 catalogVersion | 한 가구 Catalog Aggregate를 한 문서에 저장하는 단일 정본입니다. |
| 기본 카테고리 정의 | `living`, `childcare`, `fixed`, `food`, `etc`의 초기 이름·색상·순서 | 최초 초기화 정책입니다. |
| 가구 기본 카테고리 참조 | Catalog의 `defaultCategoryId` 유효성 정책 | categories와 같은 문서·트랜잭션에서 변경합니다. |
| 예산 조회 모델 | 카테고리별 사용액·비율·초과액, 월 잔여 예산 | [거래 원장 모듈](../ledger/requirements.md)의 조회 결과에서 계산합니다. |

거래의 `categoryId` 값과 가맹점 규칙의 category mapping은 이 모듈의 식별자를 참조하지만 해당 문서는 각 소유 모듈이 관리합니다. 과거 물리 문서 ID는 Catalog의 `categoryAliases`에 보존해 기존 참조를 안정 ID로 해석하며, 별도 카테고리 Projection이나 settings 문서를 읽고 쓰지 않습니다. archive process와 receipt는 개별 문서로 유지합니다.

Catalog는 `schemaVersion: 1`과 경로에 맞는 householdId를 사용합니다. categoryId는 중복될 수 없고, 기본값이 null이 아니면 존재하는 active 항목을 가리켜야 합니다. 개별 version은 1 이상, catalogVersion은 0 이상의 정수이며 별칭은 존재하는 안정 ID만 가리키고 다른 안정 ID와 충돌할 수 없습니다. 잘못된 저장 문서를 빈 목록으로 처리하지 않습니다. 서버 Writer는 문서 한도에 여유를 두어 UTF-8 JSON 크기 800,000바이트 초과를 저장 전에 거부합니다.
...
- [가구·접근 모듈](../../../access-household/modules/household-access/requirements.md): 가구 범위와 온보딩에서 기본 카테고리 초기화를 호출할 권한을 제공합니다.
...
|---|---|---|---|---|---|
| CAT-001 | 현재 명세 | 새 가구 온보딩에서 가구에 카테고리가 하나도 없으면 생활비, 육아비, 고정비, 식비, 기타를 지정 순서로 초기화하고 기타를 기본 카테고리로 설정한다. | 일부만 있으면 누락 기본값은 자동 보충하지 않는다. 초기화는 Category 소유 Application·Store의 별도 멱등 UoW이며, 가구 경로의 단일 Catalog에 entries와 기본 참조를 함께 기록한다. 여러 가구의 같은 categoryId는 가구별 Catalog 안에서 독립적이다. | [Access command handler](../../../../../../functions/src/bootstrap/commands/accessHouseholdCommandHandlers.ts), [Category store](../../../../../../functions/src/adapters/firebase/categories/firebaseCategoryCatalogStore.ts) | I |
| CAT-002 | 목표 명세 | 카테고리의 이름, 색상, 월 예산을 추가·수정하고 순서를 저장하며, 삭제 요청은 과거 참조를 보존하는 archive로 처리한다. | 공통 색상 선택지를 변경할 때 실제 가계부에서 사용 중인 색상값과 선택지는 유지한다. 서로 유사한 기존 색은 미사용 여부를 확인한 경우에만 교체한다. 예산 미입력은 null이고 음수·NaN은 거부한다. 과거 거래의 categoryId와 표시 정보는 변경하지 않는다. 설정 참조는 현재 기본 카테고리로 변경하고 archived 카테고리를 신규 선택 목록에서 제외한다. 재활성화와 hard delete는 제공하지 않는다. | [CategorySettings](../../../../../../web/src/components/settings/CategorySettings.tsx), [categoryService](../../../../../../web/src/lib/categoryService.ts), [DEC-015](../../../../governance/decisions.md#dec-015) | U, I, E2E |
...
- 카테고리 예산에 음수·`NaN`을 저장하지 못하도록 Command와 Catalog 저장 경계에서 검증합니다.
- hard delete는 제공하지 않으며 기본 카테고리 archive 금지를 서비스 경계에서도 강제합니다. 보관한 항목의 표시 정보와 과거 참조는 유지합니다.
- 일부 카테고리만 있는 경우 누락된 기본값을 자동 보충하지 않습니다. 이는 현재 명세지만 마이그레이션 정책이 필요할 수 있습니다.
- Web과 Android는 같은 Catalog의 기본 카테고리를 사용합니다. 이전 문서가 남아 있어도 이를 새 정본보다 우선하지 않습니다.
- Android QuickEdit은 조회 실패와 실제 빈 카테고리를 같은 기본 다섯 항목으로 표시합니다.
- Catalog 저장 위치 전환과 Android QuickEdit의 표시 전용 fallback 계약 변경은 별개입니다. fallback 제거는 별도 요구사항으로 다룹니다.
...
| T-CAT-002 | 목표 | 같은 빈 가구에 초기화 명령 두 번 동시 실행 / 처리 / Catalog 한 문서에 기본 키마다 항목 한 개 | CAT-001 |
