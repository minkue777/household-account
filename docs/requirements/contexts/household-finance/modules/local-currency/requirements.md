...
| BAL-004 | 현재 명세 | Web은 Canonical 가구 하위 경로에서 Home Preferences가 선택한 유형의 최신 잔액을 직접 구독하고 첫 서버 snapshot 뒤 표시한다. 유형이 하나뿐이면 그 유형을 자동 표시한다. | localStorage 표시 Snapshot과 최초 Firestore cache snapshot은 사용하지 않는다. 여러 유형인데 선택이 없을 때 임의의 첫 문서를 표시하지 않으며, 서버값을 한 번 표시한 뒤 transient listener 오류가 발생해도 현재 화면값을 지우지 않는다. | [balanceService](../../../../../../web/src/lib/balanceService.ts), [DEC-008](../../../../governance/decisions.md#dec-008), [DEC-068](../../../../governance/decisions.md#dec-068) | I, UI |
...
| T-BAL-004 | 현재 | 최초 Firestore cache/server snapshot, 선택 유형의 최신 잔액 변경, 유형 하나, 여러 유형·선택 없음, 문서 없음 / 첫 렌더와 Web Canonical 구독 / cache 값은 표시하지 않고 첫 서버값부터 선택 유형 또는 유일 유형만 반영 | BAL-004 |
| T-BAL-005 | 목표 | 한 가구에 경기·대전·세종 잔액 알림 / 저장·유형별 조회 / 세 유형의 최신값을 독립 유지하고 서로 덮어쓰지 않음 | BAL-002, DEC-008 |
| T-BAL-006 | 목표 | 최초 서버 read 실패와 서버값 표시 뒤 listener 실패 / Web·Android 조회 / 최초 실패는 과거 local 값으로 숨기지 않고 잔액 없음과 구분하며, 현재 세션에서 이미 표시한 서버값은 transient 오류만으로 지우지 않음 | BAL-004 |
