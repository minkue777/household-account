...
| BAL-004 | 현재 명세 | Web app shell은 Canonical 가구 하위 경로에서 지역화폐 최신 잔액을 직접 구독하고 첫 서버 snapshot 뒤 Home Preferences가 선택한 유형을 표시한다. 유형이 하나뿐이면 그 유형을 자동 표시한다. 이 구독은 내부 route와 연·월 변경을 넘어 유지한다. | localStorage 표시 Snapshot과 최초 Firestore cache snapshot은 사용하지 않는다. 구독 key는 `{sessionGeneration, householdId}`이며 로그아웃·SessionScope 변경에서 종료한다. `permission-denied`로 listener가 종료되면 마지막 in-memory 잔액은 유지하고 Session 권위 결과가 같은 scope이면 epoch로 재연결하며, 다른 scope가 확정될 때만 값을 폐기한다. 여러 유형인데 선택이 없을 때 임의의 첫 문서를 표시하지 않는다. | [balanceService](../../../../../../web/src/lib/balanceService.ts), [DEC-008](../../../../governance/decisions.md#dec-008), [DEC-068](../../../../governance/decisions.md#dec-068) | I, UI |
...
| T-BAL-004 | 현재 | 최초 Firestore cache/server snapshot, 선택 유형의 최신 잔액 변경, 유형 하나, 여러 유형·선택 없음, 문서 없음, 내부 route·연월·scope 전환 / 첫 렌더와 Web Canonical 구독 / cache 값은 표시하지 않고 첫 서버값부터 선택 유형 또는 유일 유형만 반영하며 route·연월 변경에는 같은 구독을 유지하고 scope 종료에서 해제 | BAL-004 |
| T-BAL-005 | 목표 | 한 가구에 경기·대전·세종 잔액 알림 / 저장·유형별 조회 / 세 유형의 최신값을 독립 유지하고 서로 덮어쓰지 않음 | BAL-002, DEC-008 |
| T-BAL-006 | 목표 | 최초 서버 read 실패와 서버값 표시 뒤 transient/permission-denied listener 실패, 권한 해석의 같은/다른 scope / Web·Android 조회 / 최초 실패는 과거 local 값으로 숨기지 않고 잔액 없음과 구분하며, 이미 표시한 서버값은 해석 중 유지하고 같은 scope면 epoch 재연결·다른 scope 확정 뒤 폐기 | BAL-004 |
