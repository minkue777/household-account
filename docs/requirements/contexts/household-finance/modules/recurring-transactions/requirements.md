...
| REC-002 | 목표 명세 | 서버 Scheduler는 사용자 접속 여부와 무관하게 `Asia/Seoul` 기준 매일 00:00에 활성 정기지출을 처리하고 실행일이 도래한 각 plan·월 거래를 한 번만 등록한다. 생성된 거래는 `source=recurring`, `cardType=recurring`, 표시값 `정기지출`을 보존하며 수동 입력으로 표시하지 않는다. | 짧은 달은 말일로 보정한다. 같은 날짜 Scheduler 중복 호출과 재시도는 결정적 `planId:YYYY-MM` execution으로 한 건에 수렴한다. 과거 문서에 `cardType=manual`이 남아 있어도 `source=recurring`이면 조회 화면은 `정기지출`로 표시한다. | [recurringExpenseService](../../../../../../web/src/lib/recurringExpenseService.ts), [DEC-009](../../../../governance/decisions.md#dec-009) | U, I, E2E |
