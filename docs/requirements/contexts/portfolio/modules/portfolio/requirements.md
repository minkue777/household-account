...
| T-AST-011 | 현재 UI | 전체와 명의자 A·B가 있고 각 일간 변동 조회 응답 시간이 다름 / 자산 페이지 진입 후 A·B 필터 전환 / 모든 필터의 변동을 병렬로 미리 준비하고 전환 순간 해당 profileId의 총자산·변동만 즉시 함께 표시하며 다른 필터의 변동을 잠깐 표시하지 않음 | AST-009 |
