...
| `households/{householdId}/assetSnapshots/{localDate}` | 날짜별 자산 스냅샷의 조회 계약을 소유합니다. 예약 작업은 공개 스냅샷 기록 명령을 통해서만 씁니다. |
...
|---|---|---|---|---|---|
| AST-001 | 현재 명세 | 예적금, 주식, 코인, 부동산, 금, 대출 자산을 이름, 소유자, 통화, 잔액, 메모, 순서와 함께 관리한다. 자산 수정 UI는 선택 자산의 유형·세부 유형·입력값으로 첫 렌더를 완성하며 기본 예금 폼을 먼저 표시하지 않는다. 자산 추가 UI도 첫 렌더부터 선택 명의와 기본 유형·세부 유형을 반영하고, 기본 명의가 없거나 허용 목록에 없으면 첫 허용 명의를 사용한다. | 이름은 필수이다. 잘못된 숫자를 0으로 바꾸는 현재 UI는 검증 개선 대상이다. 자산 목록 금액은 고정폭 숫자로 자간을 넓히지 않고 한 줄 표시한다. | [asset types](../../../../../../web/src/types/asset.ts), [AssetAddModal](../../../../../../web/src/components/assets/AssetAddModal.tsx), [AssetEditModal](../../../../../../web/src/components/assets/AssetEditModal.tsx), [AssetCard](../../../../../../web/src/components/assets/AssetCard.tsx) | U, UI, I, E2E |
| AST-002 | 현재 명세 | 활성 자산만 합계에 포함하고 대출은 절댓값을 음수로 계산한다. | 소유자는 현재 멤버 이름 문자열이다. | [assetMath](../../../../../../web/src/lib/assets/assetMath.ts) | U |
| AST-003 | 현재 명세 | 자산 순서를 결정적으로 저장한다. 자산 삭제는 AST-006의 논리 삭제를 따르고 이력·보유내역을 보존한다. | 목표 삭제·복구·영구 purge 경계는 AST-006만 따른다. 순서 변경은 대상의 중복·누락과 version 경합을 거부하고 전체 성공 또는 전체 실패여야 한다. 모바일 순서 변경은 500ms 사용자 정의 롱프레스를 사용하고 활성화 전 직선거리 16px 이내의 미세 이동은 허용하되 초과하면 취소한다. 터치 중 브라우저 기본 HTML drag·context menu를 차단하고 활성화 즉시 스크롤을 잠그며, `touchcancel`은 저장 없이 상태를 해제하고 롱프레스 뒤 합성 click은 한 번만 소비한다. 데스크톱 HTML drag는 유지한다. | [AssetList](../../../../../../web/src/components/assets/AssetList.tsx), [assetService](../../../../../../web/src/lib/assetService.ts), [DEC-017](../../../../governance/decisions.md#dec-017) | UI, I, E2E |
...
| AST-007 | 현재 명세 | production 사용자 가구에는 sample·demo 자산을 생성하는 Command·버튼·공개 함수를 제공하지 않는다. | fixture seed가 필요하면 production과 분리된 demo tenant·개발 build에서만 실행하고 demo 표식·단일 Unit of Work·결정적 제거 경로를 갖춘다. | [assets page](../../../../../../web/src/app/assets/page.tsx), [assetService](../../../../../../web/src/lib/assetService.ts), [production 사용자 흐름 E2E](../../../../../../web/e2e/portfolio-journeys.spec.ts) | C, UI, 보안 E2E |
| AST-008 | 목표 명세 | `AssetSnapshotProjector`는 total·financial과 현재·직전 owner/type scope의 합집합을 기록하여 사라진 scope에는 명시적 0원을 저장한다. | 자산이 전혀 없는 가구도 total·financial 0원을 기록한다. 0원은 `NoData`가 아니다. 과거 이력도 날짜별 Snapshot으로 보존하며 조회 시 직전 Snapshot을 기준으로 변동액을 계산한다. 기존 이력이 없는 첫 Snapshot은 비교 기준 없음으로 처리한다. | [firebaseAssetSnapshotProjection](../../../../../../functions/src/adapters/firebase/portfolio/firebaseAssetSnapshotProjection.ts), [asset stats](../../../../../../web/src/app/assets/stats/page.tsx) | U, I, E2E |
| AST-009 | 목표 명세 | Asset의 명의는 표시 이름이나 memberId가 아니라 `{ kind: 'household' }` 또는 `{ kind: 'profile', profileId }`의 안정적인 `ownerRef`로 저장하고 명의자별 합계·Snapshot도 profileId를 dimension key로 사용한다. 자산 도넛 그래프 위에는 `전체 / 선택 가능한 활성 명의자들 / +` 필터를 두며 `+`로 dependent 명의자를 추가한다. 자산 페이지는 전체와 표시 대상 명의자의 일간 변동을 미리 조회하며, 필터를 전환할 때 총자산과 일간 변동은 같은 profileId 범위의 준비된 값만 즉시 함께 표시하고 직전 필터의 값을 조합하지 않는다. 같은 날짜에 자산 페이지로 재진입할 때는 가구별 마지막 일간 변동을 먼저 표시하고 새 계산 결과로 교체한다. | profileId는 같은 가구의 Access `AssetOwnerProfile`이어야 한다. 일반 자산 UI에는 명의자 삭제를 제공하지 않고 관리자만 dependent 프로필을 논리 보관한다. 보관되었거나 `selectionVisibility=hidden`인 프로필은 신규 자산 선택과 필터에서 제외하지만 기존 자산·과거 Snapshot 조회는 유지하며 공동 자산은 가짜 프로필 대신 `household`를 선택한다. 이전 날짜의 일간 변동 cache와 다른 가구의 cache는 표시하지 않는다. | [DEC-037](../../../../governance/decisions.md#dec-037), [DEC-069](../../../../governance/decisions.md#dec-069) | U, C, UI, I, 보안 E2E |
...
- 자산 이력은 날짜별 `assetSnapshots`로 통일합니다. 구형 `asset_history`는 운영 마이그레이션 원본으로만 보존하며 런타임 조회·저장을 하지 않습니다.
...
| T-AST-004 | 목표 | 전날에만 owner A·type stock이 있고 오늘 자산이 0개인 가구 또는 이력이 없는 첫날 / snapshot projection / 사라진 dimension은 명시적 0원이며 날짜별 단일 저장·멱등 replay와 직전 조회 1회를 보장함 | AST-008 |
...
| T-AST-008 | 현재·회귀 | 중복·누락 자산 ID 순서, stale version과 정상 전체 순서, 종속 Position이 있는 자산 / 재정렬과 논리 삭제 / 잘못된 순서·stale version은 원자 거부하고 정상 순서는 version과 함께 한 번 변경하며 삭제는 종속 이력을 보존하고 물리 삭제를 호출하지 않는다. | AST-003, AST-006 |
...
| T-AST-010 | 현재 UI | 예금·부동산 등 서로 다른 유형과 허용 세부 유형의 자산 / 추가·수정 모달 첫 렌더·자산 목록·요약 도넛 렌더 / 추가 모달은 effect 전부터 선택 명의·기본 유형·세부 유형을 표시하고 기본 명의 누락·비허용 시 첫 허용 명의를 사용하며, 수정 모달도 실제 유형과 세부 유형이 선택되고 금액은 비례폭 한 줄이며 도넛은 12시 경계가 겹치는 점선 원 없이 연속 원형으로 표시 | AST-001, AST-009, DEC-068 |
| T-AST-011 | 현재 UI | 전체와 명의자 A·B가 있고 같은 날짜의 마지막 일간 변동 cache와 각 일간 변동 조회 응답 시간이 다름 / 다른 화면에서 자산 페이지 재진입 후 A·B 필터 전환 / 마지막 가구별 변동을 즉시 표시하고 모든 필터의 변동을 병렬로 다시 준비하며, 전환 순간 해당 profileId의 총자산·변동만 함께 표시하고 다른 필터·이전 날짜·다른 가구 값을 잠깐 표시하지 않음 | AST-009 |
| T-AST-012 | 현재 UI | 모바일 500ms 경계, 활성화 전 12px/17px 이동, native dragstart, touchcancel, 롱프레스 뒤 합성 click / 자산 순서 변경 / 12px 이동은 허용하고 17px 이동은 취소하며, 터치 중 native drag를 차단하고 touchcancel은 저장 없이 drag·scroll 상태를 정리하며 합성 click 한 번만 억제한 뒤 일반 click은 자산 상세를 연다 | AST-003 |
...
| 자산 논리 삭제는 종속 Position·배당·이력을 보존하며 일반 삭제 Writer는 물리 삭제를 호출하지 않는다. | I | AST-003, AST-006 |
