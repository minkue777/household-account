...
1. Presentation은 Access의 활성 `AssetOwnerProfile` Firestore Read Model 구독과 `QueryPortfolio` 결과를 조합해 도넛 위에 `전체`, `selectionVisibility=visible`인 활성 명의자 프로필, `+` 순서의 필터를 렌더링합니다. 공동 자산 scope가 필요하면 `가구 공동`을 typed household 항목으로 표시합니다. 숨김 프로필은 신규 선택과 필터에서만 제외하고 기존 ownerRef 해석을 위해 Read Model에는 보존합니다.
...
2. 수정·삭제 Command에는 화면이 읽은 `aggregateVersion`을 `expectedVersion`으로 반드시 전달합니다. 수정 낙관적 Projection은 필드와 함께 다음 `aggregateVersion`도 같은 tick에 예측해, 저장 직후 다시 연 화면이 이미 성공 중인 자기 변경의 이전 version을 재사용하지 않게 합니다. 같은 Client가 첫 수정의 서버 응답 전에 동일 자산을 다시 수정하거나 삭제하면 각 화면 변경은 즉시 순서대로 Projection하되 Command는 자산별 FIFO로 직렬 전송하고, 후속 Command는 선행 성공 version을 `expectedVersion`으로 사용합니다. 선행 Command가 실패하면 아직 전송하지 않은 후속 Command를 보내지 않고 그 연속 변경을 모두 rollback합니다. 재진입 표시용 자산 cache는 화면을 즉시 그리는 데만 사용하고 Command의 권위 version으로 사용하지 않습니다. 첫 `fromCache=false` 서버 snapshot 전에 편집하면 화면에는 실제 변경 필드만 즉시 Projection하되 전송은 서버 snapshot까지 기다립니다. 서버 version이 달라지면 편집 시작 원본과 최신 권위 상태를 필드별로 비교합니다. 사용자가 실제로 바꾼 모든 필드가 서버에서 먼저 바뀌지 않았을 때만 최소 patch를 최신 version 위에 안전하게 적용하고, 하나라도 서버에서 먼저 바뀌었으면 `ASSET_VERSION_MISMATCH`로 rollback합니다. 이 검사는 startup cache, 오래 열린 화면, 같은 Client의 선행 Command, 외부 평가 갱신에 동일하게 적용하며 같은 필드를 자동 덮어쓰지 않습니다. 삭제는 대상의 내용 전체가 편집 시작 원본과 같을 때만 최신 version으로 이어 붙입니다. Client의 동일 값 사전 검사를 통과해 서버가 수락한 수정 Command는 정규화 뒤 값이 같더라도 하나의 Aggregate 변경으로 취급해 version을 정확히 1 올리며, 화면이 예측한 version과 서버 version이 어긋나지 않게 합니다. 서버가 성공하면 이어지는 실시간 snapshot으로 확정하고, typed rejection·version conflict·transport 실패이면 해당 mutation만 최신 base snapshot으로 rollback합니다.
...
5. 같은 Aggregate에서 서버 응답을 아직 받지 못한 mutation을 둘 이상 겹치지 않습니다. 앞선 Command가 성공했지만 snapshot만 늦은 경우에는 그 Canonical entity와 version을 다음 Command의 기준으로 이어서 사용할 수 있습니다. 다른 가구·다른 자산의 구독에는 임시 Asset·Position이 노출되지 않으며 화면 unmount 뒤에도 완료 snapshot으로 임시 상태를 정리합니다.
...
- 과거 이력 전환: `functions/scripts/migrate-asset-history.mjs`가 구형 날짜별 금액과 명의자 연결을 검증하고 누락된 Snapshot만 create합니다. 현재·과거 통계는 `assetSnapshots`만 조회하며 구형 이중 쓰기와 fallback 조회는 제거합니다. 변동액은 반환된 날짜별 잔액의 차이로 계산합니다.
- 마이그레이션 시 기존 Snapshot은 덮어쓰지 않으며 원본에 없는 유형·명의자 값을 추정해 채우지 않습니다. 원본은 복구와 이전에 열린 Web 클라이언트의 읽기 호환을 위해 보존합니다.
...
| AST-003 | Application·Firebase Adapter | ReorderAssets·논리 DeleteAsset | 중복/누락 순서, stale version, 종속 Position 존재 | 잘못된 순서·stale version은 전체 무변경, 삭제는 논리 상태만 변경하고 종속 이력을 보존 | T-AST-008 |
| AST-003 | Client·UI | AssetList long-press coordinator | 500ms 경계, 활성화 전 12px/17px 이동, native dragstart, touchcancel, 합성 click | 16px 이내 미세 이동 허용·초과 취소, 터치 중 native drag 차단, 활성화 즉시 scroll lock, touchcancel 무저장 정리, 합성 click 한 번만 억제하고 다음 일반 click 허용 | T-AST-012 |
...
| AST-008 | Domain·Projector·E2E | AssetSnapshotProjector | 전날만 존재한 owner/type, 오늘 자산 0개, 유효 0원, 과거 이력을 이전한 날짜, 이력이 없는 첫날 | 직전·현재 scope 합집합의 명시적 0 snapshot과 날짜별 단일 저장·멱등 replay | T-AST-004 |
