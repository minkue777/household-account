...
| 자산 스냅샷 명령 | 스냅샷 값을 계산하지만 `households/{householdId}/assetSnapshots/{localDate}` 저장은 포트폴리오의 기록 계약에 위임합니다. |
...
| GOLD-001 | 현재 명세 | 실물 금은 돈 단위 수량과 원/돈 시세로, `KRXGOLD1KG`·`KRXGOLD100G` 금현물 Position은 g 단위 수량과 네이버 KRX 금시장 원/g 시세로, 금 ETF는 일반 KRX 주식 방식으로 평가한다. 실물 금의 KRX 금시장 기준가는 계산 정밀도를 유지하되 화면에는 원 단위로 반올림해 소수점 없이 표시한다. | 구형 memo의 N돈 표현을 실물 금 수량으로 읽는다. 금현물 Position의 시세에 돈 환산 배수 3.75를 적용하지 않는다. | [시세 공급자 Adapter](../../../../../../functions/src/adapters/firebase/portfolio/firebasePortfolioMarketData.ts), [평가 Policy](../../../../../../web/src/lib/assets/holdingValuation.ts) | U, C, I |
...
| MARKET-001 | 현재 명세 | Web의 시세·코인 검색·배당 조회는 Auth·가구 Membership을 검증한 Household Query를 통해 국내 주식을 Naver 주식 API, KRX 금현물 종목과 실물 금을 Naver KRX 금시장 페이지, 미국 종목을 Nasdaq와 USD/KRW 환율, 코인을 Upbit에서 조회하고, 배당 카드는 가구별 배당 Projection을 읽는다. 공개 기준정보인 주식 종목 카탈로그 동기화만 읽기 전용 Cloud Storage 경계를 사용한다. | 공급자별 시세 계약은 서로 다른 Adapter로 다루고 Web은 Quote 공급자를 직접 호출하지 않는다. KRX 금현물 종목을 일반 주식 API로 보내지 않는다. 공개 카탈로그 읽기는 자산·가구 데이터 접근 권한을 부여하지 않는다. Android WebView의 간헐적인 App Check token 누락은 정상 조회를 차단하지 않는다. | [Portfolio Query 경계](../../../../../../functions/src/bootstrap/queries/portfolioMarketHouseholdQueryHandlers.ts), [기기 카탈로그](../../../../../../web/src/features/portfolio/instrument-catalog/application/localStockInstrumentCatalog.ts) | C, I |
...
| MARKET-003 | 현재 명세 | 주식 검색은 기기에 동기화된 최근 성공 Cloud Storage Catalog Snapshot의 국내·미국 종목을 합쳐 관련도순으로 모든 일치 항목을 반환한다. 화면은 30개부터 표시하고 아래쪽 스크롤 또는 더 보기로 나머지 결과를 끝까지 선택할 수 있다. 코인 검색은 인증된 Query로 Upbit KRW 마켓만 최대 10개를 반환한다. | 주식의 빈 검색어는 빈 결과이며 타이핑·추가 표시 때 Cloud Function·Quote 공급자를 호출하지 않는다. 코드·이름·별칭 관련도와 중복 제거·정렬은 서버 호환 검색과 같고, 서버 호환 검색의 최대 10개 응답 계약은 유지한다. 검색어 변경 시 표시 개수와 스크롤을 초기화한다. | [기기 카탈로그](../../../../../../web/src/features/portfolio/instrument-catalog/application/localStockInstrumentCatalog.ts), [검색 UI 설계·추적성](../../../../../operations/asset-owner-search-2026-09-21.md) | U, C, UI, I, E2E |
...
| MARKET-005 | 현재 명세 | 국내 KOSPI·KOSDAQ 및 미국 주식·ETF·ETN 종목 카탈로그를 매일 06:00 `Asia/Seoul`에 생성해 Cloud Storage에 서로 다른 최근 성공일 3일치를 보관하고, Web은 검증된 snapshot을 IndexedDB와 메모리에 캐시해 로컬 검색한다. | KONEX는 검색 목록에서 제외하되 기존 보유종목·거래 이력은 변경하지 않는다. 국내 신규 ETF는 시장 원천의 ETF 유형 또는 ETF 전용 목록으로 판별해 전용 목록의 반영 지연에도 주식으로 오분류하지 않는다. snapshot 검증 뒤 `latest`를 원자 교체하고 성공 후에만 서로 다른 성공일 기준 이전본을 3개까지 정리한다. 기기는 앱 첫 화면이 아니라 자산 페이지 첫 paint 뒤 background에서 catalog를 준비하고 최대 5분 간격으로 manifest를 확인한다. generation·SHA-256·schema·종목 수가 모두 맞는 새 snapshot만 교체한다. 실패 시 마지막 성공 기기 cache를 계속 사용하며 `stocks.json` fallback·빈 성공은 금지한다. 기기 cold cache에서 Storage를 읽지 못하면 명시적 실패다. | [DEC-035](../../../../governance/decisions.md#dec-035) | U, C, I, 운영 계약 |
...
- [DEC-049: 전체 시세 갱신과 외부 호출 한도](../../../../governance/decisions.md#dec-049) — 개별·자산 페이지 진입 1회·23:55 전체 갱신, 화면 반복 갱신 없음, 전체 종목 수 무제한, 내부 50개 page·병렬 5·10초 timeout·총 3회 시도와 single-flight를 고정합니다.
...
| T-JOB-AST-001 | 목표 | 국내·미국 주식·ETF·펀드·코인·실물 금과 전날 마지막 owner·type 자산, 100개 초과 종목, 일부 timeout, 동일 가구·범위의 실행 중 중복 요청 / 다른 화면에서 자산 페이지 진입 1회·23:55 전체 평가와 snapshot intent / 페이지 체류·visibility 복귀로 반복하지 않고, 한 run의 모든 계좌를 원자 반영하며 모든 내부 page를 처리하고 성공 Quote와 실패 대상 마지막 Quote로 평가한다. 사라진 scope는 0원이고 중복 run은 새 Provider 호출 없이 정상 생략되며 실패 건수에 포함되지 않는다. | JOB-AST-001, JOB-AST-002, AST-008, DEC-049 |
...
| T-PERF-HOLD-002 | 현재 | 활성 명의자 여러 명과 전일 Canonical 자산 요약, 현재 자산 source snapshot / 자산 메인 화면 진입·현재 자산 갱신 / 전일 요약 조회는 `limit 1` 한 번이며 전체·모든 명의자 변동은 안정 profile ID 기준으로 메모리 계산되고 현재 자산 변경은 추가 전일 조회 없이 다시 계산됨 | JOB-AST-002 |
...
| T-MARKET-005 | 현재·회귀 | 국내 주식·ETF·ETN과 미국 중복 검색 결과 10건 초과, exact code·code prefix·name prefix·name contains 동률, 12번째 RISE 미국나스닥100, KRW·비KRW 코인, 빈 검색어 / 종목 검색 / 관련도와 market·code tie-break로 모든 일치 항목을 반환하고 30개씩 스크롤·더 보기로 마지막 항목까지 선택, 검색어 변경 시 처음부터 표시한다. 서버 호환 검색·KRW 코인 최대 10개 계약은 유지하며 주식 빈 검색은 빈 결과, 서버 코인 빈 검색은 typed error | MARKET-003 |
...
| 기기 주식 검색은 국내·미국의 모든 일치 결과를 관련도순으로 합쳐 스크롤로 끝까지 선택할 수 있고, 서버 호환 검색·코인은 최대 10개를 반환한다. | U, C, UI, E2E | MARKET-003 |
