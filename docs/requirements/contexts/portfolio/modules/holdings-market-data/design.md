...
Position 수정 patch에서 wire payload에 없는 필드는 “변경하지 않음”입니다. Web Command Adapter는 Firebase Callable이 직렬화할 수 없는 `undefined` 필드를 전송 전에 제거하고, 숫자 값을 의도적으로 비울 때는 Domain이 허용하는 `0`을 명시합니다. 따라서 `undefined`를 필드 삭제 신호로 사용하지 않습니다. 코드·`instrumentType`이 없던 레거시 `bond`·`cash`·`manual` Position은 읽기 Adapter가 `holdingType`과 positionId로 안정적인 내부 code를 만들고 instrument type을 복원한 뒤 같은 `ManagePosition` 경로로 수정합니다. 이 정규화는 사용자에게 보이는 이름을 바꾸지 않습니다.
...
`RefreshAccountPrices`는 개별 자산 수동 갱신, `RefreshHouseholdPrices`는 다른 화면에서 자산 메인 페이지로 진입할 때의 전체 갱신입니다. 화면은 마지막 자산 Read Model과 같은 날짜의 가구별 일간 변동 snapshot을 먼저 렌더링합니다. Firestore listener의 캐시 응답은 즉시 표시할 수 있지만 진입 갱신의 시작 신호로 사용하지 않으며, 첫 비캐시 서버 snapshot이 화면 상태에 반영된 다음 `RefreshHouseholdPrices`를 막지 않는 background 작업으로 한 번만 시작합니다. 페이지 체류 중 30초 반복과 visibility 복귀 갱신은 하지 않습니다. 같은 화면의 중복 mount는 client single-flight로 한 요청에 합칩니다. 서버 lease가 같은 가구·범위의 선행 run을 발견하면 후속 요청은 `Success(refreshedCount=0, skippedReason=MARKET_REFRESH_IN_PROGRESS)`로 즉시 끝내고 Provider·상태 조회·write를 실행하지 않습니다. 예약 작업 Adapter는 이 결과를 `SKIPPED`로 집계하며 사용자 오류·Provider Health 실패·관리자 실패 건수에는 포함하지 않습니다. 선행 run의 Canonical write를 실시간 listener가 전달하므로 화면은 같은 결과로 수렴합니다. 이 30초 window는 반복 주기가 아니라 중복 Provider fan-out 방지 경계입니다. 전체 갱신은 target 총수 상한 없이 서로 다른 Quote target을 50개씩 page 처리하고, 한 run에서 외부 호출 최대 5개·요청당 timeout 10초·retryable 결과 총 3회 제한을 적용합니다.
...
|---|---|---|---|---|---|
| HOLD-001 | Domain Unit, Application | Position·평가 Policy·ManagePosition | Quote 없음, 평균단가 100, 현금/수동, 코드·instrumentType 없는 레거시 예수금 수정, undefined 선택 필드, quantity 0, version conflict | Quote 미존재 시 평균단가 평가; 생략 필드 유지와 레거시 내부 identity 복원; Position+Asset 전체 commit/rollback | T-HOLD-001, T-HOLD-003 |
