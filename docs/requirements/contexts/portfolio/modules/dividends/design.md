...
Scheduler Adapter는 `Asia/Seoul` cron `0 19 * * 1-5`로 평일(월~금) 19:00에 1회 실행하며 토·일에는 실행하지 않습니다. 각 평일 occurrence는 `scheduledFor`를 포함한 별도 runId를 가지며, 같은 occurrence 아래 두 phase를 각각 checkpoint로 실행합니다. 같은 occurrence를 재실행하거나 다음 평일 같은 공시를 수집해도 canonical Event ID와 상태 전이 receipt로 같은 공시·상태를 중복 반영하지 않습니다. 평일 18:30에 게시된 공시는 정상 경로에서 당일 19:00 occurrence가 수집하고, 19:00 이후 게시분은 다음 평일 19:00 occurrence가 수집합니다. 금요일 19:00 이후와 주말에 게시된 공시는 월요일 19:00 occurrence가 수집합니다. 기존 배포 함수 `dividendHourly`, 운영 jobName `dividend-hourly`, 시각이 포함된 execution key 형식은 호환성을 위해 유지합니다.
...
| JOB-DIV-001 | Contract, Application, Emulator | KIND Adapter·Refresh job·Provider Health·Projection | 최근 1년 fixture, 동일 run 2회, instrument A 성공/B timeout, 전체 실패 3회, HTTP 403·503와 요청 단계, 평일 19:00 경계, 18:30·19:30 신규 공시와 당일·다음 평일 19:00 occurrence, 금요일 19:00 이후·토·일 공시와 월요일 19:00 occurrence | 결정 Event 수렴, A만 commit, 부분 실패 degraded·무경보, 전체 실패 3회째 outage, 실제 status·단계 보존, 서울 평일(월~금) 19:00 1회 실행·토·일 미실행, 평일 18:30 공시는 당일 19:00·19:30 공시는 다음 평일 19:00·금요일 19:00 이후와 주말 공시는 월요일 19:00 수집, Projection Event 처리 후 동일 | T-JOB-DIV-001 |
