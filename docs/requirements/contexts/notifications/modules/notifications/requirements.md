...
| PUSH-003 | 목표 명세 | Web/PWA는 `register()`와 `onRegistered(fid)`를, Android는 FID 기반 등록 활성화와 `onRegistered(fid)`를 사용해 로그인한 멤버의 현재 설치 endpoint를 서버에 등록한다. 로그아웃은 해당 서버 endpoint를 삭제하고, `onUnregistered(fid)`는 조건부 inactive로 반영한다. | 별도 멤버 전환 API는 없으며 `로그아웃 삭제 → 다른 멤버 로그인 등록` 순서로 소속이 바뀐다. 삭제 누락 시 새 로그인 등록이 동일 FID의 낡은 binding을 원자 교체한다. 앱 재설치·데이터 삭제·새 기기 복원으로 새 FID가 생기면 기존 endpoint를 덮어쓰지 않고 추가한다. active endpoint에는 TTL이 없고 inactive endpoint는 30일 뒤 삭제 대상이다. `getToken`·`onNewToken`과 FID API를 함께 사용하지 않는다. | [DEC-019](../../../../governance/decisions.md#dec-019), [DEC-020](../../../../governance/decisions.md#dec-020), [DEC-027](../../../../governance/decisions.md#dec-027), 현재 legacy [FcmService](../../../../../../android/app/src/main/java/com/household/account/service/FcmService.kt) | I, UI, E2E |
...
| PUSH-007 | 현재 명세 | Android foreground notification payload는 ID expense_notifications, 이름 지출 알림인 기본 중요도 채널로 시스템 알림을 표시하고 MainActivity를 연다. | data-only 메시지는 표시하지 않는다. Android 13 런타임 권한 누락은 결함이다. | [FcmService](../../../../../../android/app/src/main/java/com/household/account/service/FcmService.kt) | UI, E2E |
...
| T-PUSH-008 | 목표·Client·Contract | 로그인 전·후, Android 표시 권한 거부, iPhone 홈 화면 PWA·일반 브라우저, desktop, `onRegistered`·`onUnregistered`, 앱 재설치 새 FID / endpoint 등록 controller / 지원 모바일과 로그인 완료 뒤에만 Firebase Installation ID callback으로 등록하고 registration token API는 surface에 없으며, 설치별 endpoint·metadata·조건부 inactive·로그아웃 삭제가 수렴 | PUSH-001, PUSH-002, PUSH-003, DEC-019, DEC-020, DEC-026 |
