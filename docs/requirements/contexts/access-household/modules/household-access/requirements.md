...
| HH-007 | 목표 명세 | 전체 가계부에서 UID 전역 Membership claim이 없는 Google 사용자가 가구 이름 부분과 자기 표시 이름을 입력해 새 가계부를 만들면, 서버는 이름 부분 뒤에 `네 가계부`를 붙여 정규화하고 Household, 자기 Member, 일반 Membership, UID 전역 claim과 초기화 Outbox를 원자적으로 생성한다. 신규 householdId는 의미 접두사 없이 안정적인 32자리 소문자 16진수로 생성한다. | 이미 `네 가계부`로 끝나는 입력에는 접미사를 중복 적용하지 않으며, 기존 호환 입력이 `가계부`로 끝나면 그대로 보존한다. 생성자는 owner role이나 추가 household capability를 갖지 않는다. 기존 claim이 있으면 가계부를 추가 생성하거나 전환하지 않고 충돌로 거부한다. 사용자가 다른 가구원을 함께 만들거나 사용자 지정 가구 키를 정하지 않는다. 후속 기본 카테고리·홈 설정 초기화 실패는 가구 생성 결과와 분리해 재시도한다. | [householdService](../../../../../../web/src/lib/householdService.ts), [DEC-021](../../../../governance/decisions.md#dec-021), [DEC-034](../../../../governance/decisions.md#dec-034), [DEC-039](../../../../governance/decisions.md#dec-039) | U, I |
