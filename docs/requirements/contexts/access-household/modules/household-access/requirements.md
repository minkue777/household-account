...
| AssetOwnerProfile | householdId, profileId, 표시 이름, member/dependent 유형, linkedMemberId?, active/archived 상태, selectionVisibility | 로그인 Member와 분리한 자산 명의자 디렉터리입니다. Member 프로필은 1:1로 연결되고 dependent에는 로그인·권한·알림 의미가 없습니다. 수집 전용 Member의 연결 프로필은 active 상태를 유지하면서 신규 선택에서 숨길 수 있습니다. |
...
| `RecordAppVisit` | 현재 Membership, 앱 생명주기 visitId, 플랫폼, 선택적 첫 화면 전체 표시 시간과 Navigation 기준 단계 시각 | 중복 없는 사용자 접속 집계와 모바일 앱 시작 성능 표본 |
| `GetAdminOperationsDashboard` | `systemAdmin`, 조회 일수 | 서버·예약 작업·공급자·가구·사용자 활동과 최근 24시간 모바일 첫 화면·Functions 로직별 처리 시간의 관리자 전용 snapshot |
...
| HH-004 | 목표 명세 | PWA 로그아웃은 Notifications의 현재 설치 endpoint가 `Removed` 또는 `AlreadyAbsent`임을 확인한 뒤 가구·멤버 상태와 로컬 저장 정보를 제거한다. Android는 DEC-020의 네이티브 수신 차단 우선 절차를 따른다. | PWA endpoint 삭제 실패 시 인증·로컬 세션을 보존하고 재시도를 허용한다. Android는 설치 수신 component를 먼저 비활성화하고 원격 삭제를 best-effort 처리하며 이전 session generation의 Queue·callback을 차단한다. | [HouseholdContext](../../../../../../web/src/contexts/HouseholdContext.tsx), [DEC-020](../../../../governance/decisions.md#dec-020) | U, I, E2E |
| HH-005 | 목표 명세 | Firebase Auth가 복원한 UID와 일치하는 마지막 검증 Membership·Household metadata cache가 있으면 `sessionGeneration`, principalUid, householdId, 자기 memberId·이름과 가구 표시 정보를 원자 복원하고 월 원장·카테고리·지역화폐 구독을 즉시 시작한다. cache가 없을 때만 `ResolveSignedInUser`를 동기 실행한다. cache hit 뒤 Household 한 문서의 최신 정보는 업무 구독과 병렬로 갱신하고 실제 차이가 있을 때만 화면·cache에 반영한다. | cache는 Firestore Rules·Functions 인가를 대체하지 않는다. 시간 경과에 따른 주기적 Membership 해석은 하지 않는다. 보호 구독의 `permission-denied`에서는 bootstrap cache와 오류 listener를 즉시 지우고 같은 UID의 권위 해석을 하나로 병합하되 마지막 in-memory 화면을 유지한다. Android token 복구와 병렬 실행하지 않으며 권위 해석의 transient transport 실패만 2~30초 backoff로 재시도한다. 같은 scope이면 cache 저장·epoch 재연결, first visit·다른 scope이면 그때 이전 scope·화면·나머지 구독을 폐기한다. Firebase UID 변경은 이전 scope를 폐기한 뒤 새 UID cache hit/miss 흐름으로 들어간다. 가계부·Member 선택 또는 partner 상태를 두지 않고 endpoint 등록 실패는 세션 복원과 분리한다. | [HouseholdContext](../../../../../../web/src/contexts/HouseholdContext.tsx), [SYS-008](../../../../system/context.md#6-공통-요구사항), [DEC-020](../../../../governance/decisions.md#dec-020), [DEC-021](../../../../governance/decisions.md#dec-021), [DEC-022](../../../../governance/decisions.md#dec-022), [DEC-034](../../../../governance/decisions.md#dec-034), [DEC-068](../../../../governance/decisions.md#dec-068) | I, E2E |
...
| HH-007 | 목표 명세 | 전체 가계부에서 UID 전역 Membership claim이 없는 Google 사용자가 가구 이름 부분과 자기 표시 이름을 입력해 새 가계부를 만들면, 서버는 이름 부분 뒤에 `네`를 붙여 정규화하고 Household, 자기 Member, 일반 Membership, UID 전역 claim과 초기화 Outbox를 원자적으로 생성한다. 신규 householdId는 의미 접두사 없이 안정적인 32자리 소문자 16진수로 생성한다. Access UoW가 commit된 뒤 온보딩 조정자는 Category 공개 Application을 호출해 기본 카탈로그를 별도 멱등 UoW로 초기화하고 상태를 `completed` 또는 `failed`로 기록한다. | 이미 `네`로 끝나는 입력에는 접미사를 중복 적용하지 않으며, 과거 호환 입력인 `이름네 가계부`는 `이름네`로 정규화한다. 그 밖에 기존 입력이 `가계부`로 끝나면 그대로 보존한다. 생성자는 owner role이나 추가 household capability를 갖지 않는다. 기존 claim이 있으면 가계부를 추가 생성하거나 전환하지 않고 충돌로 거부한다. 사용자가 다른 가구원을 함께 만들거나 사용자 지정 가구 키를 정하지 않는다. 후속 기본 카테고리 초기화 실패는 생성된 Access 상태를 롤백하거나 성공으로 위장하지 않으며, 같은 초기화 키로 재시도할 수 있다. 홈 설정 등 다른 Context 초기화도 같은 분리 원칙을 따른다. | [Access command handler](../../../../../../functions/src/bootstrap/commands/accessHouseholdCommandHandlers.ts), [Category application](../../../household-finance/modules/categories-budget/design.md), [DEC-021](../../../../governance/decisions.md#dec-021), [DEC-034](../../../../governance/decisions.md#dec-034), [DEC-039](../../../../governance/decisions.md#dec-039) | U, I |
| HH-008 | 목표 명세 | 보호 화면은 Firebase가 검증한 Google Principal과 Firestore Rules 또는 서버가 허용한 활성 Membership scope에서만 가구 데이터를 표시한다. 같은 UID의 cache hit에서는 가구 metadata를 즉시 표시할 수 있지만 월 원장·카테고리·지역화폐는 권위 서버 snapshot부터 표시한다. | Membership이 없으면 첫 방문 화면을, legacy 후보가 있으면 전환 확인을 표시한다. `permission-denied`를 과거 cache로 숨기지 않고 session 복구 경계로 전달한다. admin은 별도 capability로 접근하며 신규 guest·가구 키 우회는 허용하지 않는다. | [HouseholdGuard](../../../../../../web/src/components/HouseholdGuard.tsx), [DEC-021](../../../../governance/decisions.md#dec-021), [DEC-068](../../../../governance/decisions.md#dec-068) | UI, E2E |
...
| HH-011 | 목표 명세 | Access는 자산 명의를 위한 안정적인 `AssetOwnerProfile`을 가구 범위로 관리한다. Member마다 연결된 `member` 프로필 하나를 제공하고, 활성 가구원은 Google 계정·Membership이 없는 아이 등의 이름을 `dependent` 프로필로 추가·변경할 수 있다. dependent 프로필 삭제는 서버가 검증한 관리자만 수행한다. 결제 알림 수집 전용 Member는 연결 프로필을 유지하되 `selectionVisibility=hidden`으로 일반 자산 명의자 UI에서 제외할 수 있다. | dependent 프로필은 로그인·초대·권한·알림 대상이 아니다. 일반 자산 UI에는 삭제 버튼을 두지 않고 일반 가구원의 archive API 호출도 거부한다. 관리자 삭제도 논리 보관이므로 기존·과거 참조를 지우지 않으며 신규 선택에서만 제외한다. 숨김은 archive가 아니며 기존 ownerRef 해석과 과거 기록을 유지한다. Member 연결 프로필은 관리자도 삭제할 수 없고 Member 이름 변경과 함께 표시 이름을 갱신한다. | [DEC-037](../../../../governance/decisions.md#dec-037), [DEC-069](../../../../governance/decisions.md#dec-069) | U, I, UI, 보안 E2E |
...
| HH-JOIN-001 | 목표 명세 | 활성 가구원은 설정 화면에서 5분 동안 한 번 유효한 초대 코드를 생성할 수 있고, 전체 가계부에서 UID 전역 Membership claim이 없는 Google 사용자만 코드를 입력한 뒤 자기 표시 이름으로 자기 Member·Membership·UID 전역 claim을 원자 생성한다. 설정 화면의 `가구원 초대` 카드는 `테마` 카드 바로 다음에 배치하고 보조 문구는 `5분간 유효한 초대 코드`로 표시한다. | 코드는 특정 Member를 미리 만들지 않으며 원문을 저장하지 않는다. 만료·재사용·다른 가구·이미 claim을 가진 UID는 거부하고 Invitation을 소비하거나 어떤 Member도 추가하지 않는다. 짧은 보조 문구는 일회용 정책을 변경하지 않으며 생성 결과와 재사용 거부가 이를 보장한다. | [InvitationSettings](../../../../../../web/src/components/settings/InvitationSettings.tsx), [settings page](../../../../../../web/src/app/settings/page.tsx), [join page](../../../../../../web/src/app/join/page.tsx), [HouseholdGuard](../../../../../../web/src/components/HouseholdGuard.tsx), [DEC-021](../../../../governance/decisions.md#dec-021), [DEC-034](../../../../governance/decisions.md#dec-034) | U, I, UI, E2E |
...
| T-HH-005 | 목표 | 같은 UID의 Membership·Household metadata cache hit/miss, metadata 동일·변경·일시 실패, 보호 read `permission-denied` 뒤 같은/다른 scope, 내부 route 이동, 로그인 사용자의 탈퇴 시도, 로그아웃 후 재로그인, UID 변경, 가구 논리 삭제 후 복구 / 공개 API·세션 복원 / hit은 metadata 즉시 표시와 업무 구독 시작, miss·권한 거부만 권위 해석, 실제 metadata 차이만 갱신, 권한 해석 중 화면 유지·같은 scope epoch 재연결·다른 scope 확정 뒤 폐기, route 이동은 구독 유지, 탈퇴 진입점은 없고 Membership·Member가 불변이며 같은 memberId로 복원 | HH-004, HH-005, HH-010, DEC-036, DEC-068 |
...
| T-ADM-004 | 목표·운영 | systemAdmin과 일반 사용자, Android·iPhone 홈 화면 PWA·일반 Web, 첫 화면 데이터 성공·실패, 정상·실패·기록 없음 예약 작업, 정상·저하·중단 공급자와 열린 장애 / 운영 대시보드 조회 / systemAdmin만 모바일 첫 화면 전체 표시와 Functions 로직별 평균·P95·최대 처리 시간을 초 단위 업무명으로 받고 일반 Web·custom-token 교환·실패한 전체 화면 준비를 앱 시작 성공 표본에 섞지 않으며 실패·기록 없음은 정상으로 숨기지 않음 | ADM-005 |
| T-ADM-005 | 목표·성능·멱등 | 같은 앱 문서의 동일 visitId 재전송, Android·iPhone PWA·일반 Web, 서울 날짜 경계 / 첫 화면 전체 표시 후 접속 기록 / 첫 화면 렌더를 기다리게 하지 않고 사용자·일별·플랫폼 횟수를 한 번만 증가시키며 모바일 표본은 같은 visitId에서 한 번만 기록하고 최근 30일 상세과 누적 횟수를 보존 | ADM-006 |
