...
| T-ADM-004 | 목표·운영 | systemAdmin과 일반 사용자, Android·iPhone 홈 화면 PWA·일반 Web, 첫 화면 데이터 성공·실패, 정상·실패·기록 없음 예약 작업, 정상·저하·중단 공급자와 열린 장애 / 운영 대시보드 조회 / systemAdmin만 모바일 첫 화면 전체 표시와 Functions 로직별 평균·P95·최대 처리 시간을 초 단위 업무명으로 받고 일반 Web·custom-token 교환·실패한 전체 화면 준비를 앱 시작 성공 표본에 섞지 않으며 카드번호 불일치 등 거절을 실제 실패와 분리하고 호출 합계·재시도 최종 결과를 보존하며 실패·기록 없음은 정상으로 숨기지 않음 | ADM-005 |
