...
이 설계는 `HH-001~012`, `HH-JOIN-001`, `ADM-001~006`을 서버 권위의 Access Application과 Web·Android 세션 Adapter로 옮기기 위한 테스트 경계를 정합니다. 요구사항 문장을 다시 정의하지 않으며, 상태와 제품 의도는 [요구사항](requirements.md#5-요구사항)과 [결정 기록](../../../../governance/decisions.md)을 따릅니다.
...
`HH-007`의 기본 카테고리·홈 설정은 가구 생성 transaction에 타 Context 문서를 끼워 넣지 않습니다. Access UoW는 `HouseholdCreated.v1`을 Outbox에 기록하고 먼저 commit합니다. 현재 온보딩 Inbound 조정자는 commit 뒤 Category 공개 Application의 `InitializeDefaultCategories`를 별도 receipt를 가진 멱등 UoW로 동기 호출하여 첫 화면에서 카탈로그를 바로 사용할 수 있게 합니다. `HouseholdCreated.v1`은 다른 Context 통합과 향후 비동기 소비를 위한 사실 Event로 유지합니다. 사용자 흐름은 초기화 상태를 별도로 관찰하며, Access의 생성 성공과 후속 초기화 성공·실패를 같은 결과로 위장하지 않습니다.
...
| `RecordAppVisit` Command v1 | 인증된 Web·Android WebView·iPhone PWA | visitId, platform, clientStartupDurationMs?, clientStartupTimingsMs? | `Recorded(totalAccessCount)`, `AlreadyRecorded` | 활성 Membership | household/member별 stats와 최근 visitId를 한 transaction으로 갱신하고, 신규 모바일 visit의 유효한 전체 홈 paint 시간만 PII 없는 구조화 로그로 기록 | visitId |

`RecordAppVisit.clientStartupTimingsMs`는 선택적 숫자 map이며 `navigationResponseEnd`, `bootstrapStarted`, `authStarted`, `authReady`, `membershipStarted`, `membershipReady`, `sessionReady`, `ledgerRequested`, `ledgerReady`, `categoriesRequested`, `categoriesReady`, `localCurrencyRequested`, `localCurrencyReady`, `firstLedgerPaint`, `firstHomeCompletePaint`만 허용합니다. 최대 15개, 각 값은 Web Navigation 시작부터의 유한한 0~120,000ms이고 누락 단계는 추정하지 않습니다. 단계가 있으면 유효한 모바일 `clientStartupDurationMs`가 함께 있어야 하며 구버전의 단계 없는 payload도 받습니다. Android 전체 시간의 Activity 기준은 유지하고 단계 시각과 혼합하지 않습니다. Cloud Logging의 같은 `clientStartup` 표본에 `clientStartupTimingsMs`를 기록하며 관리자 응답의 `latestStartupSample`은 해당 operation의 최신 표본에 유효한 단계가 있을 때만 `{ timestamp, elapsedMs, timingsMs }`로 반환합니다. 식별자·경로·상관관계 원문은 추가하지 않습니다. 검증: `T-ADM-004`의 실제 Command 입력 거부·구조화 logger·최신 표본 reader 검사.
...
3. 가구 생성 선택 시 이름 부분은 서버에서 `이름 + 네`로 정규화하되 완성된 `네` 접미사는 중복하지 않습니다. 과거 호환 입력인 `이름네 가계부`도 `이름네`로 정리합니다. 서버는 householdId를 의미 접두사 없는 안정적인 32자리 소문자 16진수로, memberId를 의미 접두사가 있는 안정 ID로 생성합니다.
...
| HH-004 | Client·Integration | LogoutHouseholdSession | endpoint 제거 성공·이미 없음·일시 실패, 가구·멤버·Bridge mirror 존재 | PWA는 성공·이미 없음 뒤에만 선택값 제거하고 실패 시 세션 유지; Android는 DEC-020에 따라 수신 component 차단과 generation 폐기 뒤 원격 제거를 best-effort 처리 | T-HH-005 |
...
| ADM-005 | Admin Query·Operations Adapter·UI | GetAdminOperationsDashboard | Android Activity 시작, iPhone PWA Navigation 시작, 전체 홈 데이터 성공 paint, 일반 Web, 예약 작업 정상·실패·기록 없음, 공급자 정상·저하·중단, active/deleted 가구, 열린 장애 | systemAdmin에게만 서버·운영·가구·활동 snapshot과 허용 필드로 집계한 모바일 첫 화면·Functions 로직별 처리 시간을 반환하고, custom-token 교환은 제외하며 UI는 업무명과 내부 ID를 병기하고 모든 시간을 초 단위로 표시하며 비정상을 구분 표시 | T-ADM-004 |
| ADM-006 | Domain·Command·Firebase Adapter·Client | RecordAppVisit·MemberAccessStats·clientStartup structured log | 동일 visitId 재전송, 서울 날짜 경계, 플랫폼 3종, 30일 경계, 0~120초 경계 | 첫 화면 전체 paint 뒤 비동기 호출, 사용자 범위 원자 집계, 동일 visitId 1회, 30일 일별 상세·누적 보존, 신규 Android/iPhone visit만 PII 없는 시작 시간 1회 기록 | T-ADM-005 |
