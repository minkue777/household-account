...
Android Edge
  ...
  → AndroidRawNotification.v1 → Keystore 암호화 Queue
...
- 원문 envelope와 안정 observation ID를 암호화 Queue에 저장하고 재전송합니다.
...
| `QueueRawNotification` Android Command | Notification Listener Adapter | `AndroidRawNotification.v1` | `Queued(queueEntryId)`·`AlreadyQueued`·local failure | Android authenticated session | 암호화 local store transaction; `observationId` 유일 |
...
| `SubmitAndroidRawNotification` Server Command | Android Queue | raw envelope | `CaptureSubmissionResult` | `paymentCapture:submit`와 같은 가구 Actor | 서버 source/parser 선택 후 내부 Capture 제출; parser 무결과도 terminal |
...
2. Android `DiagnosticSink`는 별도 best-effort 경계로 원문을 보냅니다. 진단 실패는 Queue·submit 결과와 분리합니다.
3. Android가 동일 package·전체 표시 본문의 30초 cache를 claim하고 observation ID를 한 번 생성합니다.
4. 제목, `text`, `bigText`, `textLines`, 게시 시각을 계약 한도 내에서 `AndroidRawNotification.v1`으로 복사해 암호화 Queue에 원자 저장합니다. 총 65,536자를 넘으면 parser가 먼저 소비하는 제목·text lines·bigText·text 순으로 보존하며, 기기에서는 승인·취소·금액·가맹점·카드·잔액을 파싱하지 않습니다.
5. Queue 저장 실패 시 성공 후속 효과나 서버 submit을 실행하지 않습니다.
6. Functions callable이 인증·App Check와 strict raw schema를 검증하고, 서버 Source Registry의 package로 parser를 선택합니다.
7. 선택 parser가 거래·잔액 후보를 만든 뒤 Functions가 원문 hash와 내부 `CaptureEnvelope.v1`을 구성합니다. parser 무결과는 Canonical 변경 없는 `completion=terminal`이며 Queue가 제거할 수 있습니다.
...
10. Android는 terminal transaction result가 `created`이고 편집 가능한 ID·version이 있을 때만 QuickEdit을 엽니다. balance-only·거래 거부·parser 무결과에서는 열지 않습니다. `completion=terminal`이면 raw Queue entry를 삭제하고 `partial-retryable`이면 같은 원문과 observation ID를 재전송합니다. 서버 receipt가 이미 terminal인 branch를 다시 실행하지 않습니다.
...
| Port | 책임 | 실패 의미 |
|---|---|---|
| `ObservationQueuePort` | `AndroidRawNotification.v1` 암호화 저장, lease, 서버 completion·후속 효과 상태 전이 | 저장 실패는 처리 실패; 같은 ID 중복은 `AlreadyQueued`; terminal 삭제·partial retry 유지 |
...
- Queue payload는 `AndroidRawNotification.v1`이며 Queue 전체를 Android Keystore의 non-exportable AES-256-GCM 키로 암호화합니다. 백그라운드 WorkManager 실행을 위해 사용자 인증 요구 조건을 붙이지 않습니다.
...
| [ING-008](requirements.md#51-수집출처-선택중복-처리) | Android Integration, Security, Clock | 암호화 Queue·WorkManager·contract router | offline·재시작, 71:59:59·72:00:00, partial retry, parser 무결과 terminal, legacy entry, logout·키 무효화 | ciphertext만 저장, 같은 observation 재시도, QuickEdit 중복 없음, terminal·만료·session 전환 삭제, legacy 유실 없음 | `T-QUEUE-001`, `T-ING-BAL-001` |
