...
Functions Raw Inbound + Payment Intake
  ActorContext → 서버 Source Registry → 공급자 parser → 내부 CaptureEnvelope.v1
  → receipt → Configuration → fingerprint/cancellation Policy
  → Ledger/Local Currency 공개 Port → typed Capture result + quickEditSnapshot
```

목표 운영 경로에서 Functions가 Android 금융 알림 원문을 해석하는 단일 parser 정본입니다. Android에는 공급자별 정규식이나 parser ID/version을 두지 않고 OS 알림 접근, 등록 package allowlist, 개인정보 최소화를 위한 admission과 오프라인 전달만 남깁니다. Functions는 요청 처리 중 원문을 파싱한 뒤 원문 자체를 저장하지 않고 해시와 최소 provenance만 내부 `CaptureEnvelope.v1`에 전달합니다. Android는 Payment Configuration, Ledger, Local Currency 저장소를 직접 읽거나 쓰지 않습니다. 이 책임 분리가 [DEC-066](../../../../governance/decisions.md#dec-066)의 목표 SSOT입니다.
...
- 새 거래의 서버 확정 응답에 포함된 최소 `quickEditSnapshot`을 암호화 표시 FIFO로 전달해 QuickEdit 앞의 중복 Query를 제거합니다.
...
### 3.1 Android→Functions 원문 계약

```kotlin
data class RawNotificationEnvelopeV1(
    val contractVersion: String = "android-raw-notification.v1",
...
거래·잔액 branch 중 하나 이상이 있어야 합니다. Functions가 `observationId`로 결정적인 `branchId`를 만들고 retry에서 바꾸지 않습니다. 지역화폐 parser가 payment branch의 유형을 검증한 경우에만 `localCurrencyType`을 전달합니다. `rawCandidate`는 서버 parser가 추출한 가맹점 후보이지 전체 알림 원문이 아닙니다. 제목, 전체 본문, `textLines`는 내부 DTO 이후 receipt, Ledger, Domain Event, Outbox, 일반 로그에 포함하지 않습니다. `rawPayloadHash`는 Functions가 strict raw DTO의 canonical JSON으로 계산합니다.
...
1. callable Adapter가 raw schema·Firebase credential을 검증하고 활성 Membership에서 `ActorContext`를 생성합니다.
...
3. canonical payload hash로 root receipt를 claim하고 존재 branch별 stable downstream key와 `pending` stage를 한 번 기록합니다. 이미 최종 저장된 terminal branch는 결과를 재생하고 다른 payload면 즉시 `Conflict`입니다.
...
9. 존재 branch를 처리한 뒤 root receipt를 이번 실행에서 한 번만 저장합니다. 모두 terminal이면 `completed`, 하나라도 retryable이면 `partial-retryable`입니다. downstream commit 뒤 최종 receipt 저장이 실패하면 retry가 같은 branch key로 각 결과를 복구합니다.
10. Android는 terminal transaction result가 `created`이고 편집 가능한 ID·version·`quickEditSnapshot`이 있을 때만 QuickEdit을 엽니다. 새 응답은 snapshot을 암호화 표시 FIFO에 저장해 별도 Ledger Query 없이 Activity를 열고, 전환 전 ID-only entry만 Query fallback을 사용합니다. balance-only·거래 거부·parser 무결과에서는 열지 않습니다. `completion=terminal`이면 follow-up durable enqueue 뒤 capture journal entry를 삭제하고, `partial-retryable`이면 같은 원문과 observation ID를 재전송합니다.
...
| Port | 책임 | 실패 의미 |
|---|---|---|
| `ObservationQueuePort` | 원격 호출 전 `AndroidRawNotification.v1` write-ahead 저장, lease, 서버 completion·후속 효과 상태 전이 | 저장 실패는 submit 미실행; 같은 ID 중복은 `AlreadyQueued`; terminal follow-up enqueue 뒤 삭제·partial retry 유지 |
...
| `Clock`, `IdGenerator` | 30초 cache와 observation ID | parser 시간·원문 hash는 서버 소유 |
| `QuickEditResultPort` | 확정된 편집 가능 ID와 최소 표시 snapshot 전달 | 서버 commit 후에만 호출; 새 snapshot은 재조회 없이 표시, ID-only legacy는 Query fallback |
...
- 서버 follow-up은 같은 expected SessionScope에서 QuickEdit FIFO에 내구화된 뒤에만 journal을 ack합니다. 두 저장소는 한 transaction이 아니므로 이 순서를 지키고 transaction ID dedup으로 재실행 중복을 흡수합니다.
- 실패·partial entry의 unique WorkManager는 `APPEND_OR_REPLACE`로 후속 flush를 잃지 않게 직렬화하고, Queue가 비어 있으면 예약하지 않습니다.
- WorkManager가 중복 실행돼도 한 entry를 같은 observation ID로 전송합니다. 거래가 먼저 생성되고 잔액이 retryable이면 로컬 terminal branch marker가 QuickEdit 중복을 막고 서버 receipt가 거래 branch 재실행을 막습니다.
- `expiresAt = queuedAt + 72시간`이며 WorkManager는 만료 entry를 삭제합니다. 서버 `completion=terminal`이면 parser 무결과를 포함해 즉시 삭제하고 `partial-retryable`이면 만료 전까지 유지합니다.
- 전환 전 APK가 저장한 `CaptureEnvelope.v1`은 같은 암호화 Store에서 contractVersion으로 구분해 레거시 callable로 전송합니다. Queue를 초기화하거나 raw 계약으로 재해석하지 않습니다.
...
- `submitAndroidRawNotification`, `executeHouseholdCommand`, `executeHouseholdQuery`는 `minInstances=1`을 사용해 결제 수집·QuickEdit Command·구버전 Query fallback의 cold start를 줄이되 Firebase Auth와 App Check 검증을 유지합니다.
...
- parser fixture는 개인정보를 제거한 단일 파일이며 운영 Functions TypeScript parser가 직접 소비합니다. Android에는 같은 parser 구현이나 정규표현식 snapshot을 두지 않습니다.
...
| 경계 | code 예시 | 처리 |
|---|---|---|
| Android admission/Queue | `EMPTY_NOTIFICATION`, local encryption/decryption failure | 빈 입력은 Queue하지 않고 로컬 Queue 장애는 Canonical 성공으로 표현하지 않음 |
| Functions raw decoder/parser | `UNKNOWN_FIELD`, `NOTIFICATION_TOO_LARGE`, `UNSUPPORTED_SOURCE`, `PARSE_FAILED`, `INVALID_AMOUNT`, `INVALID_DATE`, `INVALID_TIME` | 계약 오류는 callable 거부, 미지원·parse 무결과는 terminal; 다른 parser fallback 없음 |
...
[보안 정책](../../../../cross-cutting/security-privacy.md#5-임시-알림-원문-정책)과 [DEC-047](../../../../governance/decisions.md#dec-047)에 따라 Diagnostic Adapter는 인증된 household·member와 등록 source를 선행 gate로 사용하고 관리자/진단 역할만 읽을 수 있게 합니다. package·source·title·text·bigText·textLines·fullText·시각과 actor scope를 기능 제거 전까지 TTL·sampling·dedupe 삭제 없이 보존합니다. 인증 token, FCM FID, 가구 접근 자격 같은 별도 Secret은 추가하지 않습니다. 전송은 DEC-068에 따라 결제 처리 뒤 5초 지연 best-effort로 실행하고, 실패는 업무 성공률 지표와 분리합니다. 제거 시 Writer·Rules·index·collection 전체를 함께 정리하며 일반 로그에는 원문을 남기지 않습니다.
...
```text
android/feature/payment-capture-delivery/                    # 목표
  application/
    admit-raw-notification.kt
    queue-raw-notification.kt
    submit-queued-notification.kt
  ports/
...
functions/src/contexts/payment-capture/android-payment-ingestion/
  domain/
    model/default-payment-source-registry.ts
    parsers/                                                 # 공급자별 parser 정본
  ...
  application/
    android-raw-notification-submission-application.ts
    commands/submit-capture-envelope-v1.ts                   # 내부 공통 Intake
    ports/in/
    ports/out/
...
모든 연도 없는 날짜를 다루는 Functions `PARSE-*` parser는 Shortcut과 동일한 `T-PARSE-003` fixture를 추가로 소비합니다. 1월의 `12/31`, 12월의 `01/01`, 같은 날짜의 미래 시·분, 윤년 `02/29`, 불가능한 월·일을 하나의 TypeScript 정책으로 검증합니다.
...
DEC-066 전환은 (1) Functions raw 계약·서버 parser·내부 Capture 변환, (2) 새 raw callable 배포, (3) Android raw DTO·암호화 Queue·contract router, (4) Kotlin parser 운영 호출과 중복 구현 제거, (5) 새 APK 배포, (6) 전환 전 Queue가 소진되고 지원 최소 버전이 올라간 뒤 레거시 callable 제거 검토, (7) DEC-002 진단 Adapter 제거 순입니다.
