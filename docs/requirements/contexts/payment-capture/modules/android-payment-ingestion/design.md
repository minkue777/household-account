...
9. root receipt가 필요한 복합·취소·Shortcut 경로는 존재 branch 처리 뒤 이번 실행에서 한 번만 저장합니다. 모두 terminal이면 `completed`, 하나라도 retryable이면 `partial-retryable`입니다. 단일 Android 승인은 이 중복 저장 없이 Ledger transaction 결과를 바로 반환합니다.
