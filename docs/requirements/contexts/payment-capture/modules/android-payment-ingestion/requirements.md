...
| PARSE-LOTTE-001 | 현재 명세 | 롯데카드 | 승인·취소, 금액, 카드 토큰, 일시불·할부 메타데이터와 가맹점 추출. 문자 앱의 가맹점 우선 행, `금액 승인`, `명의자 롯데5*5*`, 쉼표가 없는 `일시불 MM/DD HH:mm`, 후행 누적 형식도 지원하며 누적액은 결제액으로 사용하지 않는다. | [서버 카드 parser](../../../../../../functions/src/contexts/payment-capture/android-payment-ingestion/domain/parsers/cardProviderParsers.ts) | U |
