...
| 카카오톡 금융 메시지 | com.kakao.talk | `MessagingStyle`의 현재 메시지를 각각 독립 후보로 분리하고 과거 메시지는 제외한다. 엄격한 카드 승인·취소 marker 또는 도시가스 청구·금액 marker가 있는 후보만 전달하며 일반 대화는 차단한다. |
...
`com.kakao.talk`은 Source Registry에서 `sourceType=kakao-talk-financial-message`, `parserId=kakao-talk-financial-message-parser`인 활성 항목 하나에만 대응한다. 이 단일 복합 parsing 경계는 package 선택 뒤 엄격한 카드 승인·취소 parser 결과와 같은 source의 도시가스 고지서 정책을 구분하며, 같은 package에 카드용·고지서용 parser를 따로 등록하거나 다른 source parser로 fallback하지 않는다. 서버 Raw Application이 정한 `paymentKind=card|bill`은 공개 Android wire에 받지 않으며, 카드 결과는 등록 카드 확인과 일반 승인·취소 흐름을 거치고 도시가스 고지서인 `bill` 결과만 카드 확인 예외·fixed category·납부마감일 회계일 정책을 사용한다.
...
| ING-004 | 특성화 | 동일 패키지·후보 본문 hash를 30초 동안 메모리 중복으로 보고 재처리를 막는다. | 30,000ms까지 중복이고 30,001ms부터 다시 처리한다. 프로세스 재시작 시 기록이 사라진다. 카카오톡은 분리한 현재 메시지별로 claim하여 알림이 누적 갱신돼도 이미 본 메시지가 새 메시지의 처리를 막지 않는다. | 같은 근거 | U |
| ING-005 | 현재 명세 | `notification_debug_logs`는 파서 개선 기간에만 활성화하는 교체 가능한 Diagnostic Adapter이며, 인증된 household·member가 있는 등록 source 입력의 package·source·title·text·bigText·textLines·fullText·시각을 관리자 전용 ACL과 함께 best-effort로 저장한다. | 결제 저장·QuickEdit의 네트워크와 App Check token을 경쟁하지 않도록 정상 수집 시작 5초 뒤 별도 coroutine에서 전송한다. 시간 TTL·자동 개별 삭제·중복 표본 삭제 없이 기능 제거 전까지 기존·신규 문서를 전부 보존한다. 진단 실패는 업무 결과를 바꾸지 않고 원문을 Domain·Queue·receipt·Event로 전달하지 않는다. 파서 안정화 뒤 Writer·Rules·index·collection을 함께 제거한다. | [알림 수집 Service](../../../../../../android/app/src/main/java/com/household/account/service/CardNotificationListenerService.kt), [NotificationDebugLogRepository](../../../../../../android/app/src/main/java/com/household/account/data/NotificationDebugLogRepository.kt), [DEC-002](../../../../governance/decisions.md#dec-002), [DEC-047](../../../../governance/decisions.md#dec-047), [DEC-068](../../../../governance/decisions.md#dec-068) | I, 보안 E2E |
| ING-006 | 현재 명세 | Google 메시지, Samsung 메시지, Android MMS는 넓은 금융 marker admission을 통과한 원문만 서버 SMS parser 입력으로 처리한다. 카카오톡은 `MessagingStyle`의 현재 메시지를 각각 독립 후보로 분리하고 과거 메시지를 제외한 뒤 카드 승인·취소 또는 도시가스 고지서 marker admission을 통과한 후보만 `kakao-talk-financial-message-parser`에 제출한다. | Android admission과 메시지 분리는 개인정보 전송 최소화와 채팅 transport 경계만 담당하고 승인·취소·금액·가맹점을 해석하지 않는다. 일반 카카오톡 대화는 기기에서 차단한다. SMS 서버 후보는 전체, 첫 행 제거, 첫 두 행 제거 순이다. | [알림 수집 Service](../../../../../../android/app/src/main/java/com/household/account/service/CardNotificationListenerService.kt), [Android 원문 후보 분리](../../../../../../android/app/src/main/java/com/household/account/paymentcapture/RawNotificationCandidateFactory.kt), [Android admission](../../../../../../android/app/src/main/java/com/household/account/paymentcapture/RawNotificationForwardingPolicy.kt), [카카오톡 복합 parser](../../../../../../functions/src/contexts/payment-capture/android-payment-ingestion/domain/parsers/kakaoTalkFinancialMessageProviderParser.ts), [서버 SMS parser](../../../../../../functions/src/contexts/payment-capture/android-payment-ingestion/domain/parsers/smsBillProviderParser.ts) | U, I |
| ING-007 | 현재 명세 | 서버의 각 SMS 후보는 KB → NH → 네이버페이 → 토스 → 카카오페이 → 온누리 → Paybooc → 삼성 → 롯데 → 경기 → 대전 순으로 시도하고, 모두 실패하면 문자 청구 parser를 마지막에 시도한다. | 후보별 첫 성공 결과만 사용한다. 세종과 `kakao-talk-financial-message-parser`는 SMS 내부 순서에 포함되지 않는다. 카카오톡 package는 자기 복합 parser 하나만 실행한다. | [서버 SMS parser](../../../../../../functions/src/contexts/payment-capture/android-payment-ingestion/domain/parsers/smsBillProviderParser.ts) | U |
| ING-008 | 현재 명세 | Android는 `AndroidRawNotification.v1`과 observation ID를 원격 호출 직전 Android Keystore 키 기반 AES-256-GCM write-ahead journal에 먼저 저장하고 Functions에 즉시 제출한다. 정상 온라인 경로는 WorkManager를 예약하지 않으며 네트워크·일시 서버 오류 또는 `partial-retryable`일 때만 entry를 남기고 같은 idempotency key로 재시도한다. | terminal 응답은 서버의 QuickEdit follow-up을 표시 FIFO에 먼저 내구화한 뒤 journal entry를 삭제한다. 실패 entry의 최대 보존은 `queuedAt`부터 72시간이다. 로그아웃·멤버·가구 변경, 만료, 키 무효화·복호화 실패 시 삭제한다. 부분 retryable이면 entry와 terminal branch 표식을 유지하고 이미 생성된 거래의 QuickEdit 후속 효과는 반복하지 않는다. 전환 전 `CaptureEnvelope.v1` entry도 기존 callable로 전달해 유실하지 않는다. | [Android delivery](../../../../../../android/app/src/main/java/com/household/account/paymentcapture/AndroidCaptureDelivery.kt), [Android Queue](../../../../../../android/app/src/main/java/com/household/account/paymentcapture/CaptureDeliveryQueue.kt), [DEC-032](../../../../governance/decisions.md#dec-032), [DEC-066](../../../../governance/decisions.md#dec-066), [DEC-068](../../../../governance/decisions.md#dec-068) | U, I, 보안 E2E |
...
| PARSE-SAMSUNG-001 | 현재 명세 | 삼성카드 | 승인·취소, 금액, MM/DD HH:mm, 가맹점, 삼성 카드 번호를 추출한다. 취소 헤더의 양수·음수 금액은 양의 원금으로 정규화하고 음수 승인·0·상한 초과·잘못된 부호는 제외한다. 카카오톡 금융 메시지는 보낸이·카카오 수신 시각 접두사를 거래 증거에서 제외하고 카드 본문의 발생 시각을 사용하며, `누적` 행을 거래 금액으로 사용하지 않는다. | [서버 카드 parser](../../../../../../functions/src/contexts/payment-capture/android-payment-ingestion/domain/parsers/cardProviderParsers.ts) | U |
| PARSE-LOTTE-001 | 현재 명세 | 롯데카드 | 승인·취소, 금액, 카드 토큰, 일시불·할부 메타데이터와 가맹점 추출. 문자 앱의 가맹점 우선 행, `금액 승인`, `명의자 롯데5*5*`, 쉼표가 없는 `일시불 MM/DD HH:mm`, 후행 누적 형식을 지원한다. `[롯데카드]`·`명의자 07월 이용`·`교통카드 3건 3,200원 승인` 월 합계는 해당 이용 월 마지막 날과 게시 시각으로 승인 한 건을 만들며, 누적액은 결제액으로 사용하지 않는다. | [서버 카드 parser](../../../../../../functions/src/contexts/payment-capture/android-payment-ingestion/domain/parsers/cardProviderParsers.ts) | U |
...
7. 직접 호출이 실패하거나 일부 branch가 retryable이면 raw DTO와 terminal branch 표식을 journal에 남기고 `APPEND_OR_REPLACE` unique WorkManager로 최대 72시간 재시도한다. terminal direct 응답은 journal을 제거하고 Worker를 만들지 않는다.
8. Android 자동 등록은 creatorMemberId를 보존하지만 자동 푸시를 만들지 않고, 저장 성공이 확인된 경우에만 QuickEdit과 완료 event를 발생시킨다.
...
| [DEC-068](../../../../governance/decisions.md#dec-068) | 확정 | 소규모 운영의 정상 결제는 direct submit·warm instance·짧은 bounded cache·응답 snapshot으로 직렬 왕복을 줄이고, Auth·App Check·가구 격리·중복 방지·실패 Queue는 유지합니다. |
...
| T-ING-003 | 현재 명세 | 등록 KB package+KB 본문, 등록 KB package+토스 본문, 미등록 package+KB 본문, `com.kakao.talk`의 단일 Registry 항목, client parser/source 주입 / 서버 출처 판별·parse / KB fallback 없음, 미등록 parser 미실행, 카카오톡은 `kakao-talk-financial-message`와 `kakao-talk-financial-message-parser`만 선택, spoof 필드 계약 거부 | ING-002, DEC-005, DEC-066 |
| T-QUEUE-001 | 현재 명세 | 온라인 terminal·원격 호출 중 process 종료·offline·앱 재시작·72시간 경계·partial 결과·follow-up enqueue 실패·레거시 entry·로그아웃·키 무효화 / write-ahead journal·direct submit·WorkManager 처리 / 항상 원격 호출 전 암호문 journal 존재, 온라인 terminal은 FIFO enqueue 뒤 journal 제거·Worker 없음, 실패는 같은 observation 재시도, partial 유지, QuickEdit 중복 없음, 레거시 callable 호환 | ING-008, DEC-032, DEC-066, DEC-068 |
...
| T-PARSE-001 | 현재 명세 | 각 공급자 승인 골든 원문, Google·Samsung·MMS 후보, 카카오톡 일반 대화·현재 메시지 3개·과거 메시지·보낸이/수신 시각 접두사·후행 누적 / Android 분리·Functions parse·승인 분기 / 일반·과거 대화 미전송, 현재 메시지별 독립 observation, 카드 본문 시각·거래액 사용, 공개 정규화 결과와 내부 Capture 입력 일치, 승인 Port만 선택 | ING-003, ING-006, PARSE-KB-001, PARSE-NH-001, PARSE-NAVER-001, PARSE-TOSS-001, PARSE-KAKAO-001, PARSE-ONNURI-001, PARSE-PAYBOOC-001, PARSE-SAMSUNG-001, PARSE-LOTTE-001, PARSE-GYEONGGI-001, PARSE-DAEJEON-001, PARSE-SEJONG-001, PARSE-CITYGAS-001, PARSE-SMSBILL-001 |
| T-PARSE-002 | 현재 명세 | 각 지원 취소 원문, 삼성 양수·음수 취소 및 잘못된 음수 승인·0·상한 초과·부호, Google·Samsung·MMS·카카오톡 현재 메시지 후보 / Functions parse·취소 분기 / 카카오톡 복합 parser를 포함해 취소와 원 금액·카드·가맹점 추출, 취소 Port만 선택 | ING-003, ING-006, PARSE-KB-001, PARSE-NH-001, PARSE-TOSS-001, PARSE-PAYBOOC-001, PARSE-SAMSUNG-001, PARSE-LOTTE-001 |
...
| T-CITYGAS-001 | 목표 | 카카오톡 복합 parser에 제목·마감일 모두 있음, 제목 없음, 마감일 없음, 형식은 맞지만 유효하지 않은 마감일 / parse·downstream 분기 / 도시가스 고지서로 분류하고 카드 검증 없이 fixed·due-date 정책 적용, 청구 월·memo 또는 수신 월·빈 memo, 유효 마감일 또는 수신일 fallback | PARSE-CITYGAS-001, ING-SAVE-003 |
...
- [Android 원문 후보 분리](../../../../../../android/app/src/main/java/com/household/account/paymentcapture/RawNotificationCandidateFactory.kt)
...
- [카카오톡 복합 parser](../../../../../../functions/src/contexts/payment-capture/android-payment-ingestion/domain/parsers/kakaoTalkFinancialMessageProviderParser.ts)
