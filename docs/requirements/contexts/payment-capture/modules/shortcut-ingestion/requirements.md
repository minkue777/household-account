...
| IOS-003 | 목표 명세 | 선두의 문자 전송 표지 `[Web발신]`은 결제 정보가 아니므로 제거한 뒤 지원 카드사 헤더와 금액, MM/DD HH:mm, 가맹점을 추출하며 카드사 헤더를 필수로 검증한다. | 표지를 제거한 본문에 헤더가 없으면 `CARD_COMPANY_REQUIRED`, 지원하지 않는 카드사이면 `UNSUPPORTED_CARD_COMPANY`로 거부하고 거래·알림을 만들지 않는다. 삼성 fallback은 레거시 특성화에만 남긴다. | 같은 근거와 [DEC-030](../../../../governance/decisions.md#dec-030) | U |
