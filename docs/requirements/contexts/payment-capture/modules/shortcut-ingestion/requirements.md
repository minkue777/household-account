...
| `notification_debug_logs`의 Shortcut parser 거부 진단 문서 | 제한된 임시 Writer | 인증된 Actor의 parser 거부 요청만 exact raw message·normalized message·actor scope·payload/credential hash·rejection code·수집 시각을 저장하며 관리자·진단 역할만 읽습니다. Domain·업무 저장소·일반 로그가 아니며 DEC-002·047에 따라 진단 기능과 함께 제거합니다. |
...
| IOS-014 | 임시 진단 | Shortcut credential과 현재 Membership 검증에 성공해 ActorContext가 확정된 요청을 parser가 거부하면 `notification_debug_logs`에 exact raw message와 normalized message, actor scope, payload hash, 비가역 credential hash, 안정 rejection code와 수집 시각을 best-effort로 보존한다. | 같은 credential·payload는 결정적 문서 ID의 한 문서로 합친다. 호출은 await하되 실패를 흡수한다. 정상 파싱·거래 성공·중복·카드 불일치와 인증·인가 실패 또는 parser 이전 schema 실패는 저장하지 않는다. Cloud Logging, Domain Event, receipt, Outbox, 응답에는 원문을 넣지 않으며 진단 저장 실패가 HTTP 결과·거래·알림을 바꾸거나 재시도시키지 않는다. 자동 TTL 없이 보존하고 DEC-002·047의 진단 기능 제거 시 Writer·Rules·index·데이터를 함께 제거한다. | [DEC-002](../../../../governance/decisions.md#dec-002), [DEC-047](../../../../governance/decisions.md#dec-047) | U, I, 보안 E2E |
...
| [DEC-002](../../../../governance/decisions.md#dec-002), [DEC-047](../../../../governance/decisions.md#dec-047) | 확정 | parser 개선용 임시 진단 원문은 자동 TTL 없이 기능 제거 전까지 보존하고, 업무 결과와 분리한 best-effort Writer·Rules·index·데이터를 진단 기능 제거 시 함께 없앱니다. Shortcut은 인증된 parser 거부 요청에만 이 정책을 적용합니다. |
...
| T-IOS-DIAG-001 | 임시 진단 | 인증·Membership 성공 뒤 parser 거부와 동일 요청 재전송, 정상 parse, 인증 실패, schema 실패, 진단 저장소 장애 / Shortcut 호출 / 허용된 credential·payload 조합만 결정적 한 문서에 exact raw·normalized message와 actor scope·두 hash·rejection code·수집 시각을 남기고 나머지는 0건이며 진단 장애에도 원래 HTTP 결과와 downstream 호출 수가 같다 | IOS-014, DEC-002, DEC-047 |
