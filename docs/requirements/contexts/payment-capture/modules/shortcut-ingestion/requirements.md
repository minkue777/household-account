...
| IOS-003 | 목표 명세 | 선두의 문자 전송 표지 `[Web발신]`은 결제 정보가 아니므로 제거한 뒤 지원 카드사 헤더와 금액, MM/DD HH:mm, 가맹점을 추출하며 카드사 헤더를 필수로 검증한다. NH 문자는 `NH카드{마스킹번호}승인`, 선택적인 별도 명의자 행, 금액 행, 별도 일시·가맹점 행과 후행 `총누적`을 지원한다. | 표지를 제거한 본문에 헤더가 없으면 `CARD_COMPANY_REQUIRED`, 지원하지 않는 카드사이면 `UNSUPPORTED_CARD_COMPANY`로 거부하고 거래·알림을 만들지 않는다. NH 명의자 행은 파싱 위치 확인 외에는 생성자 판단에 사용하지 않고, 생성자는 credential의 현재 가구원 ID로만 결정한다. 삼성 fallback은 레거시 특성화에만 남긴다. | 같은 근거와 [DEC-030](../../../../governance/decisions.md#dec-030) | U |
