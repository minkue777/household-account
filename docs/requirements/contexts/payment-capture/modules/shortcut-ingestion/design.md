...
- 인증된 parser 성공·거부 요청을 `notification_debug_logs`에 원문·정규화 결과와 최소 진단 metadata로 best-effort 보존하는 임시 Diagnostic Adapter
...
- `([0-9,]+)원` 금액과 선택 일시불·할부·체크 표기를 읽고 양의 안전 정수 원 단위로 검증합니다. 삼성 취소 헤더의 표준 layout에 한해 금액 숫자 앞 단일 `-`를 제거해 같은 양수 검증에 전달합니다. 음수 승인은 취소나 양수 승인으로 바꾸지 않으며 0·-0·범위 초과·잘못된 부호는 거절합니다.
...
알림에 카드 번호가 없을 때 본인의 같은 카드사 카드가 한 장이면 그 카드를 canonical evidence로 확정합니다. 여러 장이면 `Eligible`이지만 특정 카드 하나를 저장 순서로 선택하지 않고 parser의 카드사 증거만 유지합니다. 번호가 있는 알림에서 본인 카드가 여러 개 일치하는 경우도 같은 원칙을 적용합니다. 레거시 owner alias는 호환 요청을 읽는 용도일 뿐 `actingMemberId`나 카드 조회 범위를 변경하지 않습니다. 전환 전에는 `LegacyShortcutOwnerPolicy` 결과와 새 결과를 shadow 비교하되 실제 목표 Writer에는 새 Policy만 사용합니다.
...
### 5.2.1 인증된 parser 성공·거부 원문 진단

이 경로는 [DEC-002](../../../../governance/decisions.md#dec-002)와 [DEC-047](../../../../governance/decisions.md#dec-047)을 따르는 임시 Infrastructure Adapter입니다. Credential Adapter와 Membership Query가 모두 성공해 `ActorContext`가 확정된 뒤 parser가 성공 또는 typed rejection을 반환하면 호출합니다. 성공 원문은 정상 업무 지연을 직렬로 더하지 않도록 Payment Intake와 병렬 저장합니다. 인증·인가 실패와 ingress·wire schema 실패에는 호출하지 않습니다.
...
| `parserOutcome` | `accepted` 또는 `rejected` |
| `parserRejectionCode` | parser가 거부한 경우에만 기록하는 안정 typed code |
| `collectedAt`, `requestedAt` | 서버 수집 시각과 요청 기준 시각 |

저장은 `ShortcutMessageDiagnosticPort`를 통한 best-effort 작업입니다. 호출은 await해 기록 시도를 완료하되 실패를 흡수하고, 성공·실패를 HTTP status나 typed 결과에 반영하지 않습니다. 실패를 거래 재시도 사유로 만들거나 Payment Intake·Ledger·Notifications를 호출해서도 안 됩니다. 같은 `credentialIdHash + rawMessageHash + parserOutcome`은 결정적 문서 ID의 한 문서로 merge합니다. Cloud Logging에는 결과 code와 비민감 correlation만 남기고 `fullText`·`normalizedText`·Actor 원문을 기록하지 않습니다. 문서는 관리자·진단 역할만 읽을 수 있고 시간 기반 TTL은 두지 않습니다. 진단 기능을 제거할 때 Shortcut Writer와 Android Writer, Rules, index, `notification_debug_logs` 데이터 전체를 같은 제거 범위로 정리합니다.
...
| `ShortcutMessageDiagnosticPort` | 인증된 parser 성공·거부의 exact raw·normalized message와 actor scope·payload/credential hash·outcome·거부 code·수집 시각을 `notification_debug_logs`에 best-effort 저장 | 성공은 Intake와 병렬 시작; 인증 실패·parser 이전 거부는 미호출; unavailable·timeout·write failure가 HTTP·업무 결과를 변경하지 않음 |
...
- `notification_debug_logs`는 Canonical Aggregate나 receipt가 아닌 임시 진단 저장소입니다. 인증된 parser 성공·거부의 `credentialIdHash + rawMessageHash + parserOutcome`마다 결정적 진단 문서 한 건을 best-effort로 merge하며 업무 transaction·멱등 receipt와 결합하지 않습니다. 자동 TTL은 없고 DEC-002·047의 기능 제거 시 Writer·Rules·index·데이터를 함께 제거합니다.
...
- 인증된 parser 성공·거부의 exact 원문과 정규화 결과는 Cloud Logging이 아니라 관리자 전용 `notification_debug_logs`에만 저장합니다. 인증 실패는 수집하지 않고 별도 Secret 원문은 기록하지 않습니다.
