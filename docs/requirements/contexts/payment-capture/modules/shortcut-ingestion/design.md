...
- Legacy Adapter만 기존 `expenses` DTO의 `source=ios-shortcut`, 빈 memo, 기본 category, 필수 creatorMemberId를 legacy `createdBy`에도 매핑하고, `cardLastFour`, 1876의 레거시 `cardType`을 기록합니다. 목표 V2 Domain은 레거시 `cardType`을 공개하지 않습니다.
