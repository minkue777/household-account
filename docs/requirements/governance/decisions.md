...
- build hash가 붙은 정적 asset은 현재 worker version의 precache로 보존한다. 같은 origin에서 인증·가구·금융 데이터를 포함하지 않는 정적 navigation shell은 `StaleWhileRevalidate`로, 공개 비민감 아이콘·폰트·이미지는 명시적 allowlist로 각각 최대 7일 runtime cache할 수 있다. navigation HTML에 사용자별 데이터를 서버 렌더링하거나 인증 cookie를 싣는 구조로 바뀌면 해당 경로는 즉시 network-only로 되돌린다. 새 worker 활성화 뒤 이전 version cache를 정리하고 임의 cross-origin 응답은 cache하지 않는다.
...
- App Check와 Command 모듈은 첫 paint의 정적 import 선행 조건에서 제외하되 보호 호출 직전에는 반드시 초기화한다. 자산 prewarm 같은 비필수 작업은 인증이 확인된 뒤 idle 시간으로 미룬다.
...
- 마이그레이션된 Android 결제 수집의 Membership은 UID 전역 `PrincipalMembershipClaim`과 해당 Household lifecycle 두 문서를 직접 읽어 해석한다. claim이 없는 전환 데이터만 기존 Membership view·canonical 문서 검증 경로로 fallback한다. 활성 claim과 Household 확인 결과만 최대 5분 cache하며, 삭제된 Household은 빠른 경로에서도 거부한다.
...
- Capture root receipt는 claim 뒤 각 Ledger·Local Currency branch의 고정 downstream idempotency key를 사용하고, 한 실행의 branch 결과를 마지막에 한 번 저장한다. 중간에 중단되면 downstream 멱등 결과를 재생하여 root receipt를 완성하며 두 번째 거래를 만들지 않는다.
