...
- Web은 Firebase Auth persistence와 같은 UID의 마지막 검증 Membership 연결 정보만 재사용해 불필요한 Membership callable을 줄인다. 가계부 첫 화면의 가구 read model·현재 월 원장·카테고리·지역화폐 잔액은 localStorage나 Firestore cache snapshot을 선표시하지 않고 서버에서 받은 첫 snapshot 뒤 표시한다. 전환 전 저장된 해당 화면용 localStorage snapshot은 앱 시작 뒤 제거한다. 마지막 Membership 권위 검증 후 30분이 지나면 첫 원장 paint 뒤 idle 시점에 Membership Command를 다시 실행한다. 자산 route의 재진입 표시 snapshot과 종목 검색 카탈로그 cache는 첫 가계부 화면 정책과 별개로 유지하되 Command 권한이나 서버 ActorContext의 근거로 사용하지 않는다.
