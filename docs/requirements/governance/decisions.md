...
합치기 해제 시 원본별로 가맹점·금액·카테고리·메모를 복원한다. 2026-09-18 추가한 지출 태그도 [DEC-075](#dec-075)에 따라 원본별로 복원한다. 날짜·시간·거래 유형·카드 유형·카드 끝자리는 합쳐진 거래의 공통 값을 모든 복원 거래에 적용한다.
...
- 새 가계부를 만드는 사용자는 가구 이름 부분과 자기 표시 이름을 입력한다. 서버는 가구 이름을 `입력 이름 + 네`로 정규화하고 의미 접두사 없는 안정적인 householdId를 생성한 뒤 Household, 자기 Member, 일반 Membership을 한 transaction에서 생성한다. 완성된 `네` 접미사는 중복하지 않으며 생성자는 다른 활성 가구원보다 강한 household 권한을 갖지 않는다.
...
<a id="dec-032"></a>
## DEC-032 Android 결제 write-ahead journal과 실패 Queue는 암호화하여 최대 72시간 보존
...
- Android는 `AndroidRawNotification.v1`을 Functions에 제출하기 직전 같은 observation ID와 raw envelope를 내구성 있는 암호화 write-ahead journal에 먼저 기록한다. 정상 온라인 경로는 곧바로 Functions를 호출하고 WorkManager를 예약하지 않으며, 네트워크·일시 서버 오류 또는 `partial-retryable`일 때만 journal entry를 남겨 재시도 Worker를 예약한다. 전환 전 APK의 `CaptureEnvelope.v1` entry도 기존 계약으로 계속 처리한다.
...
- 정상 direct 응답이 terminal이면 QuickEdit follow-up을 표시 FIFO에 먼저 내구화한 뒤 해당 journal entry를 지우며 WorkManager는 만들지 않는다. Queue 재시도에서 `confirmed`, `duplicate`, 영구 `rejected`, `needs-review`처럼 서버가 terminal 결과를 확정해도 같은 순서로 follow-up enqueue를 먼저 완료한 뒤 entry를 삭제하고, 부분 retryable이면 이미 끝난 branch 표식을 함께 보존해 QuickEdit을 반복하지 않는다.
...
<a id="dec-035"></a>
## DEC-035 종목 검색 카탈로그는 Cloud Storage 최근 성공 3일치와 기기 캐시를 사용
...
- 저장소에 번들된 `stocks.json`은 목표 검색 경로와 fallback에서 제거한다. 검증된 Cloud Storage snapshot만 종목 검색 원본으로 인정한다.
- Web은 공개 읽기 전용 `latest` manifest와 압축 snapshot을 기기로 동기화하고, 검증을 통과한 snapshot을 IndexedDB에 영속 저장한 뒤 역직렬화·검색 인덱스를 프로세스 메모리에 둔다. 종목명·코드 타이핑 검색은 Household Query나 Cloud Function을 호출하지 않고 이 기기 read model에서 수행한다.
- 자산 페이지 진입 시 캐시를 미리 준비한다. 메모리가 비어 있으면 IndexedDB를 먼저 읽고, 검증된 기기 snapshot이 있으면 원격 확인을 기다리지 않고 검색한다. 원격 manifest는 최대 5분 간격으로 확인하며 generation·checksum이 바뀐 경우에만 새 snapshot을 내려받는다.
- IndexedDB와 메모리는 정합성 원본이 아니다. 새 snapshot은 object generation·SHA-256·schema·종목 수 검증이 모두 끝난 뒤 메모리 reference와 IndexedDB를 교체한다.
- 원격 갱신이 실패해도 검증된 기기 snapshot이 있으면 마지막 성공본을 계속 검색한다. 기기에 snapshot이 전혀 없는 최초 동기화가 실패한 경우에만 명시적 실패를 반환하며, 빈 성공이나 `stocks.json`으로 대체하지 않는다.
- 종목 카탈로그는 공개 시장 기준정보이므로 Storage 읽기에 가구 Membership을 요구하지 않는다. Storage Rules는 `market-catalog/v1/**` 읽기만 공개하고 모든 쓰기는 서버 publisher 외에 거부하며, 브라우저 CORS는 읽기 메서드만 허용한다. 시세 조회·자산 저장·배당 조회는 계속 인증된 서버 경계를 사용한다.

의도:

배포 파일을 매일 수정하지 않고 국내·미국 전체 종목 목록을 자동 갱신하면서, 서버리스 cold start와 인증 왕복을 타이핑 검색 경로에서 제거한다. 캐시는 성능 최적화로만 두고 Cloud Storage snapshot을 단일 원본으로 유지한다.
...
- 사용자가 다른 화면에서 자산 메인 페이지에 진입하면 현재 가구의 모든 active 시세 연동 자산을 한 번 갱신한다. 자산 페이지에 머무르는 동안의 30초 반복과 문서 visibility 복귀 갱신은 하지 않는다. 화면은 마지막 자산 Read Model과 같은 날짜의 마지막 일간 변동을 먼저 표시하고, 진입 갱신 결과가 도착하면 새 값으로 수렴한다. 사용자에게 전체 종목 수 상한이나 page 조작을 노출하지 않는다.
...
- 인증 응답, 가구·거래·자산·통계 등 금융 응답, `/api/**`와 session 정보를 포함한 응답은 Cache Storage에 저장하지 않고 network-only로 처리한다. Firestore 응답이나 offline command queue도 PWA runtime cache로 만들지 않는다.
- build hash가 붙은 정적 asset과 공개 비민감 아이콘·폰트·이미지는 현재 worker version의 precache 또는 명시적 allowlist로 최대 7일 보존할 수 있다. navigation HTML은 이전 client와 새 서버 계약이 섞이지 않도록 항상 network-only로 처리한다. 새 worker 활성화 뒤 이전 version cache를 정리하고 임의 cross-origin 응답은 cache하지 않는다.
...
사용자가 가계부를 입력하는 도중 배포 때문에 화면이 갑자기 reload되거나 구 UI와 새 worker가 섞이는 문제를 막는다. 동시에 푸시 기능은 유지하고, 로컬 cache에는 다시 내려받을 수 있는 사용자 데이터 없는 화면 shell과 공개 자원만 제한적으로 남겨 첫 실행을 빠르게 하면서도 가구·금융 데이터가 다른 세션이나 오래된 버전에 노출되지 않게 한다.
...
- 대기열은 Android Keystore 기반 암호화 저장소에 `(sessionGeneration, householdId, memberId, transactionId, sequence, enqueuedAt)`과 선택적인 `quickEditSnapshot`을 보존한다. 새 서버 응답의 snapshot은 거래 ID·가맹점·금액·회계일·시각·카테고리·memo·aggregateVersion만 가지며 표시를 위한 두 번째 서버 Query를 제거한다. 암호문과 key는 cloud backup과 기기 이전에서 제외한다.
...
- 표시 직전 현재 인증 session과 대기열 scope가 일치하는지 검증한다. 같은 저장 응답에 포함된 `quickEditSnapshot`이 있으면 이를 즉시 표시하고, snapshot이 없는 구버전 Queue entry만 Ledger Query로 최신 거래와 편집 가능 상태를 확인한다. snapshot의 aggregateVersion 이후 다른 변경이 먼저 commit되면 실제 Update·Delete·Split Command가 서버 version 검증에서 Conflict로 거부한다.
...
연속 결제가 현재 편집 화면을 덮어쓰거나 Activity stack에 쌓여 순서가 뒤집히는 문제를 막는다. 프로세스가 종료되어도 모든 저장 완료 거래를 도착 순서대로 확인할 수 있게 하며, 서버가 방금 확정한 최소 표시 snapshot을 암호화 저장해 QuickEdit 앞의 중복 네트워크 왕복을 없앤다. 동시 변경 안전성은 snapshot 재조회 대신 aggregateVersion을 포함한 서버 Command 검증으로 유지한다.
...
- 기본 구성은 왼쪽 `X월 지출`, 오른쪽 `X월 잔여 예산`이다. 설정을 저장하지 않은 가구와 유효하지 않은 레거시 값의 안전한 표시 fallback에 이 기본값을 사용한다.
...
<a id="dec-062"></a>
## DEC-062 배당 공시 수집·상태 전이는 평일(월~금) 19시에 1회 실행

> 상태: Accepted  
> 결정일: 2026-07-19  
> 변경일: 2026-09-16 — 사용자 요청으로 매일 09:00~20:00 매시 실행에서 평일(월~금) 19:00 하루 1회로 변경하고 주말 제외 요청을 반영  
> 정책 소유 Context: [Portfolio](../contexts/portfolio/requirements.md)  
...
- 배당 예약 작업은 `Asia/Seoul` cron `0 19 * * 1-5`로 평일(월~금) 19:00에 1회 실행한다. 평일마다 1개 occurrence가 생성되며 토·일에는 실행하지 않는다.
- 각 평일 occurrence는 `scheduledFor`가 포함된 별도 execution key와 runId를 사용한다. 같은 occurrence 안에서 공시 `DISCOVERY`와 기존 nonterminal Event `LIFECYCLE_SWEEP`을 각각 독립 checkpoint로 실행한다.
- 평일 18:30에 게시된 공시는 당일 19:00 occurrence에서 정상 수집한다. 19:00 이후 게시분은 다음 평일 19:00 occurrence에서 수집한다. 금요일 19:00 이후와 주말에 게시된 공시는 월요일 19:00 occurrence에서 수집한다.
- 같은 occurrence를 재실행하거나 다음 평일 같은 공시와 상태를 확인해도 `source + sourceDisclosureId + instrumentCode`의 canonical Event ID, 상태 전이 receipt와 expected version으로 같은 Event·상태·Projection을 중복 반영하지 않는다.
...
- 기존 배포 함수 `dividendHourly`, 운영 jobName `dividend-hourly`, 시각이 포함된 execution key 형식은 기존 리소스와 실행 이력의 호환성을 위해 유지한다. 실제 실행 주기는 해당 식별자 이름과 관계없이 평일(월~금) 19:00 하루 1회이다.

의도:

사용자가 요청한 평일(월~금) 저녁 19시 하루 1회 갱신과 주말 제외로 공시 수집과 상태 전이 시각을 고정하고, 반복 실행과 외부 KIND 요청 빈도를 줄인다. 19시 이후 공시는 다음 평일 19시 실행에서, 금요일 19시 이후와 주말 공시는 월요일 19시 실행에서 반영하며 기존 멱등 처리·실패 격리·Provider Health 관측은 유지한다.
...
- 표시 FIFO에는 scope·transaction ID·sequence와 DEC-054의 선택적 최소 표시 snapshot을 둔다. command payload outbox는 별도 암호화 저장소와 lifecycle을 사용하며, command outbox commit과 WorkManager 영속 예약이 모두 완료된 뒤 표시 head를 완료하여 후속 QuickEdit을 열 수 있다. 이 결정의 outbox는 사용자가 QuickEdit에서 만든 Ledger Command 전달용이며, DEC-032의 결제 원문 direct-first·실패 Queue 정책과 합치지 않는다.
...
<a id="dec-068"></a>
## DEC-068 소규모 운영의 대화형 경로는 저지연을 우선

> 상태: Accepted  
> 결정일: 2026-07-23  
> 정책 소유 영역: 공통 시스템·Payment Capture·Android Host  
> 영향 기능: Web 첫 화면, Android 결제 자동 등록·QuickEdit, Functions 결제 수집

결정:

- 현재 두 가구와 향후 소수 가구가 사용하는 운영 규모를 기준으로 대화형 경로를 설계한다. 수백·수천 동시 사용자를 위한 분산 cache, 복잡한 invalidation·fan-out·다단계 비동기 Queue를 첫 화면과 결제 저장의 선행 조건으로 두지 않는다. 외부 Provider 제한과 Scheduler처럼 별도 이유가 있는 batch·page 정책은 유지한다.
- Firebase Auth와 서버의 활성 Membership 기반 가구 격리, Ledger fingerprint·idempotency와 Android 실패 Queue의 72시간 보존은 성능을 이유로 제거하지 않는다. App Check는 Native 결제 수집·세션 교환처럼 플랫폼 증명이 안정적인 진입점에는 유지하되, Android WebView에서 간헐적으로 token이 누락되는 공용 Command·Query에는 강제하지 않고 Auth·Membership을 권위 경계로 사용한다.
...
- 2026-09-16 사용자 요청에 따라 구현·필요한 실제 배포 완료와 CI 검증 완료를 분리합니다. 개발·배포 완료 뒤 실행 중인 E2E/CI만 기다리며 대화를 유지하지 않고 정확한 SHA·run URL을 후속 확인에 넘깁니다. 후속 실행에서 실패 로그를 분석·수정·재push하고 다섯 검사와 workflow가 실제 성공한 뒤 CI 완료를 보고합니다. 실패·skip을 성공으로 바꾸지 않습니다.
...
사용자 요구는 `2026부산여행` 같은 여행·행사 목적의 지출을 태그로 묶어 얼마를 썼는지 확인하는 것입니다. 최초 제공 범위는 지출 탭이며 기존 검색을 재사용합니다.

이번 구현에서 선택한 설계는 다음과 같습니다. 입력 한도·색상·추천 표시 개수는 사용자가 별도로 지정한 값이 아니라 이 구현의 기본값입니다.

- 태그는 거래의 선택 `tags` 필드로 저장하며 메모·카테고리와 독립입니다. 메모 아래에서 입력하고 배경·테두리·안쪽 여백 없는 파란색 `#태그` 텍스트로 표시합니다. 2026-09-19 사용자 요청에 따라 목록·검색 결과에서는 메모 뒤에 같은 줄로 이어 붙이고, 공간이 부족한 태그만 다음 줄로 넘깁니다. 이미 읽은 원장의 태그를 재사용하며, 별도 태그 관리 화면이나 전역 태그 마스터를 만들지 않습니다.
- 지출 목록의 태그를 누르면 기존 검색창에 `#태그`를 전달합니다. 일반 검색은 태그 부분 일치를 포함하고, `#`로 시작하면 태그 이름만 대상으로 부분 일치 검색합니다. 2026-09-18 사용자 요청에 따라 `#2026`으로 `2026부산여행`을 찾을 수 있도록 최초의 전체 일치 규칙을 변경했습니다. 건수·합계는 같은 검색 결과에서 계산합니다.
- 새 태그 입력은 정규화 후 거래당 10개, 태그당 Unicode 코드 포인트 30자로 제한합니다. 이미 저장된 값을 읽거나 태그를 생략한 수정에는 새 입력 한도로 인한 자동 삭제·잘라내기를 적용하지 않습니다.
- 분할은 원본 태그를 상속하며 원복은 보존된 원본 태그를 복원합니다. 합치기는 태그 합집합을 사용하고 한도 초과는 전체 무변경으로 거절합니다. 해제 시 태그는 원본별로 복원하고 날짜·시간·카드 공통 값은 [DEC-010](#dec-010)을 따릅니다.
- 2026-09-23 사용자 요청으로 Android 네이티브 QuickEdit에도 메모 아래 태그 입력을 제공합니다. 저장된 태그를 표시하고 추가·수정·제거할 수 있으며 일반 저장과 분할의 현재 초안에 같은 태그를 전달합니다. 정규화·입력 한도·빈 배열 제거·생략 보존은 기존 Ledger 계약을 재사용하고 표시 FIFO·암호화 command outbox에서도 필드의 존재 여부와 값을 보존합니다. 기존 추천을 위한 별도 원장 조회는 추가하지 않습니다.
- 기존 메모의 자동 태그 변환, 태그 일괄 수정·이름 변경, 태그 전용 통계, 수입 태그 입력은 제공 범위에 포함하지 않습니다. 기존 클라이언트의 태그 없는 수정 요청은 저장된 태그를 보존합니다.

정규화·요청/응답·오류·동시성 계약과 검증 경계는 [원장 상세 설계](../contexts/household-finance/modules/ledger/design.md)가 소유합니다.

영향 요구사항: LED-011, LED-012, SEA-006, QE-002, QE-010, QE-013. Canonical 테스트: T-LED-011, T-LED-012, T-SEA-004, T-QE-004, T-QE-008.
