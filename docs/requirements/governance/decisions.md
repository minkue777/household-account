...
- 기존 메모의 자동 태그 변환, 태그 일괄 수정·이름 변경, 태그 전용 통계, 수입 태그 입력, Android 네이티브 QuickEdit 태그 입력은 이번 제공 범위에 포함하지 않습니다. 기존 클라이언트의 태그 없는 수정 요청은 저장된 태그를 보존합니다.

정규화·요청/응답·오류·동시성 계약과 검증 경계는 [원장 상세 설계](../contexts/household-finance/modules/ledger/design.md)가 소유합니다.

영향 요구사항: LED-011, LED-012, SEA-006. Canonical 테스트: T-LED-011, T-LED-012, T-SEA-004.
