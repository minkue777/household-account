...
- 태그는 거래의 선택 `tags` 필드로 저장하며 메모·카테고리와 독립입니다. 메모 아래에서 입력하고 배경·테두리·안쪽 여백 없는 파란색 `#태그` 텍스트로 표시하며 메모와 시작선을 맞춥니다. 이미 읽은 원장의 태그를 재사용하며, 별도 태그 관리 화면이나 전역 태그 마스터를 만들지 않습니다.
