...
- App Check와 Command 모듈은 첫 paint의 정적 import 선행 조건에서 제외하되 보호 호출 직전에는 반드시 초기화한다. 첫 화면의 공용 import는 Firebase·희귀 관리 기능을 끌어오지 않게 직접 경계로 구성한다. 원장·자산의 자주 쓰는 모달과 명령 코드는 첫 paint 직후 준비하되, 시세·배당·일간 변화 같은 비필수 네트워크 작업은 첫 상호작용 뒤로 미룬다. 사용자 동작에 기능상 필요 없는 고정 debounce·표시 애니메이션 대기를 두지 않는다.
