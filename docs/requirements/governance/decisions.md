...
<a id="dec-049"></a>
## DEC-049 시세 연동 자산은 페이지 활성 중·23:55에 전체 갱신하고 외부 호출은 내부 분할
...
- 사용자가 자산 메인 페이지에 진입하면 현재 가구의 모든 active 시세 연동 자산을 즉시 한 번 갱신하고, 페이지가 foreground에서 보이는 동안 30초마다 같은 전체 갱신을 반복한다. 페이지를 벗어나거나 문서가 hidden 상태가 되면 반복을 중단하고, visible로 돌아왔을 때 마지막 갱신 시작 후 30초가 지났다면 즉시 다시 갱신한다. 사용자에게 전체 종목 수 상한이나 page 조작을 노출하지 않는다.
...
- 동일 가구·갱신 범위의 실행은 single-flight로 합친다. 이미 실행 중이면 새 Provider fan-out을 만들지 않고 같은 run 결과를 사용한다. 수동·페이지 활성 중 갱신은 같은 actor·가구·범위에서 30초에 한 번만 새 외부 호출을 시작하며 그 안의 요청은 현재 실행 또는 직전 결과를 재사용한다.
...
- Firebase Auth, App Check, 서버의 활성 Membership 기반 가구 격리, Ledger fingerprint·idempotency와 Android 실패 Queue의 72시간 보존은 성능을 이유로 제거하지 않는다.
- Web은 같은 Firebase UID에서 마지막으로 서버 검증을 마친 Membership·가구 read model과 현재 월 원장·가구별 카테고리·자산·자산 명의자 snapshot을 동기식 표시 hint로 먼저 그린다. Firebase Auth persistence는 매 시작 시 UID를 확인하고, 마지막 Membership 권위 검증 후 30분 안에는 첫 화면의 중복 Membership callable을 생략한다. 30분이 지나면 첫 원장 paint 뒤 idle 시점에 Membership Command를 다시 실행하며 Firestore listener도 배경에서 권위 상태로 수렴한다. first visit·UID 불일치·가구 부재·권한·계약 오류가 확정되면 hint와 SessionScope를 폐기한다. 이 local snapshot은 Command 권한이나 서버 ActorContext의 근거가 아니다.
- App Check와 Command 모듈은 첫 paint의 정적 import 선행 조건에서 제외하되 보호 호출 직전에는 반드시 초기화한다. 첫 화면의 공용 import는 Firebase·희귀 관리 기능을 끌어오지 않게 직접 경계로 구성한다. 원장·자산의 자주 쓰는 모달은 각 route에 정적으로 포함해 첫 클릭에 동적 chunk나 준비 fallback을 기다리지 않고, 저장 명령 코드는 첫 paint 직후 준비한다. 시세·배당·일간 변화 같은 비필수 네트워크 작업은 첫 상호작용 뒤로 미룬다. 사용자 동작에 기능상 필요 없는 고정 debounce·표시 애니메이션 대기를 두지 않는다.
...
- `submitAndroidRawNotification`, `executeHouseholdCommand`, `executeHouseholdQuery`는 현재 소규모·비용 우선 운영에서 `minInstances=0`을 사용한다. process가 warm인 동안에는 성공한 활성 Membership을 최대 5분·64개, 성공한 결제 설정 snapshot을 최대 1분·32개만 process-local cache에 보존한다. 거부·실패 결과는 cache하지 않고, cache hit는 보장하지 않으며 UID를 다른 가구로 재해석하거나 Auth·App Check 검증을 대체하지 않는다. 단계별 latency 계측과 hot-path 단순화 뒤 cold start가 목표 지연을 지배한다고 확인될 때에만 특정 Function의 warm instance를 재검토한다.
...
- 활성 Membership cache 때문에 멤버 제거·가구 연결 변경 직후 기존 warm capture instance의 반영은 최대 5분 늦을 수 있다. Firebase Auth와 App Check는 매 요청 검증하고 cached UID를 다른 가구로 재해석하지 않으며, 이 제한된 권한 회수 지연은 현재 소규모·속도 우선 운영에서 수용한다.
