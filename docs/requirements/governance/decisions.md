...
- 자산 도넛 그래프 위 필터는 `전체 / 명의자들 / +` 순서로 표시한다. `+`는 이름 하나를 입력하는 명의자 추가 창을 열며 생성된 프로필은 자산 생성·수정의 명의자 선택지와 도넛 필터에 나타난다.
...
- 현재 항목의 저장·삭제·분할이 서버에서 성공하거나 사용자가 명시적으로 닫으면 해당 항목을 완료 처리한 뒤 다음 항목을 연다. 쓰기 실패·충돌·네트워크 실패에서는 현재 항목을 유지하고 다음 항목으로 진행하지 않는다.
...
- process가 종료되거나 Activity가 재생성되어도 가장 오래된 미완료 항목부터 복구한다. 로그아웃·가구 또는 멤버 전환 때는 이전 session의 QuickEdit 대기열 삭제가 성공한 뒤에만 새 SessionMirror로 전환하여 다른 actor의 거래를 표시하지 않는다.
...
- 충돌한 QuickEdit은 사용자의 로컬 form과 분할 초안을 즉시 버리지 않고 “다른 곳에서 거래가 변경됨” 상태를 표시한다. 최신 거래를 다시 불러오기 전에는 자동 병합·자동 재시도·마지막 저장 우선 덮어쓰기를 하지 않는다. 대상이 여전히 active인 단순 수정이면 사용자가 최신 값을 확인한 뒤 자신의 변경을 다시 적용해 새 expectedVersion으로 제출할 수 있다. 대상이 이미 분할·병합·삭제되어 `superseded`·`deleted`라면 같은 원본으로 재제출하지 못하게 하고 최신 파생 거래나 목록으로 이동한다.
