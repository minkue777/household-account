...
- Web은 같은 Firebase UID에서 마지막으로 서버 검증을 마친 Membership·가구 read model과 현재 월 원장·가구별 카테고리 snapshot을 동기식 표시 hint로 먼저 그린다. Firebase Auth persistence 복원, App Check 초기화, Membership Command와 Firestore listener는 배경에서 권위 상태로 수렴하며, first visit·UID 불일치·가구 부재·권한·계약 오류가 확정되면 hint와 SessionScope를 폐기한다. 이 local snapshot은 Command 권한이나 서버 ActorContext의 근거가 아니다.
...
- `submitAndroidRawNotification`, `executeHouseholdCommand`, `executeHouseholdQuery`는 `minInstances=1`로 cold start를 줄인다. warm capture instance는 성공한 활성 Membership을 최대 5분·64개, 성공한 결제 설정 snapshot을 최대 1분·32개만 process-local cache에 보존한다. 거부·실패 결과는 cache하지 않고, cache는 UID를 다른 가구로 재해석하거나 Auth·App Check 검증을 대체하지 않는다.
