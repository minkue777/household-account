...
| `notification_debug_logs` | Android, Functions Shortcut ingress | 관리자·진단 운영 도구 | [Payment Capture](../contexts/payment-capture/requirements.md) | Android·Shortcut Diagnostic Adapter | Domain 밖 임시 민감 데이터; Shortcut은 인증된 parser 거부만 exact raw·normalized message, actor scope, payload/credential hash, rejection code, 수집 시각으로 best-effort 추가하고 정상·인증 실패는 저장하지 않음; 일반 로그·Event·receipt와 분리하며 DEC-047에 따라 기능 제거 전 TTL 없이 전부 보존하고 V2 이관 없이 DEC-002로 Writer·Rules·index·컬렉션 공동 제거 |
