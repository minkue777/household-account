...
| localStorage 마지막 검증 Membership 연결 정보 | Web Auth Adapter | 반복 인증 왕복 단축 | 공통 시스템·Access | [Membership cache](../../../web/src/features/access-household/application/signedInMembershipCache.ts) | UID·householdId·memberId·검증 시각만 보존하고 가구명·가구원·원장·카테고리·지역화폐 화면 데이터는 저장하지 않음; Auth·Rules·Membership을 대체하지 않음 |
