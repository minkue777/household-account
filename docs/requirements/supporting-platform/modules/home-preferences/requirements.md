...
설정의 카드 등록·카테고리·가맹점 규칙·정기 지출·테마 섹션은 처음 열거나 다른 페이지에서 돌아오면 접힌 상태로 표시합니다. 브라우저 뒤로 가기로 기존 문서가 복원되어도 펼침 상태를 초기화합니다. 같은 설정 화면에서 테마를 바꾸거나 앱 전환·권한창에서 복귀하는 것만으로 접지는 않으며, 저장된 설정값은 유지합니다. 검증은 `settingsSectionNavigation.test.tsx`와 실제 Chromium·WebKit의 `settings-navigation.spec.ts`가 담당합니다.
