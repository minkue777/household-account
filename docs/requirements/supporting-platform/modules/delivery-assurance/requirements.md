...
|---|---|---|---|---|
| REL-001 | 확정 명세 | 배포는 해당 surface의 production build와 배포 대상·산출물·권한·호환성·smoke 검증을 수행하며 전체 CI 완료를 기다리지 않는다. main push와 PR의 독립 CI는 Functions·Web·Web E2E·Android·해당되는 instrumentation 검증을 실행하고 실제 commit별 결과를 남긴다. 실패·취소·skip·누락은 성공으로 바꾸지 않으며 이후 문제를 알리고 조사한다. 배포 완료와 CI 성공은 별도 상태다. | wrapper는 전체 테스트 실행·CI 대기·보고서 다운로드를 하지 않고 CI 상태를 `not-evaluated`로 기록한다. `quality-summary`는 다섯 job 결과와 SHA·실행 링크를 summary에 기록하고 비성공이면 실패 check를 남긴다. GitHub 기본 알림은 사용자 Actions 알림 설정을 따른다. production build lifecycle 검사는 유지한다. | T-REL-001, [DEC-074](../../../governance/decisions.md#dec-074) |
