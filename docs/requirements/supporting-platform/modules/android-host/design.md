...
| QE-009 | U, UI, E2E | QuickEditPresentationPolicy·PendingQueue·ActivityCoordinator | A 표시 중 B·C 도착, 같은 ID 중복, 같은 시각 sequence, process 재시작, A outbox 접수·접수 실패·닫기, stale·권한 철회·편집 불가·다른 session | A 유지, 접수 성공 뒤 중복 없는 A→B→C, outbox 쓰기 실패 시 A 유지, 무효 head만 민감 화면 없이 skip, 이전 session 미표시 | T-QE-003 |
