...
6. Web application은 기본 Firestore realtime 전송을 유지한다. Android는 single-tab, 일반 브라우저와 PWA는 multiple-tab IndexedDB `persistentLocalCache`를 활성화한다. 현재 월 원장 listener의 마지막 성공 snapshot은 household·월·거래 유형 key로, 카테고리 listener의 마지막 성공 snapshot은 household key로 localStorage 표시 hint에 저장하여 다음 첫 화면에서 Firestore 초기화보다 먼저 방출한다. authoritative household 부재·권한·계약·불변식 오류는 cache fallback 대상이 아니며 SessionScope와 화면 hint를 폐기한다.
...
8. 영속 Web Auth가 같은 UID로 복원되면 마지막 검증 Membership의 원격 구독을 즉시 재개하고 Firebase SDK token refresh와 서버 Rules로 수렴한다. 강제 refresh는 원격 구독의 선행 gate로 사용하지 않으며 실패·5초 무응답일 때 Native→Web custom-token 교환으로 복구한다. Web Auth가 없는 최초 설치·명시적 로그아웃 뒤 로그인에서도 Native→Web 교환을 요청한다. App Check와 Command module은 첫 paint의 정적 import 선행 조건에서 제외하되 보호 callable 직전에 초기화한다. Native SessionMirror 갱신도 첫 화면과 경쟁하지 않도록 인증 완료 뒤 idle task로 실행하고 같은 UID·가구·멤버에서는 최대 하루 한 번만 수행한다.
