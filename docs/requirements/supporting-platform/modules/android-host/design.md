...
8. 영속 Web Auth가 같은 UID로 복원되면 마지막 검증 Membership 범위에서 서버 우선 가구 read와 원격 구독을 시작하고 Firebase SDK token refresh와 서버 Rules로 수렴한다. `onIdTokenChanged`, Android Activity resume event, Web focus·pageshow·online, 15분 안전 확인, callable `UNAUTHENTICATED`, Firestore listener error를 단일 `recoverRemoteSession` 경로로 모은다. 강제 refresh가 실패·5초 무응답일 때 Native→Web custom-token 교환으로 복구한다. Native callable이 `UNAUTHENTICATED`이면 Native Auth를 한 번만 재인증하고 같은 custom-token 교환을 다시 수행한다. 성공 시 `remoteReadEpoch`를 증가시켜 error callback으로 종료된 모든 listener를 재생성한다. Web Auth가 없는 최초 설치·명시적 로그아웃 뒤 로그인에서도 Native→Web 교환을 요청한다. Command module은 첫 화면의 정적 import 선행 조건에서 제외하고 Web App Check 초기화는 수행하지 않는다. Native App Check는 Application 시작 때 설치한다. Native SessionMirror 갱신도 첫 화면과 경쟁하지 않도록 인증 완료 뒤 idle task로 실행하고 같은 UID·가구·멤버에서는 최대 하루 한 번만 수행한다.
