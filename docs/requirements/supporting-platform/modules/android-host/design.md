...
6. Web application은 기본 Firestore realtime 전송을 유지한다. Android는 WebView IndexedDB가 cache-only 상태로 고착되는 경로를 제거하기 위해 Firestore memory cache를 사용하고, 일반 브라우저와 PWA만 multiple-tab IndexedDB `persistentLocalCache`를 활성화한다. 같은 UID bootstrap cache의 Household metadata는 즉시 표시하고 가구 문서 `getDocFromServer` refresh는 비차단으로 실행합니다. 현재 월 원장·카테고리·지역화폐 listener는 `includeMetadataChanges=true`로 구독한 뒤 최초 `fromCache=true` snapshot을 무시하여 서버 snapshot부터 표시합니다.
