...
6. Web application은 기본 Firestore realtime 전송을 유지한다. Android는 WebView IndexedDB가 cache-only 상태로 고착되는 경로를 제거하기 위해 Firestore memory cache를 사용하고, 일반 브라우저와 PWA만 multiple-tab IndexedDB `persistentLocalCache`를 활성화한다. 첫 가계부 화면의 가구 문서는 `getDocFromServer`로 읽고, 현재 월 원장·카테고리·지역화폐 listener는 `includeMetadataChanges=true`로 구독한 뒤 최초 `fromCache=true` snapshot을 무시한다. 서버 snapshot이 도착하기 전에는 이전 화면 값을 방출하지 않으며 read 실패를 cache fallback으로 숨기지 않는다.
7. WebView Firebase Auth는 local persistence를 사용하고 마지막 서버 검증 Membership 연결 정보만 Firebase UID와 함께 localStorage의 versioned cache로 보존한다. 앱 재실행 때 이 값으로 반복적인 Membership callable과 Native session exchange를 줄일 수 있지만 가구명·가구원·현재 월 원장·카테고리·지역화폐 잔액은 저장하거나 선표시하지 않는다. 기존 version에 남은 가구 화면 필드는 읽지 않고 다음 Membership 저장 시 제거한다.
8. 영속 Web Auth가 같은 UID로 복원되면 마지막 검증 Membership 범위에서 서버 우선 가구 read와 원격 구독을 시작하고 Firebase SDK token refresh와 서버 Rules로 수렴한다. `onIdTokenChanged`, Web focus·pageshow·online, 15분 안전 확인, callable `UNAUTHENTICATED`, Firestore listener error를 단일 `recoverRemoteSession` 경로로 모은다. Android Activity resume event는 인증 갱신의 15분 제한과 별개인 Ledger 전용 읽기 epoch를 증가시켜 현재 월·연 원장과 지역화폐 listener만 다시 열며, 자산·카테고리·통계 read model에는 전파하지 않는다. 인증 갱신 자체는 만료 가능성이 있을 때만 기존 제한에 따라 수행한다. 같은 scope의 재연결은 이미 표시한 서버 snapshot을 지우지 않지만 실패한 최신 상태 확인을 정상 상태처럼 숨기지 않는다. 강제 refresh가 실패·5초 무응답일 때 Native→Web custom-token 교환으로 복구한다. Native callable이 `UNAUTHENTICATED`이면 Native Auth를 한 번만 재인증하고 같은 custom-token 교환을 다시 수행한다. 인증 복구 성공 시에는 전역 `remoteReadEpoch`를 증가시켜 error callback으로 종료된 모든 listener를 재생성한다. Web Auth가 없는 최초 설치·명시적 로그아웃 뒤 로그인에서도 Native→Web 교환을 요청한다. Command module은 첫 화면의 정적 import 선행 조건에서 제외하고 Web App Check 초기화는 수행하지 않는다. Native App Check는 Application 시작 때 설치한다. Native SessionMirror 갱신도 첫 화면과 경쟁하지 않도록 인증 완료 뒤 idle task로 실행하고 같은 UID·가구·멤버에서는 최대 하루 한 번만 수행한다.
