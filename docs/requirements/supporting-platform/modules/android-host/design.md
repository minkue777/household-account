...
6. Web application은 기본 Firestore realtime 전송을 유지한다. Android는 WebView IndexedDB가 cache-only 상태로 고착되는 경로를 제거하기 위해 Firestore memory cache를 사용하고, 일반 브라우저와 PWA만 multiple-tab IndexedDB `persistentLocalCache`를 활성화한다. 현재 월 원장 listener의 마지막 성공 snapshot은 household·월·거래 유형 key로, 카테고리 listener의 마지막 성공 snapshot은 household key로 localStorage 표시 hint에 저장하여 다음 첫 화면에서 Firestore 초기화보다 먼저 방출한다. 따라서 Android의 첫 화면 성능은 Firestore 영속 cache 제거와 무관하게 유지된다. authoritative household 부재·권한·계약·불변식 오류는 cache fallback 대상이 아니며 SessionScope와 화면 hint를 폐기한다.
7. WebView Firebase Auth는 local persistence를 사용하고 마지막 서버 검증 Membership과 화면 표시용 가구 read model을 Firebase UID와 함께 localStorage의 versioned cache로 보존한다. 앱 재실행 때 Auth observer를 기다리지 않고 마지막 검증 가구 snapshot부터 렌더링하며, 이후 복원된 Auth UID가 cache UID와 다르면 즉시 폐기한다. 평문 가구·현재 월 원장 cache 노출 위험보다 현재 두 가구와 소수의 신뢰된 사용자 환경에서 첫 화면 속도를 우선하며, 이 값은 권한 증거로 사용하지 않는다.
8. 영속 Web Auth가 같은 UID로 복원되면 마지막 검증 Membership의 원격 구독을 즉시 재개하고 Firebase SDK token refresh와 서버 Rules로 수렴한다. `onIdTokenChanged`, Android Activity resume event, Web focus·pageshow·online, 15분 안전 확인, callable `UNAUTHENTICATED`, Firestore listener error를 단일 `recoverRemoteSession` 경로로 모은다. 강제 refresh는 원격 구독의 선행 gate로 사용하지 않으며 실패·5초 무응답일 때 Native→Web custom-token 교환으로 복구한다. Native callable이 `UNAUTHENTICATED`이면 Native Auth를 한 번만 재인증하고 같은 custom-token 교환을 다시 수행한다. 성공 시 `remoteReadEpoch`를 증가시켜 error callback으로 종료된 모든 listener를 재생성하고, 실패 시 cache 화면을 정상 연결로 오인하지 않도록 degraded 상태와 backoff를 기록한다. Web Auth가 없는 최초 설치·명시적 로그아웃 뒤 로그인에서도 Native→Web 교환을 요청한다. App Check와 Command module은 첫 paint의 정적 import 선행 조건에서 제외하되 idle로 늦추지 않고 즉시 동적 초기화를 시작한다. Native SessionMirror 갱신도 첫 화면과 경쟁하지 않도록 인증 완료 뒤 idle task로 실행하고 같은 UID·가구·멤버에서는 최대 하루 한 번만 수행한다.
...
10. 같은 로그인 세대의 월 원장·자산 query snapshot은 조회 key별 메모리 projection에 남겨 route 재진입 시 즉시 방출한다. 현재 월 원장은 위 localStorage 표시 hint까지 사용하고, 자산 query는 권위 인증이 확인된 뒤 idle callback에서 미리 시작한다. 세션 전환은 retained projection 전체를 초기화한다. 숨겨진 모달·차트·검색 컴포넌트는 실제 열릴 때 동적 로드한다.
