...
8. 영속 Web Auth가 복원되면 Native session exchange를 생략하고 Firebase SDK token refresh와 서버 Rules로 수렴한다. Web Auth가 없는 최초 설치·명시적 로그아웃 뒤 로그인에서만 Native→Web custom-token 교환을 요청한다. App Check와 Command module은 첫 paint의 정적 import 선행 조건에서 제외하되 보호 callable 직전에 초기화한다. Native SessionMirror 갱신도 첫 화면과 경쟁하지 않도록 인증 완료 뒤 idle task로 실행하고 같은 UID·가구·멤버에서는 최대 하루 한 번만 수행한다.
