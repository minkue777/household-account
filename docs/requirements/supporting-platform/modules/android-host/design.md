...
|---|---|---|
| `UpdateTransactionClient` | Ledger `UpdateTransaction` | command outbox가 비동기로 호출하고 정규 성공 때 entry 완료 |
| `DeleteTransactionClient` | Ledger `DeleteTransaction` | command outbox가 비동기로 호출하고 정규 성공 때 entry 완료 |
| `SplitTransactionClient` | Ledger `SplitTransaction` | command outbox가 비동기로 호출하고 정규 성공 때 entry 완료 |
| `RequestHouseholdNotificationClient` | Ledger의 명시적 알림 요청 Command | command outbox가 비동기로 호출; 로컬 미저장 편집은 포함하지 않음 |
...
6. Web application은 Firestore realtime 구독을 사용한다. Android WebView와 iPhone 홈 화면 PWA는 memory cache를 사용해 IndexedDB cache-only 고착과 이전 실행의 primary lease·초기 저장소 작업 대기를 피한다. 일반 브라우저는 multiple-tab IndexedDB `persistentLocalCache`를 유지한다. 모바일 Firestore의 실행 간 resume token은 보존하지 않아 재전송이 늘 수 있지만, 별도 Firebase Auth 영속 저장소와 세션 metadata 복원은 유지한다. iPhone PWA에만 `experimentalForceLongPolling=true`를 설정하여 WebKit이 열린 stream의 완료 신호를 보류하는 문제를 피한다. 서버가 데이터를 보낸 뒤 응답을 닫고 다음 응답을 기다리는 SDK 전송 방식이며, 앱의 주기별 전체 재조회는 아니다. 그 외 환경의 기본 전송·Firebase Auth 저장소·자산 및 종목 검색 cache 정책은 유지한다. 같은 UID bootstrap cache의 Household metadata는 즉시 표시하고 가구 문서 `getDocFromServer` refresh는 비차단으로 실행합니다. 현재 월 원장·카테고리·지역화폐 listener는 `includeMetadataChanges=true`로 구독한 뒤 최초 `fromCache=true` snapshot을 무시하여 서버 snapshot부터 표시합니다.
7. WebView Firebase Auth는 local persistence를 사용하고 마지막 검증 Membership 연결과 Household 표시 metadata를 Firebase UID와 함께 localStorage의 versioned cache 한 record로 보존합니다. 앱 재실행 때 Auth UID가 같으면 이를 원자 복원해 반복적인 Membership callable과 Native session exchange를 줄이고 업무 구독을 바로 시작합니다. 현재 월 원장·카테고리·지역화폐 payload나 capability 판정은 저장하지 않습니다.
8. cache hit이면 app shell의 월 원장·카테고리·지역화폐 listener를 즉시 시작하고 Household one-doc refresh를 병렬 실행합니다. metadata 정규화 결과가 실제로 다를 때만 UI·cache를 교체합니다. cache miss와 Rules `permission-denied`만 `ResolveSignedInUser` 권위 해석을 실행하며 시간·idle·background 주기로 Membership을 재해석하지 않습니다. `onIdTokenChanged`, Web focus·pageshow·online, 15분 안전 확인은 Firebase credential과 종료 listener의 복구 신호일 뿐 Membership 해석 주기가 아닙니다. Android Activity resume event는 인증 갱신의 15분 제한과 별개인 Ledger 전용 읽기 epoch를 증가시켜 현재 월·연 원장과 지역화폐 listener만 다시 열며, 자산·카테고리·통계 read model에는 전파하지 않습니다. 같은 scope의 재연결은 이미 표시한 서버 snapshot을 지우지 않지만 실패한 최신 상태 확인을 정상 상태처럼 숨기지 않습니다. 강제 refresh가 실패·5초 무응답일 때 Native→Web custom-token 교환으로 복구하고, Native callable이 `UNAUTHENTICATED`이면 Native Auth를 한 번만 재인증합니다. Web Auth가 없는 최초 설치·명시적 로그아웃 뒤 로그인에서도 Native→Web 교환을 요청합니다. Command module은 첫 화면의 정적 import 선행 조건에서 제외하고 Web App Check 초기화는 수행하지 않으며 Native App Check는 Application 시작 때 설치합니다. Native SessionMirror 동기화는 명시적인 session 결과를 mirror에 전달하는 절차이며 주기 Membership 해석을 시작하지 않습니다.
...
7. 로그아웃은 인증 세션이 남아 있을 때 `FcmService` component 비활성화와 기존 알림 취소를 가장 먼저 시도한 뒤 로컬 억제 상태 저장, endpoint 삭제와 Firebase Messaging unregister를 각각 유한 시간 best-effort로 수행한다. component·preference·원격 삭제·unregister 중 하나의 실패가 나머지 시도나 로그아웃을 막지 않는다. 이후 Queue 삭제와 mirror·legacy identity·Native/Web 인증 제거를 반드시 완료하며, 남은 endpoint binding은 다음 로그인 등록의 원자 교체 안전장치로 수렴시킨다.
...
6. base draft, 항목 목록, expectedVersion을 단일 `SplitTransaction` envelope로 command outbox에 commit한다. 선행 Update, 원본 선삭제와 순차 생성은 금지한다.
7. `Conflict(VERSION_MISMATCH)`면 암호화 outbox의 immutable draft를 실패 알림 전달 전까지만 `needs-attention`으로 보존하고 같은 payload를 자동 재시도하지 않는다. 민감값 없는 알림 전달 성공 뒤 payload를 삭제한다. 사용자는 최신 거래를 다시 열어 active 상태를 확인한 뒤 새 draft를 만들며, `superseded`·`deleted` 원본에는 재제출하지 않는다.
...
| `AndroidWebReadCachePolicy` | 내부 Policy | Web Firestore 초기화·cache-first read 설정 | Android bridge·일반 브라우저, cache hit·miss, 서버 deadline table fixture |
...
| `LedgerTransactionQueryClient` | out | ID-only legacy QuickEdit entry의 generated Ledger Query client | current snapshot·stale Stub; 새 snapshot entry에서 호출 0회 검증 |
...
- 위 Client event는 QuickEdit FIFO에 멱등 등록하기 위한 신호일 뿐 거래 생성의 receipt나 재시도 근거가 아니다. 새 event의 snapshot은 FIFO에 암호화 보존해 복구 시 재사용하고, snapshot이 없는 전환 전 entry만 Ledger에서 다시 읽는다.
...
android/app/src/main/java/com/household/account/
  ...
  feature/quick-edit/
    ...
    model/QuickEditCommandOutboxEntry.kt
    ports/QuickEditPendingQueuePort.kt
    ports/QuickEditCommandOutboxPort.kt
...
| QE-009 | U, UI, E2E | QuickEditPresentationPolicy·PendingQueue·ActivityCoordinator | A 표시 중 B·C 도착, 같은 ID 중복, 같은 시각 sequence, 새 snapshot·ID-only legacy, process 재시작, A outbox 접수·접수 실패·닫기, stale version·다른 session | A 유지, 접수 성공 뒤 중복 없는 A→B→C, 새 snapshot은 Query 0회·legacy만 fallback, stale Command Conflict, outbox 쓰기 실패 시 A 유지, 이전 session 미표시 | T-QE-003 |
