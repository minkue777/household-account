...
| AND-005 | 현재 명세 | Android 앱은 Credential Manager와 Firebase Auth Native SDK로 Google 로그인하고, Web Auth가 없는 최초 설치·로그아웃 뒤 로그인에서 서버의 Firebase custom token을 직접 전달하여 같은 UID로 WebView에 로그인한다. 서버는 `webview-session-token.v1` 응답에 Web·Native용 토큰과 `principalUid`, `signedInUserResolution`을 함께 반환하고 각 클라이언트는 인증 UID 일치 후 해석 결과를 사용한다. 최초 방문자는 가구 권한 claim 없이 생성·참여로 안내한다. 최초 교환에 성공한 WebView는 Firebase Auth local persistence와 마지막으로 검증된 UID·Membership·Household 표시 metadata를 함께 보존한다. 다음 앱 실행에 Web Auth UID와 cache UID가 같으면 scope와 가구 metadata를 즉시 복원하고 원장·카테고리·지역화폐 구독을 시작한다. cache miss와 Rules `permission-denied`만 Membership 권위 해석을 실행하며 시간·idle 기반 주기 해석은 하지 않는다. Native callable이 `UNAUTHENTICATED`를 반환하면 같은 만료 세션을 반복하지 않고 Native Auth를 한 번 재인증한 뒤 custom-token 교환을 다시 수행한다. | 자체 5분·1회용 exchange handle·Membership receipt는 두지 않으며 토큰 검증·만료는 Firebase SDK 계약을 따른다. Native용 토큰은 Web bridge로 전달하지 않는다. Membership 조회·토큰 발급 실패를 부분 성공으로 숨기지 않는다. local bootstrap은 표시 연결 정보일 뿐 권한 증거가 아니며 Firestore Rules와 Functions가 실제 read/write를 검증한다. 가구 metadata 한 문서 서버 refresh는 비차단이고 실제 차이만 반영한다. 원장·카테고리·지역화폐는 서버 snapshot부터 표시한다. `permission-denied`는 Membership 복구 coordinator 하나로 보내고 Native token 복구와 병렬 실행하지 않는다. 권위 결과가 같은 scope면 listener를 다시 열고, UID·가구·멤버가 달라지거나 first visit이면 그때 이전 SessionScope와 화면을 폐기한다. 로그아웃은 Native/Web 인증과 bootstrap cache를 제거한다. | [AndroidHostBridge](../../../../../android/app/src/main/java/com/household/account/webhost/AndroidHostBridge.kt), [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [Membership cache](../../../../../web/src/features/access-household/application/signedInMembershipCache.ts), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx), [HouseholdPreferences](../../../../../android/app/src/main/java/com/household/account/util/HouseholdPreferences.kt), [Firebase Android Google 로그인](https://firebase.google.com/docs/auth/android/google-signin), [DEC-021](../../../governance/decisions.md#dec-021), [DEC-022](../../../governance/decisions.md#dec-022), [DEC-068](../../../governance/decisions.md#dec-068), [DEC-073](../../../governance/decisions.md#dec-073) | U, C, 보안 UI, E2E |
| AND-006 | 현재 명세 | JavaScript bridge는 허용된 origin에서만 노출되어야 한다. | 허용 origin의 top-level 문서에만 versioned bridge를 노출한다. 외부 main-frame navigation은 외부 브라우저로 넘기고 subframe 요청은 거부한다. | [MainActivity](../../../../../android/app/src/main/java/com/household/account/MainActivity.kt) | 보안 UI, E2E |
...
| AND-008 | 목표 명세 | Android 일반 로그·crash breadcrumb·analytics에는 가구 key·householdId 원문, 멤버 이름, FID·registration token, 인증 token, 알림 원문, 거래 memo를 기록하지 않는다. | 상관관계가 필요하면 목적별 salt를 사용한 비가역 hash와 안정 오류 code만 기록한다. Bridge와 FCM callback에 가구 key·token 원문 로그를 두지 않는다. DEC-068 결제 지연 계측은 observationId의 SHA-256 앞 16자리만 사용하고 원문을 기록하지 않는다. | [AndroidHostBridge](../../../../../android/app/src/main/java/com/household/account/webhost/AndroidHostBridge.kt), [FcmService](../../../../../android/app/src/main/java/com/household/account/service/FcmService.kt), [FID endpoint manager](../../../../../android/app/src/main/java/com/household/account/util/FidEndpointManager.kt) | U, 보안 E2E |
| AND-009 | 목표 명세 | 인증 상태, SessionMirror, legacy 가구 key, WebView의 민감 저장소, 암호화 결제 Queue와 그 metadata는 Android cloud backup·기기 간 이전에서 기본 거부한다. | 비민감 QuickEdit 표시 설정처럼 복원 가치가 확인된 항목만 명시적 allowlist로 허용한다. `allowBackup=false`와 구형 backup·Android 12+ device transfer 제외 규칙을 함께 유지한다. | [Android Manifest](../../../../../android/app/src/main/AndroidManifest.xml), [HouseholdPreferences](../../../../../android/app/src/main/java/com/household/account/util/HouseholdPreferences.kt), [DEC-032](../../../governance/decisions.md#dec-032) | 보안 E2E |
| AND-010 | 현재 명세 | Android 13(API 33) 이상에서는 필수 권한이 준비되어 가계부에 최초 진입할 때 `POST_NOTIFICATIONS` 권한을 한 번 자동 요청한다. 이미 허용됐거나 이전 요청 이력이 있으면 다시 요청하지 않는다. | 거부해도 WebView 진입, 알림 접근 기반 결제 수집, QuickEdit, FID endpoint 등록을 막지 않는다. API 32 이하는 요청하지 않는다. 앱 내 푸시 설정 UI를 추가하지 않으며 이후 허용·거부 변경은 Android 시스템 설정에서 수행한다. | [Android Manifest](../../../../../android/app/src/main/AndroidManifest.xml), [MainActivity](../../../../../android/app/src/main/java/com/household/account/MainActivity.kt), [PUSH-001·007](../../../contexts/notifications/modules/notifications/requirements.md#5-요구사항), [DEC-072](../../../governance/decisions.md#dec-072) | UI, E2E |
...
| AND-012 | 현재 명세 | Android WebView와 iPhone 홈 화면 PWA의 Firestore는 실행 중 memory cache를 사용하여 이전 실행의 IndexedDB primary lease와 저장소 초기화 대기를 피한다. 일반 브라우저는 multiple-tab IndexedDB `persistentLocalCache`를 유지한다. iPhone PWA는 WebKit의 열린 응답 stream 완료 신호 지연을 피하도록 `experimentalForceLongPolling=true`를 유지한다. Firebase Auth의 별도 영속 로그인 저장소는 유지하며, 모바일 Firestore의 실행 간 resume token은 보존하지 않는다. 별도 주기 조회·timeout 조정은 추가하지 않는다. Android 첫 가계부 화면은 같은 UID의 bootstrap cache에서 가구 metadata를 즉시 복원하고 `getDocFromServer` one-doc refresh를 비차단으로 실행한다. app shell의 월 원장·카테고리·지역화폐 listener는 최초 `fromCache=true` snapshot을 무시하고 서버 snapshot부터 표시하며 내부 route 이동에서 유지한다. 월 원장은 연·월 또는 SessionScope 변경에서, 지역화폐는 SessionScope 변경에서만 교체한다. `onIdTokenChanged`·Activity resume·focus·online·15분 안전 확인은 Firebase credential과 종료 listener를 복구할 수 있지만 Membership 권위 해석은 cache miss·`permission-denied`에만 실행한다. Native 인증 refresh가 실패하거나 5초 동안 끝나지 않으면 Native 로그인 세션으로 자동 교환한다. navigation HTML은 Service Worker가 cache하지 않고 새 Service Worker controller가 활성화되면 현재 runtime을 한 번 갱신한다. | 원장·카테고리·지역화폐의 localStorage 표시 snapshot은 저장하거나 읽지 않는다. Household metadata refresh의 transient 실패는 업무 구독을 취소하지 않고 실제 차이만 UI에 반영한다. `permission-denied`는 bootstrap cache와 오류 listener를 지우고 권위 해석으로 전달하되 마지막 in-memory 화면은 유지한다. 같은 scope이면 read epoch로 listener를 다시 열고 first visit·다른 scope가 확인될 때만 이전 scope·화면·나머지 구독을 폐기한다. 다른 첫 서버 read 실패는 과거 금융 화면을 정상값으로 보여주지 않는다. 자산 route 재진입 snapshot과 종목 검색 cache는 별도 정책으로 유지한다. Command 모듈, 자산 warm-up과 숨겨진 모달·차트·검색 UI는 첫 paint의 정적 bundle 선행 조건으로 만들지 않는다. Web App Check 런타임은 사용하지 않고 Native App Check는 Application에서 설치한다. | [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [Firestore 초기화](../../../../../web/src/lib/firebase.ts), [원장 서버 우선 구독](../../../../../web/src/lib/expenseService.ts), [카테고리 서버 우선 구독](../../../../../web/src/lib/categoryService.ts), [지역화폐 서버 우선 구독](../../../../../web/src/lib/balanceService.ts), [가구 서버 조회](../../../../../web/src/lib/householdService.ts), [AppProviders](../../../../../web/src/components/AppProviders.tsx), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx), [DEC-051](../../../governance/decisions.md#dec-051), [DEC-068](../../../governance/decisions.md#dec-068) | U, WebView E2E |
...
| AND-014 | 현재 명세 | Android 앱 첫 화면 성능은 `MainActivity` 인스턴스 생성 시점의 `SystemClock.elapsedRealtime()`부터 Web의 월 원장·카테고리·지역화폐와 현재 설정이 노출하는 연간 합계가 최신 서버 결과로 성공하고 실제 paint된 시점까지 측정한다. | WebView navigation만 재거나 custom-token 교환 시간을 앱 실행으로 대체하지 않는다. 같은 Activity에서 WebView를 reload해도 Native 시작 시간은 한 번만 소비하며, 2분 초과·구 APK bridge·조회 실패는 성공 표본으로 기록하지 않는다. 계측 요청과 로그 실패는 화면 표시·접속 기능을 실패시키지 않고 사용자·가구·거래 식별자를 기록하지 않는다. | [MainActivity](../../../../../android/app/src/main/java/com/household/account/MainActivity.kt), [AppLaunchDurationClock](../../../../../android/app/src/main/java/com/household/account/startup/AppLaunchDurationClock.kt), [AndroidHostBridge](../../../../../android/app/src/main/java/com/household/account/webhost/AndroidHostBridge.kt), [Web startup performance](../../../../../web/src/platform/performance/webStartupPerformance.ts) | U, UI, E2E |
...
| QE-013 | 현재 명세 | Android QuickEdit은 메모 아래 `태그` 입력과 `태그를 입력하세요` 안내를 제공하고 기존 태그를 초기 표시하여 추가·수정·제거할 수 있다. 태그만 바꿔도 일반 Ledger Update를 제출하며 마지막 태그 제거는 명시한 `tags: []`로 전달한다. 저장·분할 시 입력 중인 태그도 현재 초안에 반영한다. | `LED-011`의 공백·선행 해시 제거, 최초 순서·철자·내부 공백 유지, 동일 문자열 중복 제거와 신규 입력 최대 10개·각 30 Unicode code point 계약을 사용한다. 변경 없는 기존 초과 값은 잘라내거나 다시 검증해 다른 필드 수정을 막지 않는다. 구버전 snapshot·Intent·queue의 태그 생략은 빈 표시로 호환하며 변경 없는 저장·분할에서 생략을 빈 배열 삭제로 바꾸지 않는다. 입력 오류는 초안과 화면을 유지하고 command outbox에 넣지 않는다. 태그를 추천하기 위한 별도 서버 조회와 수입 태그 입력은 추가하지 않는다. | [QuickEditActivity](../../../../../android/app/src/main/java/com/household/account/QuickEditActivity.kt), [Ledger 태그 계약](../../../contexts/household-finance/modules/ledger/design.md#321-태그-값과-호환-계약--led-011), [DEC-075](../../../governance/decisions.md#dec-075) | U, C, UI, I, E2E; T-QE-008 |
...
| [DEC-073](../../../governance/decisions.md#dec-073) | 확정 | Firebase custom token 직접 전달과 영속 Web Auth 재사용을 유지합니다. 별도 5분·1회용 교환권 보장은 제거하고 실제 서버 발급·Web 로그인 경로를 테스트합니다. |
...
| T-QE-003 | 현재 명세 | 열린 QuickEdit A 중 B·C 연속 도착, 같은 ID 재전달, 새 snapshot·구버전 ID-only entry, process 재시작, A outbox commit·예약 성공/실패·명시 닫기, stale version, session 전환 / FIFO 표시 조정 / A 유지, 접수 성공 직후 중복 없이 A→B→C, 새 entry는 Query 0회·구버전만 Query fallback, stale Command는 Conflict, outbox 쓰기·예약 실패 시 A 유지, 다른 session 거래 미표시 | QE-009, QE-012, DEC-054, DEC-067, DEC-068 |
| T-QE-004 | 목표 | 가맹점·금액·카테고리·memo·tags를 미저장 수정한 뒤 분할, 제출 직전 다른 사용자의 수정·분할 / 원자 Split 제출 / 현재 form과 payload 일치, provenance 서버 보존, 먼저 commit한 한 요청만 성공하고 stale 요청은 write 0건·로컬 초안 유지 | QE-010, QE-006, DEC-055 |
...
| T-ANDROID-NOTIFICATION-PERMISSION-001 | 목표 | API 32·33 이상에서 표시 권한 허용·거부·기존 요청 이력 / 가계부 최초 진입·Activity 재생성 / API 33 이상에서 미허용·미요청일 때만 한 번 요청하고 거부해도 WebView·수집·QuickEdit·FID 등록 유지, 재생성 때 자동 재요청하지 않음 | AND-010, DEC-072 |
...
| T-ANDROID-STARTUP-001 | 현재·성능·멱등 | Activity 신규 생성, 같은 Activity WebView reload, 월 원장·카테고리·지역화폐·연간 합계의 서로 다른 도착 순서와 실패, 구 APK bridge, 2분 경계 / 앱 시작→첫 화면 전체 표시 측정 / 최신 서버 데이터가 모두 성공해 실제 paint된 신규 Activity에서만 PII 없는 Android 표본을 한 번 기록하고 계측 실패는 사용자 화면을 막지 않음 | AND-014, ADM-005 |

추가 UI·instrumentation 검증 대상은 권한 조합별 첫 화면, Native Google 로그인 성공·취소·실패, Firebase custom token으로 같은 UID 로그인·불일치 거부, legacy localStorage 전환, QuickEdit 비활성 사용자의 오버레이 권한 부재, bridge 허용 origin과 navigation 이후 차단, WebView 뒤로가기, 빈 memo 삭제, 누락 Intent, 잠금 화면 정책입니다. 실제 서버 발급 함수·Web authService 검증과 Credential Manager·Native SDK·WebView의 기기 전체 왕복 검증은 구분하며, 미실행 항목은 [감사 보고서](../../../../verification/full-audit-remediation.md#gap-and-005)에 기록합니다.
