...
| AND-005 | 목표 명세 | Android 앱은 Credential Manager와 Firebase Auth Native SDK로 Google 로그인하고, 서버가 인증 Principal에 귀속된 active Membership을 조회한 뒤 허용된 WebView origin에는 최대 5분의 짧고 일회성인 세션 교환 계약으로 동일 Principal을 전달한다. 서버는 custom token과 같은 UID로 확정한 authoritative Membership resolution을 한 응답에 싣고, Native와 Web은 token 로그인 결과 UID와 응답 UID가 같을 때만 이를 mirror와 최초 SessionScope에 재사용해 별도 Membership 왕복을 생략한다. 앱/WebView 시작 때 Native Firebase 세션이 있으면 사용자 입력 없이 교환을 다시 수행하며, WebView URL load·hydration과 세션 교환을 병렬 prewarm하고 Bridge 로그인은 같은 단일 사용 in-flight 결과를 소비한다. WebView는 장기 IndexedDB 인증 상태 대신 process 수명의 메모리 인증 세션을 사용한다. | prewarm은 Native `currentUser`가 이미 있을 때만 시작하고, 사용자가 없으면 Credential Manager나 Google UI를 자동으로 열지 않는다. caller·projection이 보낸 householdId·memberId·status를 Membership 증거로 신뢰하지 않으며, active view는 후보 위치만 제공한다. 서버는 canonical Membership과 Member의 존재·active 상태·Principal/가구/멤버 일치를 fail-closed로 검증하고 여러 active view나 불일치를 첫 방문으로 축약하지 않는다. resolution이 없는 실제 구버전 응답만 기존 인증된 Membership Command로 fallback하고, 현재 서버의 resolution 오류는 token-only 성공으로 숨기지 않는다. 로그아웃은 endpoint 삭제를 먼저 유한 시간 시도하되 원격 실패와 무관하게 local Queue·mirror·Native/Web 인증을 반드시 제거한다. Google OAuth 화면을 embedded WebView에서 열지 않고 householdKey·Google credential·장기 Firebase token을 일반 JavaScript Bridge API로 노출하지 않는다. Native 교환 중 auth observer callback은 세션 확정에 사용하지 않으며, 교환은 유한 deadline 안에 성공 또는 오류 상태로 끝난다. 현재 localStorage 가구 키·멤버 동기화는 DEC-021 전환 후보 포착에만 사용한 뒤 제거하고 partnerName은 목표 session에 포함하지 않는다. | [AndroidHostBridge](../../../../../android/app/src/main/java/com/household/account/webhost/AndroidHostBridge.kt), [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx), [HouseholdPreferences](../../../../../android/app/src/main/java/com/household/account/util/HouseholdPreferences.kt), [Firebase Android Google 로그인](https://firebase.google.com/docs/auth/android/google-signin), [Google Sign-In 권장사항](https://developers.google.com/identity/siwg/best-practices), [DEC-021](../../../governance/decisions.md#dec-021), [DEC-022](../../../governance/decisions.md#dec-022) | U, C, 보안 UI, E2E |
...
| AND-012 | 현재 명세 | Android WebView는 별도 long-polling을 강제하지 않고 기본 Firestore realtime 전송을 사용하며, Android Host에서만 single-tab IndexedDB `persistentLocalCache`를 활성화한다. 검증된 Membership의 가계부는 캐시 snapshot이 있으면 먼저 표시하고 서버 Read Contract 결과로 갱신한다. Native→Web 인증 교환은 60초, 로그인 Membership 확인과 최초 가계부 읽기는 각각 20초 안에 성공 또는 명시적 실패로 끝낸다. | 일반 브라우저의 인증 persistence와 기본 Firestore 초기화는 바꾸지 않는다. 캐시가 없고 auth observer 또는 네트워크가 응답하지 않으면 `로딩중` 상태를 무기한 유지하지 않고 오류와 재시도 action을 표시한다. 캐시가 있으면 deadline·network unavailable 같은 일시적인 transport 실패만 흡수한다. 서버가 authoritative household 부재를 반환하거나 permission·계약·불변식 오류를 반환하면 캐시 화면을 유지하지 않고 SessionScope를 폐기한다. | [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [Firestore 초기화](../../../../../web/src/lib/firebase.ts), [Firestore Read Model](../../../../../web/src/platform/read-model/firestoreReadModel.ts), [가구 cache-first 조회](../../../../../web/src/lib/householdService.ts), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx) | U, WebView E2E |
...
| T-WEBVIEW-004 | 현재 명세 | Android Host bridge 있음·일반 브라우저, IndexedDB cache 있음·없음, Firestore 응답 성공·20초 이상 무응답 / 최초 세션 복원 / 기본 realtime 전송을 유지하고 Android에서만 persistent cache를 사용하며, cache snapshot을 먼저 표시한 뒤 서버값으로 갱신하고 cache 없는 무응답은 오류·재시도 상태로 종료 | AND-012 |
