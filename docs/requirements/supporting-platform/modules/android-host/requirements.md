...
| T-WEBVIEW-004 | 현재 명세 | Android 재실행의 마지막 검증 UID·Membership·가구·현재 월 원장·카테고리 snapshot, cache 없음, Auth/Native 교환 지연, UID 불일치, Firestore 성공·무응답·권한 거부, 원장↔자산 재진입 / 세션과 화면 복원 / cache가 있으면 Auth·Native 교환 전에 가구·월 원장·카테고리를 즉시 표시하고 서버값으로 수렴하며, UID/권한 불일치는 hint 폐기, cache 없는 무응답은 오류·재시도로 종료, 로그아웃·세션 전환 뒤 이전 snapshot 미표시 | AND-005, AND-012, DEC-068 |
