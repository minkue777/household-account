...
| AND-012 | 현재 명세 | Android WebView의 Firestore는 프로세스 수명의 memory cache를 사용하고, 일반 브라우저와 PWA만 multiple-tab IndexedDB `persistentLocalCache`를 사용한다. Android 첫 가계부 화면은 가구 문서를 `getDocFromServer`로 읽고 월 원장·카테고리·지역화폐 listener의 `fromCache=true` 최초 snapshot을 무시하여 서버 snapshot 뒤 표시한다. 영속 Firebase Auth가 같은 UID로 복원되면 마지막으로 검증된 Membership 범위의 원격 구독을 재개하고, `onIdTokenChanged`·Activity resume·focus·online·15분 안전 확인 및 callable/listener 인증 오류가 단일 복구 경로를 깨운다. refresh가 실패하거나 5초 동안 끝나지 않으면 Native 로그인 세션으로 자동 교환한다. 복구 성공은 원격 read epoch를 전진시켜 오류로 종료된 월 원장·카테고리·지역화폐·자산·명의자·보유 종목 listener를 전부 다시 생성한다. navigation HTML은 Service Worker가 cache하지 않고 새 Service Worker controller가 활성화되면 현재 runtime을 한 번 갱신한다. | 가구·현재 월 원장·카테고리·지역화폐의 localStorage 표시 snapshot은 저장하거나 읽지 않는다. 첫 서버 read가 실패하면 과거 화면을 정상값으로 보여주지 않고 오류로 처리한다. 자산 route 재진입 snapshot과 종목 검색 cache는 별도 정책으로 유지한다. Command 모듈, 자산 warm-up과 숨겨진 모달·차트·검색 UI는 첫 paint의 정적 bundle 선행 조건으로 만들지 않는다. Web App Check 런타임은 사용하지 않고 Native App Check는 Application에서 설치한다. | [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [Firestore 초기화](../../../../../web/src/lib/firebase.ts), [원장 서버 우선 구독](../../../../../web/src/lib/expenseService.ts), [카테고리 서버 우선 구독](../../../../../web/src/lib/categoryService.ts), [지역화폐 서버 우선 구독](../../../../../web/src/lib/balanceService.ts), [가구 서버 조회](../../../../../web/src/lib/householdService.ts), [AppProviders](../../../../../web/src/components/AppProviders.tsx), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx), [DEC-051](../../../governance/decisions.md#dec-051), [DEC-068](../../../governance/decisions.md#dec-068) | U, WebView E2E |
...
| T-WEBVIEW-004 | 현재 명세 | Android 재실행의 영속 Web Auth·Membership 연결 정보, Web Auth `null`, 같은 UID token 갱신, Native 교환 성공·실패·무응답·`UNAUTHENTICATED`, UID 불일치, 가구 server read, Firestore 최초 cache/server snapshot, listener 종료 오류, callable 인증 오류, 원장↔자산 재진입 / 세션과 화면 복원 / Membership 연결 정보는 반복 인증 왕복만 줄이고 가구·월 원장·카테고리·지역화폐는 서버 snapshot 전 표시하지 않는다. 복구 성공은 원격 read epoch를 전진시켜 종료된 구독을 모두 다시 만들며, 첫 서버 read 실패·UID/권한 불일치는 과거 화면으로 숨기지 않고 오류로 종료한다. Android Firestore 재실행은 IndexedDB 상태를 재사용하지 않고 memory cache에서 서버로 수렴하며 로그아웃·세션 전환 뒤 이전 화면 미표시 | AND-005, AND-012, DEC-068 |
