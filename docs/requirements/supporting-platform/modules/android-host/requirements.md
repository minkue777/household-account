...
| AND-005 | 현재 명세 | Android 앱은 Credential Manager와 Firebase Auth Native SDK로 Google 로그인하고, Web Auth가 없는 최초 설치·로그아웃 뒤 로그인에서 서버의 짧고 일회성 custom-token 계약으로 동일 Principal을 WebView에 전달한다. 최초 교환에 성공한 WebView는 Firebase Auth local persistence와 마지막으로 서버 검증된 UID·Membership·가구 화면 cache를 보존한다. 다음 앱 실행에는 이 cache를 Auth persistence 복원보다 먼저 화면 표시 hint로 사용하고, Web Auth를 재사용해 반복적인 Native session exchange를 생략하며 Firebase SDK token refresh와 서버 read로 수렴한다. Native callable이 `UNAUTHENTICATED`를 반환하면 같은 만료 세션을 반복하지 않고 Native Auth를 한 번 재인증한 뒤 custom-token 교환을 다시 수행한다. Native SessionMirror 확인은 첫 화면 이후 idle 시간에 같은 binding 기준 최대 하루 한 번 수행한다. | local Membership·가구 snapshot은 화면 선표시 hint일 뿐 권한 증거가 아니며 Firestore Rules와 Functions가 실제 read/write를 검증한다. 평문 localStorage에 가구명·가구원명과 마지막 화면 snapshot이 남는 위험은 현재 두 가구와 소수의 신뢰된 사용자 환경에서 허용하는 성능 절충이다. UID·가구·멤버가 달라지거나 first visit·권한·authoritative household 부재가 확인되면 hint와 SessionScope를 폐기한다. 로그아웃은 Native/Web 인증과 cache 진입점을 제거한다. | [AndroidHostBridge](../../../../../android/app/src/main/java/com/household/account/webhost/AndroidHostBridge.kt), [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [Membership cache](../../../../../web/src/features/access-household/application/signedInMembershipCache.ts), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx), [HouseholdPreferences](../../../../../android/app/src/main/java/com/household/account/util/HouseholdPreferences.kt), [Firebase Android Google 로그인](https://firebase.google.com/docs/auth/android/google-signin), [DEC-021](../../../governance/decisions.md#dec-021), [DEC-022](../../../governance/decisions.md#dec-022), [DEC-068](../../../governance/decisions.md#dec-068) | U, C, 보안 UI, E2E |
...
| AND-012 | 현재 명세 | Android WebView의 Firestore는 프로세스 수명의 memory cache를 사용하고, 일반 브라우저와 PWA만 multiple-tab IndexedDB `persistentLocalCache`를 사용한다. Android 첫 화면은 마지막으로 검증된 UID의 Membership·가구·월 원장·카테고리 localStorage snapshot으로 먼저 표시하므로 Firestore IndexedDB를 중복 사용하지 않는다. 영속 Firebase Auth가 같은 UID로 복원되면 마지막으로 검증된 Membership 범위의 원격 구독을 즉시 재개하고, `onIdTokenChanged`·Activity resume·focus·online·15분 안전 확인 및 callable/listener 인증 오류가 단일 복구 경로를 깨운다. refresh가 실패하거나 5초 동안 끝나지 않으면 Native 로그인 세션으로 자동 교환한다. 복구 성공은 원격 read epoch를 전진시켜 오류로 종료된 월 원장·카테고리·지역화폐·자산·명의자·보유 종목 listener를 전부 다시 생성한다. navigation HTML은 Service Worker가 cache하지 않고 새 Service Worker controller가 활성화되면 현재 runtime을 한 번 갱신한다. | 가구·현재 월 원장·카테고리 local snapshot을 평문으로 보존하는 것은 DEC-068의 소규모 신뢰 사용자 성능 절충이다. 이는 Command 권한·ActorContext·Canonical 원장이 아니며 인증·App Check·Rules를 우회하지 않는다. UID 불일치, 로그아웃, 가구/멤버 전환 또는 authoritative household 부재·permission·계약·불변식 오류가 확정되면 이전 hint를 화면에 유지하지 않는다. cache가 있어도 인증 복구 실패를 정상 연결로 숨기지 않으며 degraded 상태와 유한 backoff로 관리한다. App Check·Command 모듈, 자산 warm-up과 숨겨진 모달·차트·검색 UI는 첫 paint의 정적 bundle 선행 조건으로 만들지 않는다. | [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [Firestore 초기화](../../../../../web/src/lib/firebase.ts), [월 원장 snapshot](../../../../../web/src/features/ledger/application/monthlyExpenseSnapshot.ts), [카테고리 snapshot](../../../../../web/src/features/category-budget/application/categorySnapshot.ts), [Optimistic read projection](../../../../../web/src/platform/read-model/optimisticEntityProjection.ts), [가구 cache-first 조회](../../../../../web/src/lib/householdService.ts), [AppProviders](../../../../../web/src/components/AppProviders.tsx), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx), [DEC-051](../../../governance/decisions.md#dec-051), [DEC-068](../../../governance/decisions.md#dec-068) | U, WebView E2E |
...
| [DEC-068](../../../governance/decisions.md#dec-068) | 확정 | 소규모 운영에서는 마지막 검증 Web snapshot 선표시와 서버 `quickEditSnapshot` 직접 표시로 첫 paint·QuickEdit의 중복 대기를 제거하되 Auth·App Check·가구 격리·멱등성을 유지합니다. |
...
| T-WEBVIEW-004 | 현재 명세 | Android 재실행의 마지막 검증 UID·Membership·가구·현재 월 원장·카테고리 snapshot, Web Auth `null`, auth observer 등록 실패, 같은 UID token 갱신, Native 교환 성공·실패·무응답·`UNAUTHENTICATED`, UID 불일치, Firestore listener 성공·종료 오류, callable 인증 오류, 원장↔자산 재진입 / 세션과 화면 복원 / cache가 있으면 Auth·Native 교환 전에 가구·월 원장·카테고리를 즉시 표시한다. 복구 실패는 cache-only 정상 상태로 숨기지 않고 degraded로 정착·재시도하며, 이후 성공은 원격 read epoch를 전진시켜 종료된 구독을 모두 다시 만든다. Android Firestore 재실행은 IndexedDB 상태를 재사용하지 않고 memory cache에서 서버로 수렴하며, UID/권한 불일치는 hint 폐기, cache 없는 무응답은 오류·재시도로 종료, 로그아웃·세션 전환 뒤 이전 snapshot 미표시 | AND-005, AND-012, DEC-068 |
