...
| AND-012 | 현재 명세 | Android WebView는 single-tab, 일반 브라우저와 PWA는 multiple-tab IndexedDB `persistentLocalCache`를 사용한다. 마지막으로 검증된 UID의 Membership·가구 localStorage snapshot이 있으면 Firebase Auth persistence·IndexedDB보다 먼저 화면을 표시하고 서버 결과로 갱신한다. 영속 Firebase Auth가 같은 UID로 복원되면 마지막으로 검증된 Membership 범위의 원격 구독을 즉시 재개하고, 강제 token refresh는 구독의 선행 gate가 아닌 백그라운드 복구 작업으로 수행한다. refresh가 실패하거나 5초 동안 끝나지 않으면 Native 로그인 세션으로 자동 교환한다. 앱을 15분 이상 떠났다가 복귀해도 같은 복구를 수행한다. 현재 월 원장은 household·월·거래 유형 key로, 카테고리는 household key로 localStorage 표시 hint를 즉시 방출하고 각 Firestore listener 결과로 덮어쓴다. navigation HTML은 Service Worker가 cache하지 않으며 자산 read model은 인증 확인 뒤 idle 시간에 미리 구독한다. | 가구·현재 월 원장·카테고리 local snapshot을 평문으로 보존하는 것은 DEC-068의 소규모 신뢰 사용자 성능 절충이다. 이는 Command 권한·ActorContext·Canonical 원장이 아니며 인증·App Check·Rules를 우회하지 않는다. UID 불일치, 로그아웃, 가구/멤버 전환 또는 authoritative household 부재·permission·계약·불변식 오류가 확정되면 이전 hint를 화면에 유지하지 않는다. App Check·Command 모듈, 자산 warm-up과 숨겨진 모달·차트·검색 UI는 첫 paint의 선행 조건으로 내려받거나 실행하지 않는다. | [Web Auth Adapter](../../../../../web/src/lib/authService.ts), [Firestore 초기화](../../../../../web/src/lib/firebase.ts), [월 원장 snapshot](../../../../../web/src/features/ledger/application/monthlyExpenseSnapshot.ts), [카테고리 snapshot](../../../../../web/src/features/category-budget/application/categorySnapshot.ts), [Optimistic read projection](../../../../../web/src/platform/read-model/optimisticEntityProjection.ts), [가구 cache-first 조회](../../../../../web/src/lib/householdService.ts), [AppProviders](../../../../../web/src/components/AppProviders.tsx), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx), [DEC-051](../../../governance/decisions.md#dec-051), [DEC-068](../../../governance/decisions.md#dec-068) | U, WebView E2E |
...
| T-WEBVIEW-004 | 현재 명세 | Android 재실행의 마지막 검증 UID·Membership·가구·현재 월 원장·카테고리 snapshot, cache 없음, 같은 UID의 영속 Auth 복원, token refresh 성공·실패·무응답, Native 교환 지연, UID 불일치, Firestore 성공·무응답·권한 거부, 원장↔자산 재진입 / 세션과 화면 복원 / cache가 있으면 Auth·Native 교환 전에 가구·월 원장·카테고리를 즉시 표시하고, 같은 UID의 Auth가 복원되면 token refresh 무응답에도 원격 구독을 막지 않으며 5초 뒤 Native 복구를 시도하고 서버값으로 수렴한다. UID/권한 불일치는 hint 폐기, cache 없는 무응답은 오류·재시도로 종료, 로그아웃·세션 전환 뒤 이전 snapshot 미표시 | AND-005, AND-012, DEC-068 |
