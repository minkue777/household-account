...
| AND-012 | 현재 명세 | Android WebView가 공개된 Firestore Read Contract를 읽을 때 WebView 전용 전송 정책으로 long-polling을 사용하고, 로그인 세션 확인과 최초 가계부 읽기는 각각 20초 안에 성공 또는 명시적 실패로 끝낸다. | 일반 브라우저의 Firestore 전송 방식은 바꾸지 않는다. 네트워크가 응답하지 않아도 `로딩중` 상태를 무기한 유지하지 않고 오류와 재시도 action을 표시한다. | [Firestore 초기화](../../../../../web/src/lib/firebase.ts), [WebView 전송 정책](../../../../../web/src/platform/read-model/firestoreTransportPolicy.ts), [HouseholdContext](../../../../../web/src/contexts/HouseholdContext.tsx) | U, WebView E2E |
