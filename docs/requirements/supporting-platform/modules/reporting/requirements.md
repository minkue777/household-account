...
완료된 지출 통계 원천에서 같은 기간을 선택하면 동일한 배열을 재사용하여 차트 입력을 중복 교체하지 않습니다. 기간별 배열은 원천당 최대 4개로 제한하고 원천·Actor·revision 변경 시 함께 무효화합니다. 재진입의 서버 재확인과 최신성 정책은 유지합니다. 추이 카테고리는 현재 가구 소유의 목록에서 첫 렌더부터 계산하고, 해당 화면에서 사용자가 바꾼 선택은 유지합니다.
