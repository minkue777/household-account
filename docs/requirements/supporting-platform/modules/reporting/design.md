...
자산 이력은 재진입·앱 복귀 때 같은 세션의 완료 캐시를 즉시 표시한 채 서버를 재확인합니다. 30초 캐시는 재검증 생략이나 자동 polling을 뜻하지 않습니다. 진행 중인 동일 조회는 공유하고 기간 변경은 메모리에서 처리합니다. 재검증 실패는 기존 차트·선택 기간을 보존하며 오류를 안내합니다. 세션·가구·원격 epoch 변경과 명시적 변경 무효화는 이전 원본을 차단합니다. 현재 총자산의 별도 실시간 구독은 그대로 유지합니다.
