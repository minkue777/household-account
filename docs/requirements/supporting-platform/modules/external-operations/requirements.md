...
| EXT-004 | 현재 명세 | 운영 관리자는 Admin에서 현재 Google Cloud 프로젝트의 이번 달 순비용, 서비스별 순비용과 월말 추정액 및 SKU·리전·과금 단위별 사용량·사용 비용·크레딧·순비용을 조회할 수 있어야 한다. App Engine에 포함된 Firestore 과금 항목과 0원 사용 항목도 구분한다. | `billing-cost-refresh`가 `Asia/Seoul` 기준 6시간마다 Standard Billing Export의 `cost + credits`를 읽는다. 월말 추정액은 이번 달 누적액에 오늘을 제외한 최근 7개 완료일의 일평균과 남은 온전한 날짜 수를 곱해 더하며, 데이터가 없는 완료일은 0원으로 계산한다. 성공 시 고정 경로의 단일 Snapshot을 교체하고 실패 시 마지막 성공본을 유지한다. 표시 금액은 Export 지연과 추정식을 포함한 참고값이며 확정 청구액이 아니다. | Functions Billing Cost Adapter·Admin Dashboard | U, Adapter C, Store C, UI C, 운영 계약 |
...
| KIND occurrence에서 일부 target만 403·timeout이면 실제 HTTP status와 `search|viewer|contents|detail` 단계를 기록하고 Health는 degraded·무경보이다. 모든 target 실패가 3회 연속일 때만 outage 경보를 열며 다음 정상 또는 부분 성공에서 해제한다. | U, C, I, 운영 계약 | EXT-001, JOB-DIV-001 |
