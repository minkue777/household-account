...
| PWA-005 | 결함 | Firebase project 설정은 Web client와 통합 worker가 같은 빌드 단일 원본에서 받고, 생성된 실제 worker artifact의 Messaging SDK는 애플리케이션 SDK와 지원되는 호환 조합으로 검증한다. | 현재 worker가 Firebase compat 9.0.0과 설정 값을 별도로 고정해 Web 설정과 독립적으로 drift할 수 있다. 통합 `/sw.js`에는 compat SDK, deprecated token API, Firebase 설정 값 hardcode를 남기지 않으며 별도 `/firebase-messaging-sw.js`를 산출하지 않는다. 비밀값 저장 요구가 아니라 설정 일치·배포 계약이다. | [Web Firebase 설정](../../../../../web/src/lib/firebase.ts), [messaging worker](../../../../../web/worker/index.js) | C, production E2E |
