...
- `fetch`: build hash 정적 asset은 현재 version precache에서 제공한다. 공개 비민감 아이콘·폰트·이미지의 `GET` 성공 응답만 명시적 allowlist로 runtime cache하며 저장 시점부터 최대 7일 뒤 제거한다. `Set-Cookie`가 있거나 `Cache-Control: private|no-store`인 응답, navigation HTML, `/api/**`, 인증 header·cookie가 있거나 가구·거래·자산·통계 경로인 응답, 비 GET method와 임의 cross-origin 응답은 network-only로 처리한다. cache key에 UID·memberId·householdId·token을 넣지 않는다.
...
| `CachePolicy` | 인증·금융·API·navigation HTML과 임의 cross-origin 응답을 cache하지 않는다. build-versioned precache와 공개 아이콘·폰트·이미지의 최대 7일 runtime cache만 허용한다. |
...
- navigation HTML과 `/api/**`는 offline fallback으로 이전 성공 응답을 돌려주지 않고 네트워크 실패를 그대로 반환한다.
...
[DEC-051](../../../governance/decisions.md#dec-051)에 따라 새 worker는 waiting 상태로 설치하고, 미저장 입력이 없는 사용자의 갱신 선택 또는 모든 client 종료 뒤 재실행 때 활성화합니다. 금융·인증·API·navigation HTML과 임의 cross-origin 응답은 cache하지 않으며 공개 비민감 아이콘·폰트·이미지만 최대 7일 보존합니다.
