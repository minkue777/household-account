...
function buildParts(input: {
  ...
  return {
    ...
    transactions: split.installments.map((installment) => ({
      ...
      ...(input.source.captureLineageId === undefined ? {} : { captureLineageId: input.source.captureLineageId }),
      ...(input.source.localTime === undefined ? {} : { localTime: input.source.localTime }),
