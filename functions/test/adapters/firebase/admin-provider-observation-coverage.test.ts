...
import { advanceScheduledJobStatusSummary, scheduledJobStatus } from '../../../src/adapters/firebase/operations/scheduledJobStatusSummary';
...
it('one observed healthy route cannot conceal other unobserved providers', async () => {
  ...
  for (const definition of loadScheduledJobDefinitions().definitions) memory.seed('operations/runtime/scheduledJobStatuses/' + definition.jobName,
    { ...advanceScheduledJobStatusSummary(undefined, scheduledJobStatus({ occurrenceId: definition.jobName, jobName: definition.jobName, status: 'COMPLETE', scheduledFor: '2026-09-06T00:00:00Z' })!) });
