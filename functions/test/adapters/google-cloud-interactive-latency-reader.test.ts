...
describe("Google Cloud interactive latency reader", () => {
  it.each([
    ["최신 상세", { authReady: 300, firstHomeCompletePaint: 2_500 }, true],
    ["최신 구버전", undefined, false],
    ["최신 잘못된 상세", { authReady: -1 }, false],
    ["최신 비허용 키", { householdId: 1 }, false],
  ])("[T-ADM-004] %s를 총시간과 같은 최신 시작 표본으로 반환한다", async (_label, latestTimings, detailed) => {
      ...
      async json() {
        return { entries: [
          ...
          },
          {
            ...
            jsonPayload: {
              correlationId: "old-startup",
              endpoint: "clientStartup",
              operation: "client.ios-pwa-first-home-complete-paint.v1",
              elapsedMs: 4_000,
              status: "succeeded",
              clientStartupTimingsMs: { authReady: 900, firstHomeCompletePaint: 4_000 },
            },
          },
        ] };
      },
    }));
    const result = await reader.read({ generatedAt: "2026-09-12T01:00:00Z", windowHours: 24 });
    expect(result.operations).toHaveLength(1);
    expect(result.operations[0]).toMatchObject({
      sampleCount: 2, succeededCount: 2, failedCount: 0,
      averageMs: 3_250, p95Ms: 4_000, maxMs: 4_000,
      latestAt: "2026-09-12T00:01:00.000Z",
    });
    if (detailed) {
      expect(result.operations[0].latestStartupSample).toEqual({
        timestamp: "2026-09-12T00:01:00.000Z",
        elapsedMs: 2_500,
        timingsMs: latestTimings,
      });
    } else {
      expect(result.operations[0]).not.toHaveProperty("latestStartupSample");
    }
    expect(JSON.stringify(result)).not.toMatch(/private-uid|new-startup|old-startup/u);
  });

  it("[T-ADM-004] 각 플랫폼의 최신 시각을 입력 순서와 무관하게 고르고 서버 업무에는 상세를 넣지 않는다", () => {
    const summary = summarizeInteractiveLatency([
      {
        endpoint: "clientStartup", operation: "client.android-app-first-home-complete-paint.v1",
        elapsedMs: 3_000, status: "succeeded", timestamp: "2026-09-12T00:00:00.000Z",
        clientStartupTimingsMs: { authReady: 500 },
      },
      {
        endpoint: "clientStartup", operation: "client.android-app-first-home-complete-paint.v1",
        elapsedMs: 2_000, status: "succeeded", timestamp: "2026-09-12T00:01:00.000Z",
        clientStartupTimingsMs: { authReady: 300 },
      },
      {
        endpoint: "executeHouseholdCommand", operation: "ledger.update-transaction.v1",
        elapsedMs: 100, status: "succeeded", timestamp: "2026-09-12T00:02:00.000Z",
        clientStartupTimingsMs: { authReady: 300 },
      },
    ]);
    expect(summary.find(({ endpoint }) => endpoint === "clientStartup")?.latestStartupSample).toEqual({
      timestamp: "2026-09-12T00:01:00.000Z",
      elapsedMs: 2_000,
      timingsMs: { authReady: 300 },
    });
    expect(summary.find(({ endpoint }) => endpoint === "executeHouseholdCommand"))
      .not.toHaveProperty("latestStartupSample");
  });
