...
import { FirebaseShortcutMessageDiagnosticAdapter } from "../../src/adapters/firebase/payment-capture/firebaseShortcutMessageDiagnostic";
...
describe("FirebaseShortcutMessageDiagnosticAdapter", () => {
  it("인증된 parser 성공 원문을 accepted 진단 문서로 보존한다", async () => {
    ...
    await adapter.retain({
      ...
      rawMessage: "[Web발신]\r\n허유재병원\r\n6,480원 승인",
      normalizedMessage: "[Web발신]\r\n허유재병원\r\n6,480원 승인",
      parserOutcome: { kind: "accepted" },
      requestedAt: "2026-08-03T03:02:59.000Z",
    });

    expect(set).toHaveBeenCalledWith(
      expect.objectContaining({
        diagnosticType: "ios-shortcut-parser-accepted",
    ...
    expect(set.mock.calls[0]?.[0]).not.toHaveProperty("parserRejectionCode");
  });
  ...
  it("인증된 iPhone parser 거부 원문을 관리자 전용 진단 문서로 보존한다", async () => {
    ...
    await adapter.retain({
      ...
      parserOutcome: { kind: "rejected", code: "UNSUPPORTED_MESSAGE" },
  ...
  it("같은 credential과 payload는 동일 진단 문서 ID를 사용한다", async () => {
    ...
    const input = {
      ...
      parserOutcome: {
        ...
        code: "UNSUPPORTED_MESSAGE" as const,
      },
      requestedAt: "2026-08-03T01:42:02.614Z",
    };
