...
suite('[HH-002][ADM-003] 실제 Firestore 운영 교정과 단계별 영구 삭제', () => {
  ...
  it('재시작 후 page부터 재개하고 모든 Context 완료 전 claim 보존, 변경된 claim과 다른 가구·전역 device는 보존한다', async () => {
    ...
    await db.doc('households/h1/ledgerTransactions/e1').set({ transactionId: 'e1', householdId: 'h1', amountInWon: 100 });
    await db.doc('households/h2/ledgerTransactions/e2').set({ transactionId: 'e2', householdId: 'h2', amountInWon: 200 });
    ...
    expect((await db.doc('households/h1/ledgerTransactions/e1').get()).exists).toBe(false);
    expect((await db.doc('households/h2/ledgerTransactions/e2').get()).exists).toBe(true);
