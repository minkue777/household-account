...
test('[T-AST-007][T-AST-010][AST-001][AST-002][AST-003][AST-006][AST-007][AST-009][HH-011] 명의자·자산 생성 수정 정렬 삭제가 저장 결과와 합계에 반영된다', async ({ page, request }) => {
  ...
  await expect.poll(async () => (await documents(request, `households/${scope.householdId}/assets`)).filter(x => x.householdId === scope.householdId).length).toBe(3);
  const assets = await documents(request, `households/${scope.householdId}/assets`);
  ...
  expect(savings).toMatchObject({ currentBalance: 100_000, ownerRef: { kind: 'profile', profileId: owner.id }, lifecycleState: 'active', aggregateVersion: 1 });
  ...
  await expect.poll(async () => (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === savings.id)).toMatchObject({ currentBalance: 120_000, memo: '변경된 메모', aggregateVersion: 2 });
  ...
  await expect.poll(async () => (await documents(request, `households/${scope.householdId}/assets`)).sort((a, b) => Number(a.order) - Number(b.order)).map(x => x.id)).not.toEqual(original);
  const storedOrder = (await documents(request, `households/${scope.householdId}/assets`)).sort((a, b) => Number(a.order) - Number(b.order)).map(x => x.id);
  await page.reload();
...
test('[T-LOAN-002][LOAN-001][LOAN-002][AUTO-003] 실제 대출 자동상환은 원금균등·원리금균등의 다른 감소액을 한 번씩 반영한다', async ({ page, request }) => {
  ...
  for (const asset of assets) expect((await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === asset.id)?.currentBalance).toBe(asset.amount);
  await runScheduled('assetAutomationDaily', `${due}T00:00:00+09:00`);
  for (const asset of assets) expect((await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === asset.id)?.currentBalance).toBe(asset.amount);
});
...
test('[HOLD-001][HOLD-003][HOLD-004][HOLD-005] 수동 보유 항목 CRUD는 부모 평가와 version까지 저장하고 재진입에서 유지된다', async ({ page, request }) => {
  ...
  const asset = (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.name === '증권계좌')!;
  ...
  await expect.poll(async () => (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === asset.id)).toMatchObject({ currentBalance: 15_000, aggregateVersion: 2 });
  ...
  await expect.poll(async () => (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === asset.id)).toMatchObject({ currentBalance: 27_000, aggregateVersion: 3 });
  ...
  await expect.poll(async () => (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === asset.id)).toMatchObject({ currentBalance: 0, costBasis: 0, aggregateVersion: 4 });
...
test('[AUTO-001][AUTO-002][AUTO-003][JOB-ERR-001][JOB-ERR-002] UI 적금 생성 뒤 실제 Scheduler가 도래 월만 한 번 반영한다', async ({ page, request }) => {
  ...
  const asset = (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.name === '자동 적금')!;
  ...
  await runScheduled('assetAutomationDaily', `${due}T00:00:00+09:00`);
  await expect.poll(async () => (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === asset.id)?.currentBalance).toBe(120_000);
  ...
  await runScheduled('assetAutomationDaily', `${due}T00:00:00+09:00`);
  expect((await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === asset.id)?.currentBalance).toBe(120_000);
  await page.reload();
...
test('[T-HOLD-003][HOLD-002][HOLD-003][HOLD-004][AST-001][AST-003] 실제 callable은 코인 평가를 반영하고 stale position/asset 및 잘못된 순서를 원자 거절한다', async ({ page, request }) => {
  ...
  const asset = (await documents(request, `households/${scope.householdId}/assets`)).find(x => x.id === create.assetId)!;
  expect(asset).toMatchObject({ currentBalance: 30_001, costBasis: 25_000, aggregateVersion: 2 });
  const positions = await documents(request, `households/${scope.householdId}/assets/${create.assetId}/positions`);
  expect(positions).toHaveLength(1);
  const before = await documents(request, `households/${scope.householdId}/assets`);
  ...
  expect(await documents(request, `households/${scope.householdId}/assets`)).toEqual(before);
  expect(await documents(request, `households/${scope.householdId}/assets/${create.assetId}/positions`)).toEqual(positions);
