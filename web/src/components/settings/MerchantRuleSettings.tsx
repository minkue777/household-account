...
const MATCH_TYPES: MatchType[] = ['contains', 'exact', 'startsWith', 'endsWith'];
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-slate-500">
          ...
          <span className="block mt-1 text-slate-400">쉼표로 구분하면 OR 조건 (예: 응급, 병원, 의원)</span>
        </p>
      </div>

      <div>
        <label className="block text-sm text-slate-600 mb-1">
          표시용 가맹점명 <span className="text-slate-400">(선택)</span>
        </label>
        <input
          type="text"
          value={ruleMappedMerchant}
          onChange={(e) => setRuleMappedMerchant(e.target.value)}
          placeholder="비워두면 원본 가맹점명을 유지"
          className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
