...
export default function StockHoldingList({
  ...
  const fetchDividendInfo = async (stockCode: string) => {
    if (!stockCode || dividendInfoMap[stockCode] || loadingDividends.has(stockCode)) return;
  ...
  useEffect(() => {
    ...
    holdings.forEach((holding) => {
      if (getHoldingType(holding) !== 'stock' || !holding.stockCode) return;
      if (dividendInfoMap[holding.stockCode] || loadingDividends.has(holding.stockCode)) return;
      void fetchDividendInfo(holding.stockCode);
    });
  ...
  const hasRefreshableHolding = holdings.some(
    (holding) => getHoldingType(holding) === 'stock' && !!holding.stockCode
  );
          ...
          {holdings.map((holding) =>
            editingHolding?.id === holding.id ? (
              ...
              <HoldingItem
                key={holding.id}
                holding={holding}
                dividendInfo={holding.stockCode ? dividendInfoMap[holding.stockCode] : undefined}
                isLoadingDividend={holding.stockCode ? loadingDividends.has(holding.stockCode) : false}
...
function HoldingItem({ holding, dividendInfo, isLoadingDividend, onEdit }: HoldingItemProps) {
  ...
  return (
    ...
    >
      ...
      ) : holdingType === 'stock' && dividendInfo && dividendInfo.recentDividend ? (
        <div className="mt-2 border-t border-slate-200 pt-2">
