...
const legacy = (index: number) => ({ id: 'legacy-' + index, data: () => ({
  assetId: 'TOTAL', date: new Date(Date.UTC(2019, 0, index + 1)).toISOString().slice(0, 10), balance: index,
}) });
...
it('reads 2,014 legacy rows with five bounded pages, in parallel with canonical, and only publishes complete totals', async () => {
  const rows = Array.from({ length: 2014 }, (_, index) => legacy(index));
  ...
  first.resolve({ docs: rows.slice(0, 500) });
  const history = await result;
  expect(read).toHaveBeenCalledTimes(6);
  expect(history.filter(row => row.assetId === 'TOTAL')).toHaveLength(2015);
  expect(history.filter(row => row.assetId === 'TOTAL').at(-1)).toMatchObject({ balance: 3000, changeAmount: 987 });
});
...
it.each(['invalidation', 'remote epoch', 'forced refresh'] as const)('stops the previous full page before requesting another page after %s', async change => {
  ...
  pending.resolve({ docs: Array.from({ length: 500 }, (_, index) => legacy(index)) });
  await rejected;
  expect(read).toHaveBeenCalledTimes(4);
...
it('rejects repeated pages rather than showing a truncated total', async () => {
  const rows = Array.from({ length: 500 }, (_, index) => legacy(index));
