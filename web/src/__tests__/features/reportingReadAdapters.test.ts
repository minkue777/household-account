...
it('reads every bounded expense page before publishing and excludes income/deleted rows using the actual mapper', async () => {
  read.mockResolvedValueOnce({ docs: Array.from({ length: 500 }, (_, i) => expense('a' + i)) }).mockResolvedValueOnce({ docs: [expense('income', { transactionType: 'income' }), expense('deleted', { lifecycleState: 'deleted' }), expense('last', { amount: 10 })] });
  const result = await readExpenseStatistics('2026-09-01', '2026-09-30');
  expect(result).toHaveLength(501); expect(result.reduce((sum, row) => sum + row.amount, 0)).toBe(10);
  expect(where).toHaveBeenCalledWith('date', '>=', '2026-09-01'); expect(where).toHaveBeenCalledWith('date', '<=', '2026-09-30');
  expect(limit).toHaveBeenCalledWith(500); expect(startAfter).toHaveBeenCalledTimes(1);
});
it('rejects repeated cursor records and previous-session replies instead of publishing partial success', async () => {
  const rows = Array.from({ length: 500 }, (_, i) => expense('a' + i));
...
it('reads a 1,743-document year in four bounded requests instead of 35, with every row included', async () => {
  const rows = Array.from({ length: 1743 }, (_, i) => expense('expense-' + i, { amount: i }));
  for (let start = 0; start < rows.length; start += 500) read.mockResolvedValueOnce({ docs: rows.slice(start, start + 500) });
  const result = await readExpenseStatistics('2025-10-01', '2026-09-30');
  expect(read).toHaveBeenCalledTimes(4);
  expect(result).toHaveLength(1743);
  expect(new Set(result.map(row => row.id)).size).toBe(1743);
  expect(result.reduce((total, row) => total + row.amount, 0)).toBe(1743 * 1742 / 2);
});
it('stops between pages when the captured revision is invalidated', async () => {
  let current = true;
  read.mockImplementationOnce(async () => {
    current = false;
    return { docs: Array.from({ length: 500 }, (_, i) => expense('expense-' + i)) };
  });
